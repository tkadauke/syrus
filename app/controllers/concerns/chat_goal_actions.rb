module ChatGoalActions
  ...
  def upsert_goal
    chat_session = find_chat_session
    goal_attrs = chat_goal_attributes(chat_session)
    return if performed?
  ...
  def chat_goal_attributes(chat_session)
    goal_params = params[:goal].presence || params
    prompt = goal_params[:prompt].to_s.strip
    if prompt.blank?
    ...
    attrs = {
      prompt: prompt,
      completion_condition: goal_params[:completion_condition].to_s.strip.presence,
      mode_snapshot: chat_goal_mode_snapshot(chat_session),
      approval_policy: goal_params[:approval_policy].to_s.strip.presence || "manual",
      auto_file_proposals: boolean_param(goal_params[:auto_file_proposals]) || false,
      auto_submit_jobs: boolean_param(goal_params[:auto_submit_jobs]) || false
    }
  ...
  def chat_goal_mode_snapshot(chat_session)
    {
      "mode" => chat_session.mode.presence || "planning",

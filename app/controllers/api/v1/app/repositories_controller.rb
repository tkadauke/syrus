    ...
    module App
      class RepositoriesController < BaseController
        ...
        def release_needs_triage_job
          return unless (repository = find_repository_for_admin_mutation)
          unless can_release_triage_jobs?(repository)
        ...
        def repository_form_json(repository)
          {
            ...
            land_on_inherited_check_failure: repository.land_on_inherited_check_failure?,
            known_flaky_failure_dismissal_enabled: repository.known_flaky_failure_dismissal_enabled?,
            known_flaky_failure_min_score: repository.known_flaky_failure_min_score,
            new_test_flakiness_gate_enabled: repository.new_test_flakiness_gate_enabled?,
            new_test_flakiness_gate_repeats: repository.new_test_flakiness_gate_repeats,
            isolated_repro_dismissal_enabled: repository.isolated_repro_dismissal_enabled?,
            ...
            distributed_workflow_dag_enabled: repository.distributed_workflow_dag_enabled?,
        ...
        def visual_review_mode(visual_review)
          return "not configured" unless visual_review
          return "instance default" if visual_review.enabled.nil?

          visual_review.enabled ? "enabled" : "disabled"
        end
        ...
        def needs_triage_jobs_json(repository)
          return [] unless can_release_triage_jobs?(repository)
        ...
        def needs_triage_jobs_count(repository)
          return 0 unless can_release_triage_jobs?(repository)
        ...
        def can_release_triage_jobs?(repository)
          (Current.user&.admin? || Current.user&.developer?) &&
            RepositoryPolicy.new(Current.user, repository).admin?
        end

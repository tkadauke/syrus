      ...
      class JobTestResultsController < BaseController
        def index
          ...
          render json: {
            ...
            test_runs: test_runs.map { |tr| test_run_json(tr, flakiness_by_pair) }
          }
        end
        ...
        def test_run_json(test_run, flakiness_by_pair)
          suites = test_run.test_cases.group_by(&:suite_name).map do |suite_name, cases|
            {
              ...
              test_cases:    cases.map { |tc| test_case_json(tc, flakiness_by_pair) }
            }
          end
        ...
        def test_case_json(test_case, flakiness_by_pair)

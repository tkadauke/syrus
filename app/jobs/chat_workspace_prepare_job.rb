class ChatWorkspacePrepareJob < ApplicationJob
  ...
  def mark_prepare_failed!(chat_session, message)
    ...
    return unless blocked_goal_continuation?(chat_session)

    chat_session.messages.create!(
      role: "system",
      content: {
        "text" => "Coding checkout preparation failed. Goal continuation is waiting for a ready checkout.",
        "source" => "coding_checkout_readiness",
        "prepare_status" => "failed",
        "prepare_failure" => message.to_s.truncate(2_000)
      }
    )
    chat_session.update!(last_message_at: Time.current)
  end

  def blocked_goal_continuation?(chat_session)
    chat_session.active_goal&.requires_ready_coding_checkout_for_continuation? &&
      chat_session.queued_messages.any? { |queued| queued.content.is_a?(Hash) && queued.content["goal_continuation"] == true }
  end
end

class Job < ApplicationRecord
  ...
  has_many :diff_review_versions, dependent: :destroy
  ...
  has_many :workflows, -> { order(:created_at, :id) }, dependent: :destroy
  ...
  def switch_job_provider_setting!(setting, model: nil, effort_level: nil)
    ...
    ProviderAvailabilityWakeup.call(provider: next_provider, user: user, force: true)
  end
  ...
  def cancel_queued_retry_workflows_after_approval
    WorkUnits::WorkflowCancellation.cancel_queued_retry_workflows_for_job!(job: self, reason: "job_approved")
  end
  ...
  def epic_children_share_one_effective_owner
    ...
    # "Open" here deliberately matches LandingBundleAssembler::Scopes::Epic's
    # open-children query (state != "closed"), not the TERMINAL_STATES-based
    # open_threads/open? used elsewhere on Job -- this validation exists
    # specifically to keep that scope's landing bundle single-owner.

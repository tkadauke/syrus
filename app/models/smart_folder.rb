...
class SmartFolder < ApplicationRecord
  ...
  JOB_BUILTINS = [
    ...
    { key: "promotion_pending",        name: "Promotion pending",        visibility: :when_present, filter: user_job_attention_preset_filter("promotion_pending") },
    { key: "delivery_needs_attention", name: "Delivery needs attention", visibility: :when_present, filter: user_job_attention_preset_filter("delivery_needs_attention") },
  ...
  SPAWNED_PROCESS_BUILTINS = [
    ...
    # `outcome`, not `state`: the spawned-process state chip only knows
    # running/finished, so filtering it by "failed" compiled to `1=0` and this
    # folder could never match. Being :when_present, it was then hidden on
    # every render — silently invisible rather than visibly broken.
    { key: "recently_failed", name: "Recently failed", visibility: :when_present, filter: { "and" => [ { "field" => "outcome", "op" => "is", "value" => "failed" }, { "field" => "started_at", "op" => "within_last", "value" => { "n" => 1, "unit" => "hours" } } ] } }
  ].freeze
  ...
  def self.ensure_builtin_set!(subject, definitions)
    ...
    Rails.cache.fetch(cache_key, expires_in: BUILTIN_RECONCILE_CACHE_TTL) do
      with_builtin_reconcile_lock(subject) do
  ...
  def self.reconcile_builtin_set!(subject, definitions)
    ...
    definitions.each_with_index do |definition, index|
      existing_by_name[definition.fetch(:name)] = reconcile_builtin_definition!(subject, definition, index, existing_by_name[definition.fetch(:name)])
    end

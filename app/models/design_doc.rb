class DesignDoc < ApplicationRecord
  ...
  belongs_to :current_version, class_name: "DesignDocVersion", optional: true

  has_many :design_doc_repositories, dependent: :destroy
  has_many :repositories, through: :design_doc_repositories
  has_many :collaborators, class_name: "DesignDocCollaborator", dependent: :destroy
  has_many :collaborator_users, through: :collaborators, source: :user
  has_many :versions, class_name: "DesignDocVersion", dependent: :destroy
  has_many :anchors, class_name: "DesignDocAnchor", dependent: :destroy
  has_many :threads, class_name: "DesignDocThread", dependent: :destroy
  has_many :suggestions, class_name: "DesignDocSuggestion", dependent: :destroy
  ...
  def self.visible_to(user)
    ...
    collaborator_doc_ids = DesignDocCollaborator.where(user: user).select(:design_doc_id)
    public_doc_ids = DesignDocRepository

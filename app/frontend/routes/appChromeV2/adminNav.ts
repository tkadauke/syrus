...
export const CORE_ADMIN_NAV_ITEMS: readonly CoreAdminNavItem[] = [
  ...
  {
    id: "reconciler_activity",
    labelKey: "nav_reconciler_activity",
    to: "/admin/reconciler_activity",
    paths: ["/admin/reconciler_activity"],
    groupId: "operations",
    order: 40
  },
  ...
  { id: "console", labelKey: "nav_console", to: "/admin/console", paths: ["/admin/console"], groupId: "observability", order: 10 },
  {
    ...
    labelKey: "nav_resource_admission",
    to: "/admin/resource_admission",
    paths: ["/admin/resource_admission"],
  ...
  {
    id: "backend_exceptions",
    labelKey: "nav_backend_exceptions",
    to: "/admin/backend_exceptions",
    paths: ["/admin/backend_exceptions"],
    groupId: "observability",
    order: 40
  },
  ...
  {
    id: "features",
    labelKey: "nav_features",
    to: "/admin/features",
    paths: ["/admin/features"],
    groupId: "system",
    order: 20,
    visible: (ff) => Object.keys(ff).length > 0
  },
  { id: "plugins", labelKey: "nav_plugins", to: "/admin/plugins", paths: ["/admin/plugins"], groupId: "system", order: 30 },
  {
    id: "scoped_chat_events",
    labelKey: "nav_scoped_chat_events",
    to: "/admin/scoped_chat_events",
    paths: ["/admin/scoped_chat_events"],
    groupId: "product_data",
    order: 10
  }
]
...
export function buildAdminNavItems(
  ...
  const coreItems: MergedAdminNavItem[] = CORE_ADMIN_NAV_ITEMS.filter((item) => !item.visible || item.visible(featureFlags)).map((item) => ({
    id: item.id,
  ...
  const groups = ADMIN_NAV_GROUPS.map((group) => ({
    group,
    items: nonOverview.filter((item) => item.groupId === group.id).sort((a, b) => a.order - b.order || a.label.localeCompare(b.label))
  })).filter(({ items }) => items.length > 0)

  const claimedIds = new Set(groups.flatMap(({ items }) => items.map((i) => i.id)))
  const ungroupedExtensions = nonOverview.filter((item) => !claimedIds.has(item.id)).sort((a, b) => a.order - b.order || a.label.localeCompare(b.label))

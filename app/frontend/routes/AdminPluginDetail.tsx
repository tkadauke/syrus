    ...
    <Surface aria-label={t("plugins.links_heading")} className="space-y-3">
      ...
      {links.length > 0 ? (
        <div className="flex flex-wrap gap-2">
          {links.map((link) => <PluginLinkAction key={`${link.label}-${link.path}`} link={link} />)}
        </div>
      ) : (
...
function PluginLinkAction({ link }: { link: AdminPluginLink }) {
  ...
  return <Link className={buttonClasses("primary")} title={link.description || undefined} to={link.path}>{link.label}</Link>
}

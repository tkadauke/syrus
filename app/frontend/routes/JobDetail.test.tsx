...
describe("JobDetailView", () => {
  ...
  it("renders summary route panels with shared section surfaces and dense readable content", () => {
    const { container } = renderJobDetail(
      jobPayload({
        ...
        test_plan: {
          workflow_id: 1,
          steps: ["Open the workflow tab and inspect long log output.", "Open the chat tool card and copy raw details."],
  ...
  it("opens an implemented Job directly in Coding Mode chat from the actions menu", async () => {
    ...
    await waitFor(() => {
      expect(fetchSpy).toHaveBeenCalledWith(
        "/api/v1/app/jobs/1/open_in_coding_mode",
        expect.objectContaining({
          method: "POST"
        })
      )
  ...
  it("opens request-changes feedback in Coding Mode chat", async () => {
    const fetchSpy = vi
      .spyOn(window, "fetch")
      .mockResolvedValue(jsonResponse({ redirect_to: "/chats/42", message: "Opened Coding Mode chat and queued your feedback." }, 200))
    renderJobDetail(
      jobPayload({
        job: { ...baseJob(), state: "approved", summary_state: "approved" },
        actions: { ...jobPayload().actions, can_open_in_coding_mode: true }
  ...
  it("wraps long test plan entries in the Summary tab instead of widening the card", () => {
    const longStep = [
      ...
      "source_snapshot_projection_metadata_fingerprint",
...
describe("Job detail navigation", () => {
      ...
      jobNavigationContext({
        ...
        items: [
          { id: 1, slug: "JOB-1", path: "/jobs/1", title: "First snapshot title", repository: "acme/widgets", state: "running" },
          {
            ...
            slug: "JOB-2",
            path: "/jobs/2",
            title: "Second snapshot title",
            repository: "acme/widgets",
            state: "implemented",
            ownerBadge: { label: "jane@example.com", kind: "other_user" },
  ...
  it("ignores navigation shortcuts while a dialog is open", () => {
    storeJobNavigationContext(jobNavigationContext({ currentJobId: 2 }))
    renderJobDetail(
      jobPayload({ job: { ...baseJob(), id: 2, state: "implemented", summary_state: "implemented" }, actions: { ...jobPayload().actions, can_approve: true } }),
      {
        initialEntry: "/app-shell/jobs/2?job_nav=nav-token",
        showLocation: true
      }

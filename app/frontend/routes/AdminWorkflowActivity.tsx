...
export function AdminWorkflowActivity() {
  ...
  return (
    ...
    >
      <AdminEventFilterBar
        clearLabel={t("activity.clear_filters")}
        filter={activity.data?.filter}
        filterSchema={activity.data?.filter_schema}
        fields={[
          {
            name: "event_type",
            label: t("activity.filter_event_type"),
            options: [{ value: "", label: t("activity.all_event_types") }, ...(activity.data?.event_types || []).map((type) => ({ value: type, label: type }))]
          },
        ...
        searchLabel={t("activity.apply_filters")}
...
function ActivityTable({
  ...
  payload: AdminWorkflowActivityPayload
  prefix: string
  search: string
}) {
  const { t } = useT("admin")
...
function Pagination({ pagination, prefix }: { pagination: AdminWorkflowActivityPayload["pagination"]; prefix: string }) {
  ...
  return (
    <nav
      aria-label={t("activity.aria_pagination")}
    ...
    >
      ...
      <div className="flex items-center gap-2">
        {pagination.previous_path ? (
          ...
          <span className={disabledPaginationClass()}>{t("activity.previous")}</span>
        )}
        {pagination.next_path ? (
          ...
          <span className={disabledPaginationClass()}>{t("activity.next")}</span>
        )}
      </div>
    </nav>
  )

...
describe("ChatJobStatusPanel blocker banner", () => {
  it("shows an amber (not red, not the warning token) blocker banner for awaiting_review, since it is an expected step, not a failure", async () => {
    ...
    // The "warning" semantic token collapses onto the brand accent color
    // (a muted red-orange) in the built-in Terracotta theme, so this must
    // not fall back to it either -- see ChatJobStatusPanel.tsx for details.
    expect(banner.className).not.toMatch(/warning/)

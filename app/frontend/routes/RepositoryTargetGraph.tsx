...
const TARGET_HEALTH_RECORD_PREFIX = "THR-"
...
export function RepositoryTargetGraphRoute() {
  ...
  return (
    <RepositoryPageShell
      ...
      ariaLabel={t("target_graph.repository_aria")}
      heading={query.data ? (
        <PageHeading mono>
    ...
    >
      ...
      {query.data ? (
        <TargetGraphExplorer
          backLink={{ label: t("target_graph.overview"), path: `/repositories/${query.data.repository.id}` }}
          payload={query.data}
          prefix={prefix}
          query={graphQuery}
        />
      ...
      {payload.targets.length === 0 ? (
        ...
        <section className="overflow-hidden rounded border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-200 px-4 py-3 dark:border-gray-700">
            ...
            <span className="text-xs text-gray-500 dark:text-gray-400">{t("target_graph.rendered_of", { rendered: payload.targets.length.toLocaleString(), total: payload.page.total.toLocaleString() })}</span>
          </div>
          <TargetGraphCanvas
            rows={rows}
...
function TargetHealthRefs({ entry }: { entry: TargetGraphTargetExplanation }) {
  ...
  return (
    <div className="mt-2 flex flex-wrap gap-1.5 text-xs">
      {entry.target_health_record_id ? <span className="rounded bg-gray-100 px-2 py-1 font-mono text-gray-700 dark:bg-gray-800 dark:text-gray-300">{TARGET_HEALTH_RECORD_PREFIX}{entry.target_health_record_id}</span> : null}
      ...
      {refs.map((ref, index) => (
        <span className="rounded bg-gray-100 px-2 py-1 font-mono text-gray-700 dark:bg-gray-800 dark:text-gray-300" key={`${ref.target_health_record_id}-${index}`}>
          {TARGET_HEALTH_RECORD_PREFIX}{ref.target_health_record_id ?? "?"}{ref.status ? t("target_graph.health_ref_status", { status: ref.status }) : ""}
        </span>
      ))}
    </div>
  )
    ...
    <section className="rounded border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
      <div className="grid gap-3 xl:grid-cols-[minmax(18rem,1fr)_repeat(2,minmax(8rem,12rem))]">
        <form className="flex min-w-0 gap-2" onSubmit={onSubmit}>
          <Input
            aria-label={t("target_graph.search_label")}
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        {FOCUS_STATES.map((state) => (
          ...
          <Button
            ...
            size="sm"
            title={disabled ? t("target_graph.overlay_unavailable") : undefined}
            type="button"
...
function TargetGraphStats({ payload, query }: { payload: TargetGraphPayload; query: TargetGraphQuery }) {
  ...
  return (
    <div className="grid gap-3 md:grid-cols-4">
      ...
      <StatBox label={t("target_graph.mode")} value={query.mode === "window" ? t("target_graph.browse") : t("target_graph.neighborhood")} detail={query.focusLabel || query.focusState || t("target_graph.first_target")} />
      <div className="rounded border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-900">
        ...
        <div className="mt-2 flex min-h-7 flex-wrap gap-1.5">
          {healthSummary.length === 0 ? <span className="text-sm text-gray-500 dark:text-gray-400">{t("target_graph.no_records")}</span> : null}
...
function stateLabel(state: (typeof FOCUS_STATES)[number], t: TFunction<"common">) {
...
function ambiguityLabel(kind: string, t: TFunction<"common">) {

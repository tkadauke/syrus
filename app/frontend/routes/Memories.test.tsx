...
describe("MemoriesRoute", () => {
  ...
  it("updates query params when filters are applied", async () => {
    ...
    fireEvent.change(screen.getAllByLabelText("Scope")[1], { target: { value: "repository" } })

    await waitFor(() => {
      expect(fetchSpy).toHaveBeenCalledWith("/api/v1/app/memories?scope=repository", expect.objectContaining({ credentials: "same-origin" }))
    })
  })
  ...
  it("creates, edits, publishes, and deletes through the API", async () => {
    ...
    await screen.findByText("Use Rails for the app.")
    fireEvent.change(screen.getByLabelText("Content"), { target: { value: "Remember this." } })
    ...
    fireEvent.click(screen.getByText("Use Rails for the app."))
    const editArea = await screen.findByLabelText("Content for memory 10")
    fireEvent.change(editArea, { target: { value: "Use Rails and Vite." } })
    fireEvent.keyDown(editArea, { key: "Enter" })

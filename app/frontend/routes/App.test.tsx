...
describe("App", () => {
  ...
  it("loads bootstrap data into the SPA shell", async () => {
    ...
    expect(actionCable.createSubscription).toHaveBeenCalledWith({ channel: "AppUserChannel" }, expect.objectContaining({ received: expect.any(Function) }))
  })
  ...
  it("routes bare /app-shell to the dashboard for signed-in users", async () => {
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation(async (input) => {
      ...
      if (path.startsWith("/api/v1/app/dashboard")) {
        return new Response(JSON.stringify(dashboardPayload({ subject: "job", view: "list", items: [dashboardJobItem()] })), {
          status: 200,
        ...
        system_alerts: [
          {
            ...
            message:
              "SYRUS_DATA_ROOT is 96% full with <code>3.5GB</code> available. On single-host Docker installs this volume shares a disk with Docker's own image store.",
            action_steps: [
  ...
  it("renders a minimal first-run welcome for a new instance", async () => {
    ...
    expect(fetchSpy).toHaveBeenCalledWith("/api/v1/app/bootstrap", expect.objectContaining({ credentials: "same-origin" }))
  })
  ...
  it("renders invitation-only landing CTA for locked instances", async () => {
    ...
    expect(
      screen.getByText("Access to this Syrus instance is controlled by its operator. Use an invitation link, or sign in with an existing account.")
    ).toBeInTheDocument()
  })
  ...
  it("routes desktop-shell invitation links straight to the invite signup", async () => {
    ...
    try {
      ...
      expect(fetchSpy).toHaveBeenCalledWith("/api/v1/app/auth/signup?token=invite-123", expect.objectContaining({ credentials: "same-origin" }))
    } finally {
      restoreUserAgent()
    }
  }, 15000)
  ...
  it("renders the sign-up route from the public signup payload", async () => {
    ...
    expect(fetchSpy).toHaveBeenCalledWith("/api/v1/app/auth/signup", expect.objectContaining({ credentials: "same-origin" }))
  })

  it("shows the sign-in link on the sign-up route once users exist", async () => {
    vi.spyOn(window, "fetch").mockResolvedValue(
      new Response(JSON.stringify({ allowed: true, first_signup: false, signups_open: true, invitation: null }), {
        status: 200,
        headers: { "Content-Type": "application/json" }
      })
    )
  ...
  it("renders the logged-out landing CTA from public bootstrap state", async () => {
    const fetchSpy = vi
      .spyOn(window, "fetch")
      .mockResolvedValue(new Response(JSON.stringify(publicBootstrapPayload()), { status: 200, headers: { "Content-Type": "application/json" } }))
  ...
  it("renders shared app chrome from embedded bootstrap data", async () => {
    ...
    try {
      ...
      expect(within(accountNav).getByRole("link", { name: "Admin" })).toHaveAttribute("title", "Curia — The Roman Senate house")
      expect(within(accountNav).getByRole("button", { name: "Sign out" })).toBeInTheDocument()
      expect(fetchSpy).toHaveBeenCalledWith("/api/v1/app/chats", expect.objectContaining({ credentials: "same-origin" }))
    } finally {
      script.remove()
    }
  })
  ...
  it("toggles and persists the app shell theme", async () => {
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation(async (input) => {
      ...
      if (path.startsWith("/api/v1/app/dashboard")) {
        return new Response(JSON.stringify(dashboardPayload({ subject: "job", view: "list", items: [] })), {
          status: 200,
          headers: { "Content-Type": "application/json" }
        })
      }
  ...
  it("renders a Publilius Syrus quote in the footer on non-chat routes", async () => {
    ...
    const fetchSpy = vi
      .spyOn(window, "fetch")
      .mockResolvedValue(new Response(JSON.stringify({ groups: [], repositories: [] }), { status: 200, headers: { "Content-Type": "application/json" } }))
  ...
  it("hides the Publilius Syrus quote on chat routes", async () => {
    const fetchSpy = vi
      .spyOn(window, "fetch")
      .mockResolvedValue(new Response(JSON.stringify(chatPayload({ messages: [] })), { status: 200, headers: { "Content-Type": "application/json" } }))
  ...
  it("renders the v2 sidebar navigation and account popup actions", async () => {
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/chats" && (init as RequestInit)?.method === "POST") {
        return Promise.resolve(
          new Response(JSON.stringify({ message: "Chat created.", redirect_to: "/chats/8", chat: chatPayload().chat }), {
            status: 201,
            headers: { "Content-Type": "application/json" }
          })
        )
    ...
    try {
      ...
      expect(fetchSpy).toHaveBeenCalledWith("/api/v1/app/chats", expect.objectContaining({ method: "POST", body: undefined }))
    } finally {
      fetchSpy.mockRestore()
      script.remove()
    }
  ...
  it("renders the v2 chat search empty state and all filter fields", async () => {
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input) => {
      ...
      if (path.includes("field=repository_id")) {
        return Promise.resolve(
          new Response(JSON.stringify({ options: [{ value: "3", label: "acme/widgets" }] }), { status: 200, headers: { "Content-Type": "application/json" } })
        )
      }
      if (path.includes("field=epic_id")) {
        return Promise.resolve(
          new Response(JSON.stringify({ options: [{ value: "5", label: "EPIC-5 Search" }] }), { status: 200, headers: { "Content-Type": "application/json" } })
        )
      }
      if (path.includes("field=job_id")) {
        return Promise.resolve(
          new Response(JSON.stringify({ options: [{ value: "8", label: "JOB-8 Recent job" }] }), {
            status: 200,
            headers: { "Content-Type": "application/json" }
          })
        )
  ...
  it("renders v2 chat search result cards, expands matches, and follows message links", async () => {
    ...
    try {
      ...
      await waitFor(() => {
        expect(fetchSpy).toHaveBeenCalledWith("/api/v1/app/chats/77", expect.objectContaining({ credentials: "same-origin" }))
      })
    } finally {
      vi.useRealTimers()
      fetchSpy.mockRestore()
  ...
  it("restores hidden chats from v2 settings", async () => {
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/settings/hidden_chats?page=1") {
        return Promise.resolve(
          new Response(JSON.stringify({ chats: hiddenChats, total: hiddenChats.length, page: 1, per_page: 20, total_pages: hiddenChats.length > 0 ? 1 : 0 }), {
            status: 200,
            headers: { "Content-Type": "application/json" }
          })
        )
  ...
  it("groups recent chats in the v2 sidebar", async () => {
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input) => {
      ...
      if (path === "/api/v1/app/chats") {
        return Promise.resolve(
          new Response(JSON.stringify({ groups: recentGroups, repositories: [] }), { status: 200, headers: { "Content-Type": "application/json" } })
        )
      }
      if (path === "/api/v1/app/chats/more?repository_id=3&before_id=14") {
        return Promise.resolve(
          new Response(JSON.stringify({ chats: [recentChats[7]], has_more: false }), { status: 200, headers: { "Content-Type": "application/json" } })
        )
      }
    ...
    try {
      ...
      const headers = within(recentNav)
  ...
  it("hides a chat from the v2 recent chats sidebar", async () => {
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/chats") {
        return Promise.resolve(
          new Response(JSON.stringify({ groups: sidebarGroups(recentChats), repositories: [] }), {
            status: 200,
            headers: { "Content-Type": "application/json" }
          })
        )
  ...
  it("reveals the other nav tabs as soon as the onboarding chat starts, without a reload", async () => {
    ...
    vi.spyOn(window, "fetch").mockImplementation(async (input, init) => {
      ...
      if (url.endsWith("/api/v1/app/chats/onboarding") && (init as RequestInit)?.method === "POST") {
        return new Response(JSON.stringify({ message: "Chat created.", redirect_to: "/chats/5", chat: chatPayload().chat }), {
          status: 201,
          headers: { "Content-Type": "application/json" }
        })
      }
  ...
  it("renders the chat route", async () => {
    ...
    const fetchSpy = vi
      .spyOn(window, "fetch")
      .mockResolvedValue(new Response(JSON.stringify(chatPayload()), { status: 200, headers: { "Content-Type": "application/json" } }))
  ...
  it("submits bug reports from the shared app chrome", async () => {
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/bug_reports" && init?.method === "POST") {
        return Promise.resolve(
          new Response(JSON.stringify({ message: "Bug report queued.", job_id: 44 }), { status: 201, headers: { "Content-Type": "application/json" } })
        )
      }
  ...
  it("scales oversized full-page bug report screenshots instead of rejecting them", async () => {
    ...
    try {
      ...
      fireEvent.click(screen.getByRole("radio", { name: "Full page" }))
      await waitFor(() => expect(html2canvasMock).toHaveBeenCalledTimes(2))
      const fullPageCall = html2canvasMock.mock.calls[1] as unknown as [
        HTMLElement,
        { scale?: number; scrollX?: number; scrollY?: number; width?: number; height?: number }
      ]
  ...
  it("submits bug reports without a screenshot when that option is selected", async () => {
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/bug_reports" && init?.method === "POST") {
        return Promise.resolve(
          new Response(JSON.stringify({ message: "Bug report queued.", job_id: 45 }), { status: 201, headers: { "Content-Type": "application/json" } })
        )
      }
  ...
  it("accepts bug reports with the command-enter shortcut", async () => {
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/bug_reports" && init?.method === "POST") {
        return Promise.resolve(
          new Response(JSON.stringify({ message: "Bug report queued.", job_id: 46 }), { status: 201, headers: { "Content-Type": "application/json" } })
        )
      }
  ...
  it("renders the spending insights route from the app API", async () => {
        ...
        new Response(
          JSON.stringify({
            ...
            breakdowns: {
              ...
              users: [
                { id: 2, label: "operator@example.com", path: "/profiles/2", jobs_count: 2, total_usd: 3.75, average_job_usd: 1.875, last_30_days_usd: 3.75 }
              ],
            ...
            top_runs: [
              {
                ...
                job: {
                  id: 9,
                  title: "Replace Meyer's-singleton statics with inline-variable constants across every remaining math header",
                  path: "/jobs/9?tab=workflows#workflow-4"
                },
    ...
    expect(fetchSpy).toHaveBeenCalledWith(
      "/api/v1/app/insights/spending?start_date=2026-06-01&end_date=2026-06-05",
      expect.objectContaining({ credentials: "same-origin" })
    )
    expect(screen.getByRole("link", { name: "EPIC-7 / Cost Senate" })).toHaveAttribute("href", "/app-shell/epics/7")
    const topRunJobLink = screen.getByRole("link", {
      name: "Replace Meyer's-singleton statics with inline-variable constants across every remaining math header"
    })
  ...
  it("persists the selected dashboard view when the view toggle is clicked", async () => {
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/dashboard/preferences" && init?.method === "PATCH") {
        return Promise.resolve(
          new Response(JSON.stringify({ message: "Dashboard preferences updated.", dashboard_preferences: {} }), {
            status: 200,
            headers: { "Content-Type": "application/json" }
          })
        )
      ...
      return Promise.resolve(
        new Response(JSON.stringify(dashboardPayload({ subject: "job", view: path.includes("view=kanban") ? "kanban" : "list" })), {
          status: 200,
          headers: { "Content-Type": "application/json" }
        })
      )
  ...
  it("shows dashboard subjects in the sidebar", async () => {
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input) => {
      ...
      if (dashboardPathMatches(path, "/api/v1/app/dashboard?view=list&subject=job")) {
        return Promise.resolve(
          new Response(JSON.stringify(dashboardPayload({ subject: "job", view: "list" })), { status: 200, headers: { "Content-Type": "application/json" } })
        )
      }
  ...
  it("shows a chat preview card on hover for the dashboard source chat slug", async () => {
    const restoreMedia = mockMediaQuery(true)
    try {
      mockDashboardFetchWithChatPreview(
        dashboardPayload({
          subject: "job",
          view: "list",
          items: [
        ...
        entries: [
          {
            ...
            blocker_jobs: [
              {
                id: 2,
                title: "Prepare data layer",
                job_path: "/jobs/2",
                state: "open",
                pr_number: 22,
                pr_path: "https://github.com/acme/widgets/pull/22",
                epic_id: 20,
                epic_title: "Data Layer",
                repository: { id: 3, slug: "acme/widgets", repository_path: "/repositories/3" },
  ...
  it("resets queue sorting outside the landing queue folder", async () => {
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation(async (input, init) => {
      ...
      if (path === "/api/v1/app/dashboard/preferences") {
        ...
        return new Response(JSON.stringify({ message: "Dashboard preferences updated.", dashboard_preferences: {} }), {
          status: 200,
          headers: { "Content-Type": "application/json" }
        })
      }
  ...
  it("renders dashboard smart folder visibility groups and badges", async () => {
    ...
    try {
      ...
      expect(dashboardSections).toHaveClass(
        "inline-flex",
        "max-w-full",
        "flex-wrap",
        "overflow-hidden",
  ...
  it("preserves the active smart folder while dashboard filters change", async () => {
    ...
    vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      const url = new URL(path, "http://example.test")
      if (url.pathname === "/api/v1/app/dashboard" && (url.searchParams.get("smart_folder_id") === "7" || url.searchParams.get("section") === "chrome")) {
    ...
    try {
      ...
      await waitFor(() => {
        expect(
          dashboardRequests.some((request) => {
  ...
  it("limits Kanban lanes to 20 cards and loads more on demand", async () => {
    const runningJobs = Array.from({ length: 25 }, (_, index) =>
      dashboardJobItem({
        id: index + 1,
  ...
  it("renders backlog job kanban cards with source, age, dependencies, and actions", async () => {
    ...
    mockDashboardFetch(
      dashboardPayload({
        ...
        lanes: [{ key: "backlog", title: "Backlog", count: 1, items: [backloggedJob] }],
        kanban_limit: 100
      })
    )
  ...
  it("shows an update button for admin queue saved folder filter drift", async () => {
    ...
    try {
      ...
      await waitFor(() => {
        expect(fetchSpy).toHaveBeenCalledWith("/api/v1/app/smart_folders/10", expect.objectContaining({ method: "PATCH" }))
      })
      const patchCall = fetchSpy.mock.calls.find(
        ([path, init]) => path === "/api/v1/app/smart_folders/10" && (init as RequestInit | undefined)?.method === "PATCH"
      )
      ...
      await waitFor(() => {
        expect(fetchSpy).toHaveBeenCalledWith("/api/v1/app/smart_folders", expect.objectContaining({ method: "POST" }))
      })
  ...
  it("shows an update button for admin process saved folder filter drift", async () => {
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockResolvedValue(
      jsonResponse(
        adminProcessesPayloadWithSavedFolder({
  ...
  it("shows an update button for admin user saved folder filter drift", async () => {
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockResolvedValue(
      jsonResponse(
        adminUsersPayloadWithSavedFolder({
  ...
  it("keeps the transcript scrolled to the bottom when new events arrive at the bottom", async () => {
    ...
    act(() => {
      queryClient.setQueryData(
        ["admin", "transcript", "4", { page: 1, per: 100 }],
        transcriptPayload({
          totalEvents: 2,
          events: [
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockResolvedValue(
      new Response(
        JSON.stringify({
          ...
          users: [{ id: 1, email_address: "operator@example.com", display_name: "Operator" }],
          recent_admin_actions: [
            {
              id: 4,
              action: "pause_runs",
  ...
  it("renders the GitHub App registration route from the app admin API", async () => {
    ...
    expect(
      within(registration).getByText(
        "Register the singleton Syrus GitHub App. Repositories use App credentials only after the App is installed on their GitHub account or repository."
      )
    ).toBeInTheDocument()
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/tags" && init?.method === "POST") {
        return Promise.resolve(
          new Response(
            JSON.stringify({
              palette,
              tags: [{ id: 4, name: "epic:attachments", color: "indigo", jobs_count: 0 }],
              message: "Tag created."
            }),
            { status: 201, headers: { "Content-Type": "application/json" } }
          )
      ...
      return Promise.resolve(
        new Response(
          JSON.stringify({
            palette,
            tags: [{ id: 2, name: "triage", color: "blue", jobs_count: 3 }]
          }),
          { status: 200, headers: { "Content-Type": "application/json" } }
        )
      )
  ...
  it("renders the credentials route as provider cards and connects Claude through the setup flow", async () => {
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/credentials" && init?.method === "PATCH") {
        return Promise.resolve(
          new Response(JSON.stringify(credentialsPayload({ chatProvider: "codex", chatProviders: ["claude", "codex"] })), {
            status: 200,
            headers: { "Content-Type": "application/json" }
          })
        )
      ...
      if (path === "/api/v1/app/credentials/claude_oauth_start" && init?.method === "POST") {
        return Promise.resolve(
          new Response(JSON.stringify({ authorize_url: "https://claude.ai/oauth/authorize?state=abc" }), {
            status: 200,
            headers: { "Content-Type": "application/json" }
          })
        )
      ...
      return Promise.resolve(
        new Response(JSON.stringify(credentialsPayload({ chatProviders: ["claude", "codex"] })), {
          status: 200,
          headers: { "Content-Type": "application/json" }
        })
      )
    ...
    expect(within(claudeCard).getByRole("link", { name: "Anthropic authentication docs" })).toHaveAttribute(
      "href",
  ...
  it("renders notification settings as a sibling settings route", async () => {
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/notification_preferences" && init?.method === "PATCH") {
        return Promise.resolve(
          new Response(JSON.stringify(notificationPreferencesPayload({ job_failed: false, message: "Notification preferences updated." })), {
            status: 200,
            headers: { "Content-Type": "application/json" }
          })
        )
  ...
  it("authorizes Codex ChatGPT login from the Codex card", async () => {
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/credentials/codex_oauth_start" && init?.method === "POST") {
        return Promise.resolve(
          new Response(JSON.stringify({ authorize_url: "https://auth.openai.com/oauth/authorize?state=abc", listener_started: true }), {
            status: 200,
            headers: { "Content-Type": "application/json" }
          })
        )
      ...
      return Promise.resolve(
        new Response(JSON.stringify(credentialsPayload({ codexAuthMode: "chatgpt_login" })), { status: 200, headers: { "Content-Type": "application/json" } })
      )
    })
  ...
  it("renders and saves the agent settings route", async () => {
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/credentials" && init?.method === "PATCH") {
        return Promise.resolve(
          new Response(JSON.stringify(credentialsPayload({ agentProvider: "codex", message: "Credentials updated." })), {
            status: 200,
            headers: { "Content-Type": "application/json" }
          })
        )
    ...
    await waitFor(() => {
      ...
      expect(JSON.parse(String(patchCall?.[1]?.body)).user).toEqual(
        expect.objectContaining({
          agent_provider: "codex",
          agent_max_turns: 42
        })
  ...
  it("renders and saves the preferences route", async () => {
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/credentials" && init?.method === "PATCH") {
        return Promise.resolve(
          new Response(JSON.stringify(credentialsPayload({ schedulingPaused: true, message: "Credentials updated." })), {
            status: 200,
            headers: { "Content-Type": "application/json" }
          })
        )
  ...
  it("rotates an admin API token from the credentials route", async () => {
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/credentials/rotate_api_token" && init?.method === "POST") {
        return Promise.resolve(
          new Response(JSON.stringify(credentialsPayload({ apiToken: true, newApiToken: "syrus_newtoken", message: "API token rotated." })), {
            status: 200,
            headers: { "Content-Type": "application/json" }
          })
        )
      ...
      return Promise.resolve(
        new Response(JSON.stringify(credentialsPayload({ apiToken: true })), { status: 200, headers: { "Content-Type": "application/json" } })
      )
    })
  ...
  it("renders the repository form with GitHub selectors and submits it to the app API", async () => {
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/repositories" && init?.method === "POST") {
        return Promise.resolve(
          new Response(JSON.stringify({ error: { code: "validation_failed", message: "Owner has already been taken" } }), {
            status: 422,
            headers: { "Content-Type": "application/json" }
          })
        )
      ...
      if (path === "/api/v1/app/repositories/repos?owner=acme&owner_type=user") {
        return Promise.resolve(
          new Response(
            JSON.stringify({
              repos: [{ name: "widgets", github_repository_id: 456, github_owner_id: 123 }]
            }),
            { status: 200, headers: { "Content-Type": "application/json" } }
          )
        )
      ...
      if (path === "/api/v1/app/repositories/branches?owner=acme&name=widgets") {
        return Promise.resolve(
          new Response(JSON.stringify({ branches: ["trunk", "main"], default_branch: "trunk" }), {
            status: 200,
            headers: { "Content-Type": "application/json" }
          })
        )
  ...
  it("creates a repository and navigates within the React shell", async () => {
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path.startsWith("/api/v1/app/repositories/branches")) {
        return Promise.resolve(
          new Response(JSON.stringify({ branches: ["main"], default_branch: "main" }), { status: 200, headers: { "Content-Type": "application/json" } })
        )
      }
  ...
  it("renders the edit repository form and patches repository settings", async () => {
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/repositories/3" && init?.method === "PATCH") {
        return Promise.resolve(
          new Response(JSON.stringify({ error: { code: "validation_failed", message: "Trigger label can't be blank" } }), {
            status: 422,
            headers: { "Content-Type": "application/json" }
          })
        )
  ...
  it("runs repository detail commands through the app API", async () => {
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/repositories/3/archive" && init?.method === "POST") {
        return Promise.resolve(
          new Response(JSON.stringify(repositoriesPayload({ message: "acme/widgets archived." })), {
            status: 200,
            headers: { "Content-Type": "application/json" }
          })
        )
    ...
    await waitFor(() => {
      expect(fetchSpy).toHaveBeenCalledWith("/api/v1/app/repositories/3/archive", expect.objectContaining({ method: "POST", credentials: "same-origin" }))
    })
  ...
  it("renders and removes Epic dependencies from the detail page", async () => {
    ...
    await waitFor(() => {
      expect(fetchSpy).toHaveBeenCalledWith("/api/v1/app/epics/7/dependencies/6", expect.objectContaining({ method: "DELETE", credentials: "same-origin" }))
    })
  ...
  it("adds Epic dependencies and reports cycle errors inline", async () => {
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path.startsWith("/api/v1/app/filters/fk_options")) {
        ...
        const options =
          query === "Cycle" ? [{ value: 10, label: "Cycle dependency" }] : query === "Index" ? [{ value: 9, label: "Index chat transcripts" }] : []
        return Promise.resolve(new Response(JSON.stringify({ options }), { status: 200, headers: { "Content-Type": "application/json" } }))
      }
      if (path === "/api/v1/app/epics/7/dependencies" && init?.method === "POST") {
        if (init.body === JSON.stringify({ depends_on_epic_id: 10 })) {
          return Promise.resolve(
            new Response(JSON.stringify({ error: { code: "validation_failed", message: "Depends on epic would create a cycle" } }), {
              status: 422,
              headers: { "Content-Type": "application/json" }
            })
          )
    ...
    await waitFor(() => {
      expect(fetchSpy).toHaveBeenCalledWith("/api/v1/app/filters/fk_options?field=epic_id&q=Index", expect.objectContaining({ credentials: "same-origin" }))
      expect(fetchSpy).toHaveBeenCalledWith(
  ...
  it("claims and unclaims an Epic from the detail controls", async () => {
    ...
    await waitFor(() => {
      expect(fetchSpy).toHaveBeenCalledWith("/api/v1/app/epics/7/claim", expect.objectContaining({ method: "PATCH", credentials: "same-origin" }))
    })
    ...
    await waitFor(() => {
      expect(fetchSpy).toHaveBeenCalledWith("/api/v1/app/epics/7/unclaim", expect.objectContaining({ method: "PATCH", credentials: "same-origin" }))
    })
  ...
  it("renders a Job detail page and runs commands through the app API", async () => {
    ...
    const payload = jobDetailPayload({
      landing_queue_entry: {
        ...
        waiting_for_jobs: [{ id: 43, label: "#43", title: "Approve sibling aqueduct", job_path: "/jobs/43" }]
      },
      dependencies: [
        {
          id: 13,
    ...
    const gradeStep = payload.workflows[0].steps[0] as {
      kind: string
      display_name: string
      details: unknown
      runs: Array<{ app_grade_log_path: string | null }>
    }
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/jobs/42/claim" && init?.method === "POST") {
        ...
        return Promise.resolve(
          new Response(JSON.stringify({ message: "Job claimed.", job: payload.job }), { status: 200, headers: { "Content-Type": "application/json" } })
        )
      }
      if (path === "/api/v1/app/jobs/42/poll_feedback" && init?.method === "POST") {
        return Promise.resolve(
          new Response(JSON.stringify({ message: "Checking PR feedback now..." }), { status: 200, headers: { "Content-Type": "application/json" } })
        )
      }
      ...
      if (path === "/api/v1/app/jobs/42/runs/9/grade_log?name=tests&workflow_id=5") {
        return Promise.resolve(
          new Response(JSON.stringify({ job_id: 42, run_id: 9, name: "tests", contents: "rspec output\n" }), {
            status: 200,
            headers: { "Content-Type": "application/json" }
          })
        )
    ...
    fireEvent.click(screen.getByRole("button", { name: "Claim work" }))
    await waitFor(() => {
      expect(fetchSpy).toHaveBeenCalledWith("/api/v1/app/jobs/42/claim", expect.objectContaining({ method: "POST", credentials: "same-origin" }))
    })
    ...
    await waitFor(() => {
      expect(fetchSpy).toHaveBeenCalledWith("/api/v1/app/jobs/42/poll_feedback", expect.objectContaining({ method: "POST", credentials: "same-origin" }))
    })
  ...
  it("coalesces adjacent transcript chunks from the same command source", async () => {
    ...
    expect(
      await screen.findByText(
        (_, element) =>
          element?.tagName === "PRE" && element.textContent === "[prepare] (1/2) $ bundle install\nFetching rack 3.2.6\nFetching rack-session 2.1.2"
      )
    ).toBeInTheDocument()
    expect(
      screen.getByText((_, element) => element?.tagName === "PRE" && element.textContent === "[prepare] (2/2) $ npm ci\nadded 42 packages")
    ).toBeInTheDocument()
    expect(
      screen.queryByText(
        (_, element) =>
  ...
  it("uses the job name as the Job detail title and moves source metadata below it", async () => {
    ...
    vi.spyOn(window, "fetch").mockResolvedValue(new Response(JSON.stringify(payload), { status: 200, headers: { "Content-Type": "application/json" } }))
  ...
  it("keeps the admin-only Job timeline collapsed until opened", async () => {
    ...
    expect(fetchSpy).not.toHaveBeenCalledWith("/api/v1/app/jobs/42/timeline", expect.objectContaining({ credentials: "same-origin" }))
    ...
    expect(fetchSpy).toHaveBeenCalledWith("/api/v1/app/jobs/42/timeline", expect.objectContaining({ credentials: "same-origin" }))
  })
      ...
      workflows: [
        {
          ...
          id: 15,
          slug: "WF-15",
          path: "/jobs/42?tab=workflows&workflows_page=2#workflow-15",
          trigger_kind: "retry",
          steps: []
        },
        {
          ...
          id: 16,
          slug: "WF-16",
          path: "/jobs/42?tab=workflows&workflows_page=2#workflow-16",
  ...
  it("renders the Job source browser from the app source API", async () => {
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input) => {
      ...
      if (path.startsWith("/api/v1/app/jobs/42/source?")) {
        return Promise.resolve(
          new Response(JSON.stringify(jobSourcePayload({ withFile: true })), { status: 200, headers: { "Content-Type": "application/json" } })
        )
      }
  ...
  it("switches the Job source browser into diff mode", async () => {
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path.startsWith("/api/v1/app/jobs/42/source_diff?")) {
        return Promise.resolve(
          new Response(JSON.stringify(jobSourceDiffPayload({ baseRef: "deadbeef12345678", headRef: "aabbccdd1234567" })), {
            status: 200,
            headers: { "Content-Type": "application/json" }
          })
        )
    ...
    await waitFor(() => {
      expect(fetchSpy).toHaveBeenCalledWith("/api/v1/app/jobs/42/source_diff?base=deadbeef12345678", expect.objectContaining({ credentials: "same-origin" }))
    })
  })
  ...
  it("dispatches Job header commands through the app API", async () => {
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (commandPaths.has(path)) {
        return Promise.resolve(
          new Response(JSON.stringify({ message: commandPaths.get(path) }), { status: 200, headers: { "Content-Type": "application/json" } })
        )
      }
      if (path === "/api/v1/app/jobs/42/timeline") {
        return Promise.resolve(new Response(JSON.stringify(jobTimelinePayload()), { status: 200, headers: { "Content-Type": "application/json" } }))
    ...
    const visibleCommands = [["Approve", "POST", "/api/v1/app/jobs/42/approve"]]
  ...
  it("sends retry feedback from the Job header More menu", async () => {
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/jobs/42/run_again" && init?.method === "POST") {
        return Promise.resolve(
          new Response(JSON.stringify({ message: "Retry workflow enqueued." }), { status: 200, headers: { "Content-Type": "application/json" } })
        )
      }
  ...
  it("dispatches Job metadata controls through the app API", async () => {
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/jobs/42/stack_base" && init?.method === "PATCH") {
        return Promise.resolve(
          new Response(JSON.stringify({ message: "Stack base updated." }), { status: 200, headers: { "Content-Type": "application/json" } })
        )
      }
      ...
      if (path === "/api/v1/app/jobs/42/dependencies/9" && init?.method === "DELETE") {
        return Promise.resolve(
          new Response(JSON.stringify({ message: "Dependency removed." }), { status: 200, headers: { "Content-Type": "application/json" } })
        )
      }
      if (path === "/api/v1/app/jobs/42/dependencies/override" && init?.method === "POST") {
        return Promise.resolve(
          new Response(JSON.stringify({ message: "Dependency gate overridden." }), { status: 200, headers: { "Content-Type": "application/json" } })
        )
      }
      if (path === "/api/v1/app/jobs/42/timeline") {
        return Promise.resolve(new Response(JSON.stringify(jobTimelinePayload()), { status: 200, headers: { "Content-Type": "application/json" } }))
    ...
    fireEvent.click(screen.getByRole("button", { name: "JOB-41 - Build hill" }))
    await waitFor(() => {
      expect(fetchSpy).toHaveBeenCalledWith(
  ...
  it("dispatches Job workflow and run commands through the app API", async () => {
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/jobs/42/workflows/5/retry_step" && init?.method === "POST") {
        return Promise.resolve(
          new Response(JSON.stringify({ message: "Retrying implement for WF-5..." }), { status: 200, headers: { "Content-Type": "application/json" } })
        )
      }
      if (path === "/api/v1/app/jobs/42/workflows/5/push_commits" && init?.method === "POST") {
        return Promise.resolve(
          new Response(JSON.stringify({ message: "Pushing commits to GitHub..." }), { status: 200, headers: { "Content-Type": "application/json" } })
        )
      }
      ...
      if (path === "/api/v1/app/jobs/42/runs/9/diagnose" && init?.method === "POST") {
        return Promise.resolve(
          new Response(JSON.stringify({ message: "Diagnostic queued." }), { status: 200, headers: { "Content-Type": "application/json" } })
        )
      }
      if (path === "/api/v1/app/jobs/42/resume" && init?.method === "POST") {
        return Promise.resolve(
          new Response(JSON.stringify({ message: "Resume workflow enqueued." }), { status: 200, headers: { "Content-Type": "application/json" } })
        )
      }
              ...
              workflows: [
                {
                  ...
                  state: "running",
                  finished_at: null,
                  steps: [{ ...step, state: "queued", display_status: "queued", finished_at: null, runs: [queuedRun] }]
                }
              ]
            })
          ),
              ...
              workflows: [
                {
                  ...
                  steps: [
                    step({
                      ...
                      display_name: "Plan preflight graders",
                      display_status: "succeeded",
                      position: 0,
                      state: "succeeded"
                    }),
                    step({
                      ...
                      details: {
                        ...
                        duration_s: 1.1,
                        log_bytes: 512,
                        description: "Check for migration timestamp collisions.",
                        command: "bin/check-migration-collisions"
                      }
                    }),
                    step({
                      id: 73,
                    ...
                    step({
                      ...
                      display_name: "Preflight grader check",
                      display_status: "failed",
                      position: 3,
                      state: "failed"
                    }),
              ...
              workflows: [
                {
                  ...
                  steps: [
                    ...
                    loopStep({
                      ...
                      iteration: 2,
                      state: "succeeded",
                      runs: [{ ...run, id: 92, state: "succeeded" }]
                    }),
                    loopStep({
                      id: 66,
                      kind: "grader_fanout",
  ...
  it("adds and removes Job attachments through the app API", async () => {
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/jobs/42/attachments/8" && init?.method === "DELETE") {
        return Promise.resolve(
          new Response(JSON.stringify({ message: "Attachment removed." }), { status: 200, headers: { "Content-Type": "application/json" } })
        )
      }
    ...
    await waitFor(() => {
      expect(fetchSpy).toHaveBeenCalledWith("/api/v1/app/jobs/42/attachments", expect.objectContaining({ method: "POST", body: expect.any(FormData) }))
    })
  ...
  it("blocks sending a ready walkthrough alongside image attachments", async () => {
    ...
    expect(
      await screen.findByText("Send the walkthrough on its own — remove image or file attachments and send them in a separate message.")
    ).toBeInTheDocument()
  ...
  it("blocks dropping a second walkthrough while the first is uploading", async () => {
    ...
    expect(await screen.findByText("A walkthrough is already being processed — wait for it to finish, then add another.")).toBeInTheDocument()
  ...
  it("shows a removable attached repository chip in the empty chat landing compose area", async () => {
    ...
    await waitFor(() => {
      expect(fetchSpy).toHaveBeenCalledWith("/api/v1/app/chats/8/attachments/2", expect.objectContaining({ method: "DELETE" }))
    })
  ...
  it("intercepts registered system slash commands before posting chat messages", async () => {
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/chats/8/rename" && init?.method === "PATCH") {
        return Promise.resolve(
          new Response(JSON.stringify(chatPayload({ message: "Chat renamed." })), { status: 200, headers: { "Content-Type": "application/json" } })
        )
      }
  ...
  it("copies a shared chat link from the share slash command", async () => {
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/chats/8/share" && init?.method === "POST") {
        return Promise.resolve(
          new Response(JSON.stringify({ share_url: "http://syrus.test/chats/shared/token-123" }), {
            status: 200,
            headers: { "Content-Type": "application/json" }
          })
        )
  ...
  it("clears the composer after declaring a goal from a slash command", async () => {
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/chats/8/goal" && init?.method === "PATCH") {
        return Promise.resolve(
          new Response(JSON.stringify({ ...chatPayload(), active_goal: activeGoal }), { status: 200, headers: { "Content-Type": "application/json" } })
        )
      }
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/chats") {
        if (init?.method === "POST") {
          ...
          return Promise.resolve(
            new Response(JSON.stringify({ message: "Chat created.", redirect_to: "/chats/9", chat: newPayload.chat }), {
              status: 201,
              headers: { "Content-Type": "application/json" }
            })
          )
        ...
        return Promise.resolve(
          new Response(JSON.stringify({ groups: sidebarGroups(sidebarChats), repositories: [] }), {
            status: 200,
            headers: { "Content-Type": "application/json" }
          })
        )
  ...
  it("confirms clear before deleting chat history", async () => {
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/chats/8/messages" && init?.method === "DELETE") {
        return Promise.resolve(
          new Response(JSON.stringify(chatPayload({ message: "Chat history cleared.", messages: [] })), {
            status: 200,
            headers: { "Content-Type": "application/json" }
          })
        )
  ...
  it("handles panel and attachment system slash commands in the app", async () => {
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/chats/8/attachments" && init?.method === "POST") {
        return Promise.resolve(
          new Response(JSON.stringify(chatPayload({ message: "acme/tools attached." })), { status: 200, headers: { "Content-Type": "application/json" } })
        )
      }
    ...
    expect(fetchSpy).toHaveBeenCalledWith(
      "/api/v1/app/chats/8/attachments",
      expect.objectContaining({
        method: "POST",
  ...
  it("shows the pinned chat provider without offering chat provider switching", async () => {
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/chats/8/switch_provider" && init?.method === "POST") {
        return Promise.resolve(
          new Response(JSON.stringify({ message: "Switching to codex." }), { status: 200, headers: { "Content-Type": "application/json" } })
        )
      }
      return Promise.resolve(
        new Response(
  ...
  it("opens the bug report dialog from the /report slash command with transcript opt-in", async () => {
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input) => {
      ...
      if (path === "/api/v1/app/bug_reports") {
        return Promise.resolve(
          new Response(JSON.stringify({ message: "Bug report queued." }), { status: 201, headers: { "Content-Type": "application/json" } })
        )
      }
      return Promise.resolve(new Response(JSON.stringify(chatPayload()), { status: 200, headers: { "Content-Type": "application/json" } }))
    })
    ...
    try {
      ...
      await waitFor(() => {
        expect(fetchSpy).toHaveBeenCalledWith("/api/v1/app/bug_reports", expect.objectContaining({ method: "POST" }))
      })
      expect(await screen.findByRole("status")).toHaveTextContent("Bug report queued.")
      expect(screen.queryByRole("dialog", { name: "Report a bug" })).not.toBeInTheDocument()
      expect(fetchSpy).not.toHaveBeenCalledWith("/api/v1/app/chats/8/message", expect.objectContaining({ method: "POST" }))
  ...
  it("enqueues and edits follow-up messages while the agent is busy", async () => {
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      ...
      if (path === "/api/v1/app/chats/8/queued_messages" && init?.method === "POST") {
        return Promise.resolve(
          new Response(JSON.stringify(chatPayload({ agentBusy: true, queuedMessages })), { status: 200, headers: { "Content-Type": "application/json" } })
        )
      }
      ...
      if (path === "/api/v1/app/chats/8") {
        return Promise.resolve(
          new Response(JSON.stringify(chatPayload({ agentBusy: true, turnInFlight: false, queuedMessages })), {
            status: 200,
            headers: { "Content-Type": "application/json" }
          })
        )
  ...
  it("shows whiteboard snapshots in media and loads one onto the canvas", async () => {
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input, init) => {
          ...
          new Response(
            JSON.stringify({
              scene_json: {
                elements: [
                  { id: "box-1", type: "rectangle" },
                  { id: "fresh-snapshot-box", type: "rectangle", boundElements: [{ id: "fresh-snapshot-label", type: "text" }] }
                ],
                appState: {},
                files: { "file-snapshot": { id: "file-snapshot", mimeType: "image/png" } }
              },
      ...
      return Promise.resolve(
        new Response(JSON.stringify(chatPayload({ whiteboardSnapshotCount: 1 })), { status: 200, headers: { "Content-Type": "application/json" } })
      )
    })
  ...
  it("renders raw chat messages on the frontend", async () => {
    ...
    await waitFor(() => expect(screen.getByText("# RSpec config").style.color).toBe("var(--shiki-token-comment)"))
    expect(screen.getAllByText("do")[0].style.color).toBe("var(--shiki-token-keyword)")
    expect(screen.getByText(":rspec").style.color).toBe("var(--shiki-token-constant)")
    expect(screen.getByText("true").style.color).toBe("var(--shiki-token-constant)")
    ...
    await waitFor(() => expect(screen.getByText("Chat").style.color).toBe("var(--shiki-token-function)"))
    expect(screen.getAllByText("class")[0].style.color).toBe("var(--shiki-token-keyword)")
    expect(screen.getAllByText("end")[0].style.color).toBe("var(--shiki-token-keyword)")
  ...
  it("runs chat commands through the app API", async () => {
    ...
    await waitFor(() => {
      expect(fetchSpy).toHaveBeenCalledWith(`/api/v1/app/chats/8/attachments/2${search}`, expect.objectContaining({ method: "DELETE" }))
    })

    fireEvent.click(screen.getByRole("button", { name: "Confirm proposal and implement" }))
    await waitFor(() => {
      expect(fetchSpy).toHaveBeenCalledWith(`/api/v1/app/chats/8/proposals/5/confirm${search}`, expect.objectContaining({ method: "POST" }))
    })
    ...
    fireEvent.click(screen.getByRole("button", { name: "Confirm" }))
    await waitFor(() => {
      expect(fetchSpy).toHaveBeenCalledWith(`/api/v1/app/chats/8/pending_actions/7/confirm${search}`, expect.objectContaining({ method: "POST" }))
    })
  ...
  it("renders pending action cards inline in chat messages", async () => {
    ...
    await waitFor(() => {
      expect(fetchSpy).toHaveBeenCalledWith("/api/v1/app/chats/8/pending_actions/7/reject", expect.objectContaining({ method: "POST" }))
    })
  })
  ...
  it("renders proposal dependency pills with confirmed and pending badges", async () => {
    ...
    vi.spyOn(window, "fetch").mockResolvedValue(
      new Response(JSON.stringify(chatPayload({ messages: [...chatPayload().messages, proposalMessage] })), {
        status: 200,
        headers: { "Content-Type": "application/json" }
      })
    )
  ...
  it("renders queued pending actions as waiting without action buttons", async () => {
    vi.spyOn(window, "fetch").mockResolvedValue(
      new Response(
        JSON.stringify({
          ...chatPayload({
            pendingActions: [pendingAction({ id: 7, label: "Send feedback to JOB-44", state: "queued" })]
          })
        }),
        { status: 200, headers: { "Content-Type": "application/json" } }
      )
          ...
          ...chatPayload({
            pendingActions: [
              ...
              pendingAction({
                ...
                executionError: "ArgumentError: Job is not accessible."
              })
            ]
          })
        }),
  ...
  it("renders pending actions after their linked message in the stream", async () => {
    ...
    expect(
      screen.getByText((_content, element) => element?.tagName === "P" && element.textContent === "Please tighten this implementation.")
    ).toBeInTheDocument()
      ...
      new Response(
        JSON.stringify({
          ...chatPayload({
            ...
            pendingActions: [pendingAction({ id: 7, label: "Send feedback to JOB-44", chatMessageId: null })]
          })
        }),
        { status: 200, headers: { "Content-Type": "application/json" } }
      )
  ...
  it("loads older chat messages when scrolling near the top", async () => {
    ...
    const fetchSpy = vi.spyOn(window, "fetch").mockImplementation((input) => {
      ...
      return Promise.resolve(
        new Response(JSON.stringify(chatPayload({ hasMoreOlder: true })), { status: 200, headers: { "Content-Type": "application/json" } })
      )
    })
  ...
  it("renders active chat bookmarks from the v2 sidebar cache", async () => {
    ...
    initialPayload.bookmarks = [{ id: 7, label: "Earlier aqueduct note", chat_message_id: 4, anchor_message_id: 5 }]
    const recentChats = [
      sidebarChat({
        id: 8,
        title: "Aqueduct planning",
    ...
    try {
      ...
      expect(fetchSpy).not.toHaveBeenCalledWith("/api/v1/app/chats/8/messages?before=9", expect.anything())
    } finally {
      fetchSpy.mockRestore()
      script.remove()
    }
  ...
  it("renders inactive chat bookmarks from the v2 sidebar cache with chat deep links", async () => {
    ...
    inactivePayload.bookmarks = [{ id: 12, label: "Inactive aqueduct note", chat_message_id: 13, anchor_message_id: 13 }]
    const recentChats = [
      sidebarChat({
        id: 8,
        title: "Aqueduct planning",
  ...
  it("fetches inactive chat bookmarks from the v2 sidebar menu when uncached", async () => {
    ...
    try {
      ...
      expect(fetchSpy).toHaveBeenCalledWith("/api/v1/app/chats/11", expect.objectContaining({ credentials: "same-origin" }))
    } finally {
      fetchSpy.mockRestore()
      script.remove()
    }
  ...
  it("ignores a message_id query param and opens the chat at the latest messages without scrolling", async () => {
    ...
    try {
      ...
      await waitFor(() => expect(fetchSpy).toHaveBeenCalledWith("/api/v1/app/chats/8?message_id=9", expect.objectContaining({ credentials: "same-origin" })))
...
function bootstrapPayload(overrides: Record<string, unknown> & { setupStatus?: ReturnType<typeof bootstrapSetupStatusPayload> | null } = {}) {
  const { setupStatus: setupStatusOverride, setup_status: setupStatusPayloadOverride, ...payloadOverrides } = overrides
...
function setupStatusPayload(overrides: Record<string, unknown> = {}) {
  return {
    ...
    github_app: {
      registered: false,
      explanation:
        "Repositories use a GitHub App installation when one is active for that owner. Syrus falls back to your GitHub PAT for repositories without an active installation.",
...
function bootstrapSetupStatusPayload(overrides: { nextStep?: "configure_credentials" | "add_repository" | "start_first_job" | "watch_first_job" | null } = {}) {
  ...
  return {
    state:
      nextStep === "configure_credentials"
        ? "not_started"
        : nextStep === "add_repository"
          ? "credentials_only"
          : nextStep === "start_first_job"
            ? "ready_for_first_job"
            : nextStep === "watch_first_job"
              ? "first_job_started"
...
function notificationPreferencesPayload(
  overrides: Partial<
    Record<
      "job_failed" | "job_implemented" | "pr_comment_addressed" | "pr_merged" | "epic_completed" | "epic_review_ready" | "epic_failed" | "epic_feedback_queued",
      boolean
    >
  > & { message?: string } = {}
) {
...
function personalDocumentsPayload(
  overrides: {
...
function repositoryDocumentsPayload(
  overrides: {
    documents?: Array<Record<string, unknown>>
    message?: string
  } = {}
    ...
    controls: {
      ...
      filter_schema: [
        ...
        },
        {
          ...
          bucket: "fk",
          operators: ["is"],
          values: [{ value: 3, label: "acme/widgets" }]
        },
        {
          field: "kind",
          label: "Kind",
...
function expectDashboardFetch(fetchSpy: { mock: { calls: Array<[unknown, unknown?]> } }, expectedPath: string, expectedOptions?: unknown) {
...
function dashboardSectionPayload(payload: ReturnType<typeof dashboardPayload>, input: unknown) {
  ...
  if (section === "chrome") {
    const { total: _total, total_pages: _totalPages, items: _items, lanes: _lanes, kanban_limit: _kanbanLimit, ...chrome } = payload

    return chrome
  }

  if (section === "rows") {
    const { subject, view, page, per_page, total, total_pages, landing_queue, items, lanes, kanban_limit } = payload
...
function dashboardJobItem(overrides: Record<string, unknown> = {}) {
  return {
    ...
    paths: {
      ...
      app_pause_path: "/api/v1/app/jobs/42/pause",
      app_unpause_path: "/api/v1/app/jobs/42/unpause"
    },
    ...overrides
  }
}
    ...
    graph: {
      ...
      nodes: [
        { id: "epic_7", kind: "epic", label: "EPIC-7 Raise the forum", state: "ready", epic_id: 7, url: "/epics/EPIC-7", is_focal: true },
        {
          id: "epic_6",
          kind: "epic",
          label: "the Kubernetes write-tool feature Deliver marble",
          state: "done",
          epic_id: 6,
          url: "/epics/the Kubernetes write-tool feature",
          is_focal: false
        }
      ],
      edges: [{ from_id: "epic_7", to_id: "epic_6" }]
    },
    dependencies: overrides.dependencies || [{ epic_id: 6, title: "Deliver marble", state: "done", url: "/epics/6" }],
...
function jobDetailPayload(overrides: Record<string, unknown> = {}) {
  const payload = {
    ...
    dependents: [],
    unsatisfied_dependencies: [],
    dependency_target_options: [{ label: "JOB-41 - Build hill ", value: "issue:3:11" }],
...
function jobSourcePayload(overrides: { withFile?: boolean } = {}) {
  return {
    ...
    branch_commits: [{ sha: "deadbeef12345678", short_sha: "deadbee", message: "Repair aqueduct", date: "2026-05-30T11:00:00Z" }],
    tree_items: [
...
function jobSourceDiffPayload(overrides: { baseRef?: string; headRef?: string } = {}) {
  return {
    ...
    version: {
      id: 100,
      version_index: 1,
      base_sha: overrides.baseRef || "aabbccdd1234567",
      head_sha: overrides.headRef || "deadbeef12345678",
      label: "Version 1",
      reason: "initial"
    },
    versions: [
...
function chatPayload(
  ...
  return {
    ...
    bookmarks: [{ id: 1, label: "Aqueducts", chat_message_id: 9, anchor_message_id: 9 }],
    ...
    attachment_groups: {
      repositories: [{ id: 2, label: "acme/widgets", app_detach_path: "/api/v1/app/chats/8/attachments/2" }],
      epics: [],
      jobs: [],
      documents: []
    },
    documents_in_scope: [{ id: 5, title: "Launch notes", repository_slug: "acme/widgets" }],
    attachment_results: [],
    preview_panels: [],

...
export function AgentConversationTab({ jobId, prUrl }: { jobId: number; prUrl: string | null }) {
  ...
  return (
    <div className="mx-auto w-full min-w-0 max-w-3xl space-y-4 overflow-x-auto">
      <WorkflowSelector workflows={conversation.data.workflows} selectedWorkflowId={conversation.data.selected_workflow_id} onSelect={selectWorkflow} />
      {conversation.data.nodes.length === 0 ? (

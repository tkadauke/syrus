...
function MessageImageAttachments({ attachments, align = "start" }: { attachments?: ChatMessageItem["attachments"]; align?: "start" | "end" }) {
  ...
  return (
    <>
      ...
      {lightboxImage ? <ImageLightbox name={lightboxImage.name || "Image attachment"} src={attachmentDataUrl(lightboxImage)} onClose={() => setLightboxImage(null)} /> : null}
    </>
  )
}
...
export function ImageLightbox({ name, src, onClose }: { name: string; src: string; onClose: () => void }) {
  const { t } = useT("chat")
  useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-gray-950/35 p-4" onClick={onClose} role="presentation">
      <section aria-label={name || "Image attachment"} aria-modal="true" className="relative max-h-full max-w-full" onClick={(event) => event.stopPropagation()} role="dialog">
        <button
        ...
        <img alt={name || "Image attachment"} className="max-h-[calc(100dvh-2rem)] max-w-[calc(100vw-2rem)] rounded bg-white object-contain shadow-lg dark:bg-gray-900" src={src} />
      </section>
    </div>
  )
}

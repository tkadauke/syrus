      ...
      >
        ...
        <div className="relative">
          <textarea
            ...
            onPaste={handlePaste}
            placeholder={ghostSuggestion ? "" : payload.switching_provider ? t("switching_to_provider", { provider: providerLabel(payload.chat.chat_provider ?? "", providerLabels) }) : agentActive ? t("queue_followup") : payload.chat.repository ? t("ask_repository") : t("ask_anything")}
            ref={textareaRef}
...
function ChatModeSelector({ chatId, payload, queryKey }: { chatId: string; payload: ChatPayload; queryKey: ChatQueryKey }) {
  ...
  if (modeOptions.length <= 1 || isAgentActive(payload)) return null

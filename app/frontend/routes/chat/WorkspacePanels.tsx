...
function MediaGallery({ payload, queryKey, onNotice }: { payload: ChatPayload; queryKey: ChatQueryKey; onNotice: (message: string | null) => void }) {
  ...
  const chatImages = media.data?.chat_images || []
  ...
  if (chatImages.length === 0 && snapshotItems.length === 0 && walkthroughs.length === 0 && artifactItems.length === 0 && !snapshots.isPending && !snapshots.isError && !media.isPending && !media.isError) {
  ...
  return (
    <div className="space-y-5">
      ...
      {lightboxImage ? <ImageLightbox name={lightboxImage.title || lightboxImage.filename || "Image attachment"} src={lightboxImage.image_url || lightboxImage.file_path || ""} onClose={() => setLightboxImage(null)} /> : null}
    </div>
  )
}

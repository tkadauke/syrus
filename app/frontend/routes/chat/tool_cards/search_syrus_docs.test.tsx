...
describe("search_syrus_docs tool card", () => {
  ...
  it("summarizes the collapsed row with the query and hit count", () => {
    const parsedResult = { query: "landing queue", count: 2, results: [{ title: "Landing Queue" }, { title: "Auto Merge" }] }

    expect(searchSyrusDocsToolCard.collapsedSummary?.(context({ parsedResult }))).toBe('"landing queue" - 2 hits')
  })

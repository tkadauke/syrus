...
export function PendingJobTitle({ pending, title }: { pending: boolean; title: string }) {
  ...
  return (
    <span className="inline-flex min-w-0 items-center gap-1 italic text-gray-500 dark:text-gray-400">
      <span
        aria-hidden="true"
        className="inline-block h-3 w-3 shrink-0 animate-spin rounded-full border-2 border-gray-300 border-t-gray-500 dark:border-gray-700 dark:border-t-gray-300"
      />
...
export function WorkflowTriggerPill({ ariaPrefix, triggerKind }: { ariaPrefix: string; triggerKind: string }) {
  ...
  return (
    <span
      ...
      className={`inline-flex items-center gap-1.5 whitespace-nowrap rounded-full px-2 py-0.5 text-xs font-medium capitalize ring-1 ${className}`}
...
export function ExternalMetadataLink({
  children,
  className = "text-gray-500 hover:text-brand hover:underline dark:text-gray-400",
  href
}: {
  children: ReactNode
  className?: string
...
export function RepositorySlugLink({
  className = "font-mono text-xs text-gray-500 hover:text-brand hover:underline dark:text-gray-400",
  prefix,
  repository
}: {
  className?: string
  ...
  return (
    <Link className={className} to={withRoutePrefix(repository.repository_path, prefix)}>
      {repository.slug}
    </Link>
  )
}
...
export function EpicCommitsBehindBadge({ commits }: { commits: number | null }) {
  ...
  return <span className={className}>{t("epic_commits_behind", { count: commits })}</span>
}
...
export function NeutralStatePill({ state }: { state: string }) {
  ...
  return (
    <span className="inline-flex whitespace-nowrap rounded-full bg-gray-100 px-2 py-0.5 text-xs font-medium capitalize text-gray-700 ring-1 ring-gray-200 dark:bg-gray-800 dark:text-gray-200 dark:ring-gray-700">
      {label}
    </span>
  )
}
...
export function EpicProgressBar({ epic, fullWidth = false }: { epic: DashboardEpicItem; fullWidth?: boolean }) {
  ...
  return (
    <div
      ...
      className={
        fullWidth
          ? "flex h-1.5 w-full overflow-hidden rounded-b bg-gray-200 dark:bg-gray-700"
          : "flex h-1.5 w-20 overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700"
      }
      role="progressbar"
      title={titleText}
    >
...
export function SortableColumnHeader({ subject, column, sortState }: { subject: DashboardSubject; column: string; sortState: DashboardSortState }) {
  ...
  return (
    ...
    >
      <span>{label}</span>
      {active ? (
        <span aria-hidden="true" className="text-2xs leading-none text-gray-700 dark:text-gray-300">

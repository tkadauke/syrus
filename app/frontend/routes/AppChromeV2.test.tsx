...
describe("AppChromeV2", () => {
  ...
  it("uses semantic brand tokens for app chrome active link helpers", () => {
    for (const className of [recentChatLinkClass(true), adminSubnavLinkClass(true), adminNavLinkClass(true)]) {
      expect(className).toContain("text-brand")
      expect(className).toContain("bg-brand/10")
      expect(className).not.toMatch(/\b(?:text|bg|border)-blue-/)
    }
...
describe("AppChromeV2 mobile header pinning", () => {
  ...
  it("keeps the menu icon next to the wordmark when the mobile drawer opens, so it doesn't shift left", () => {
    // Previously the drawer header rendered only the wordmark with no
    // leading icon, so opening the sidebar looked like the hamburger
    // vanished and the logo jumped to the left.
    const restoreMatchMedia = mockNarrowViewport()

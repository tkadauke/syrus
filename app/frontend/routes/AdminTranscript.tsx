      ...
      <header className="shrink-0 flex flex-col gap-3 border-b border-gray-200 dark:border-gray-700 pb-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="text-xs uppercase text-gray-500 dark:text-gray-400">
            <Link className="underline hover:no-underline" to={withRoutePrefix(`/jobs/${payload.job_id}`, prefix)}>
              back to {payload.job_slug || `JOB-${payload.job_id}`}
            </Link>
          </div>
          <PageHeading className="mt-1">Run #{payload.run_id} · transcript</PageHeading>
...
function TranscriptEventStream({ payload }: { payload: TranscriptPayload }) {
  ...
  return (
    <div className="relative min-h-0 flex-1">
      ...
      {hasNewMessages ? <NewMessagesButton onClick={scrollToBottom} /> : null}
    </div>
  )
}
...
function SummaryGrid({ payload }: { payload: TranscriptPayload }) {
  ...
  return (
    <section
      aria-label={t("transcript.aria_summary")}
      className="grid gap-4 rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4 text-sm sm:grid-cols-2 lg:grid-cols-4"
    >
        ...
        {Object.keys(summary.tool_call_counts).length > 0 ? (
          <div className="flex flex-wrap gap-2">
            {Object.entries(summary.tool_call_counts).map(([name, count]) => (
              <span className="rounded bg-gray-100 dark:bg-gray-800 px-2 py-0.5 font-mono text-xs text-gray-700 dark:text-gray-200" key={name}>
                {name} x{count}
              </span>
            ))}
          </div>
  ...
  return (
    <nav aria-label={t("transcript.aria_pagination")} className="flex items-center justify-between text-sm text-gray-600 dark:text-gray-300">
      ...
      <div className="flex gap-2">
        {page > 1 ? (
          <Link
            className="rounded border border-gray-300 dark:border-gray-600 px-3 py-1 hover:bg-gray-50 dark:hover:bg-gray-800"
            to={`${basePath}?page=${page - 1}&per=${per}`}
          >
            {t("transcript.previous")}
          </Link>
        ) : (
      ...
      if (item.kind === "user_prompt") {
        return (
          <div className="flex justify-end">
            <pre className="max-w-[85%] whitespace-pre-wrap break-words rounded bg-brand px-4 py-2 text-sm leading-normal text-on-brand">
...
function DetailsCard({ badge, title, data, tone = "gray" }: { badge: string; title: string; data: unknown; tone?: "gray" | "red" | "emerald" }) {
  ...
  return (
    <details className="rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-xs">
      ...
      <pre className="overflow-x-auto whitespace-pre-wrap break-words px-3 pb-3 font-mono text-gray-600 dark:text-gray-300">
        <AnsiText text={pretty(data)} />
      </pre>
    </details>
  )
}
...
function ResultCard({ data }: { data: Record<string, unknown> }) {
  ...
  return (
    <div
      className={`rounded border px-3 py-2 text-xs ${isError ? "border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950/40" : "border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950/40"}`}
    >

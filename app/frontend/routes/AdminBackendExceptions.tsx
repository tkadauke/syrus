...
export function AdminBackendExceptions() {
  ...
  return (
    <AdminEventPageShell
      actions={
        <Button className="w-fit" disabled={exceptions.isFetching} onClick={() => void exceptions.refetch()} variant="secondary">
    ...
    >
      ...
      {exceptions.isError ? (
        <AdminEventPanelMessage tone="error">{errorMessage(exceptions.error, t("backend_exceptions.error_load"))}</AdminEventPanelMessage>
      ) : null}
...
function BackendExceptionFilters({
  onNavigate,
  payload,
  search
}: {
  onNavigate: (params: URLSearchParams) => void
  payload?: BackendExceptionEventsPayload
  search: string
}) {
  const { t } = useT("admin")
  return (
    <AdminEventFilterBar
      clearLabel={t("backend_exceptions.clear")}
      ...
      fields={[
        ...
        {
          name: "source",
          label: t("backend_exceptions.source"),
          options: [{ value: "", label: t("backend_exceptions.all_sources") }, ...(payload?.sources || []).map((source) => ({ value: source, label: source }))]
        },
        ...
        {
          name: "revision_scope",
          label: t("backend_exceptions.revision_scope"),
          defaultValue: "current",
          options: revisionScopes.map((scope) => ({ value: scope, label: t(`backend_exceptions.revision_${scope}`) }))
        },
        {
          name: "per_page",
          label: t("backend_exceptions.per_page"),
          defaultValue: "50",
          options: [25, 50, 100].map((value) => ({ value: String(value), label: String(value) }))
        }
      ]}
      search={search}
      searchLabel={t("backend_exceptions.search")}
      onNavigate={onNavigate}
    />
  )
}

function BackendExceptionsView({
  onNavigate,
  payload,
  search
}: {
  ...
  return (
    <section className="overflow-hidden rounded border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900">
      <div className="flex flex-col gap-2 border-b border-gray-200 px-4 py-3 text-sm text-gray-600 dark:border-gray-700 dark:text-gray-300 sm:flex-row sm:items-center sm:justify-between">
        ...
        <span>
          {t("backend_exceptions.revision_hint", {
            revision: payload.revision_scope === "all" ? t("backend_exceptions.all_revisions") : shortRevision(payload.current_revision)
          })}
        </span>
      </div>
      {payload.events.length > 0 ? (
        <BackendExceptionsTable revisionScope={payload.revision_scope} rows={payload.events} search={search} onNavigate={onNavigate} />
      ) : (
...
function BackendExceptionsTable({
  onNavigate,
  revisionScope,
  rows,
  search
  ...
  const columns: Array<AdminEventLogTableColumn<BackendExceptionEventRow>> = [
      ...
      render: (row) => {
        ...
        return (
          <>
            ...
            {row.job_id ? (
              <Link className="mt-1 inline-block underline" to={`/jobs/${row.job_id}`}>
                {row.job_slug || `JOB-${row.job_id}`}
              </Link>
            ) : null}
          </>
        )
      ...
      render: (row) => (
        <>
          ...
          <div className="mt-1 break-all font-mono text-xs text-gray-500 dark:text-gray-400">
            {row.exception_class} · {row.fingerprint}
          </div>
        </>
      )
    },
    {
      ...
      render: (row) => (
        <>
          <div>
            {row.role || "-"} {row.hostname ? `@ ${row.hostname}` : ""}
          </div>
    ...
    },
    {
      ...
      render: (row, state) => (
        <AdminEventActions
          actions={row.actions}
          eventId={row.id}
          eventType="backend_exception"
          showDetailsLabel={state.expanded ? t("backend_exceptions.hide_details") : t("backend_exceptions.show_details")}
    ...
    <AdminEventLogTable
      ...
      renderExpanded={(row) => (
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <JsonBlock
            title={t("backend_exceptions.request_context")}
            value={{ controller: row.controller, action: row.action, method: row.method, path: row.path, status: row.status, request_id: row.request_id }}
          />
          <JsonBlock
            title={t("backend_exceptions.job_context")}
            value={{
          ...
          />
          <JsonBlock
            title={t("backend_exceptions.environment")}
            value={{ role: row.role, hostname: row.hostname, pid: row.pid, metadata: row.metadata || {} }}
          />
        </div>
      )}
    />

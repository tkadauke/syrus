...
describe("attaching a media gallery image to the composer", () => {
  ...
  it("attaches a media library image on mobile, where the Media tab and the composer are mutually exclusive views", async () => {
    mockMobileViewport()
    vi.spyOn(window, "fetch").mockImplementation((input, init) => {
      const path = String(input)
      if (path === "/api/v1/app/chats/8/mark_read" && init?.method === "PATCH") {
    ...
    // The mobile single-pane layout renders the Media tab and the composer
    // (under the Chat tab) as mutually exclusive alternatives -- there is no
    // mounted Compose to receive an event while the Media tab is active.
    fireEvent.click(await screen.findByRole("button", { name: "Media" }))
    await screen.findByText("runtime-frame.png")
    expect(screen.queryByRole("textbox", { name: "Ask about this repository..." })).not.toBeInTheDocument()

    fireEvent.click(screen.getByRole("button", { name: "Attach runtime-frame.png to message" }))
    expect(await screen.findByText("Added to message composer")).toBeInTheDocument()
...
describe("bang command mode (the chat shell-command cancellation feature)", () => {
  ...
  it("flips into warning-styled command mode when ! is typed as the first character, in a Local Mode chat", async () => {
    mockChatRouteFetch(chatPayload({ chat: { mode: "local" } }))
    renderRoute()
  ...
  it("does not enter command mode outside Coding Mode and Local Mode chats", async () => {
...
describe("attached coding Job strip", () => {
  ...
  it("updates when the attached Job payload changes live", async () => {
    const queryClient = new QueryClient({ defaultOptions: { queries: { retry: false } } })
    mockChatRouteFetch(chatPayload({ chat: { mode: "coding" } }, {
      coding_mode_enabled: true,
    ...
    await waitFor(() => {
      expect(within(screen.getByTestId("attached-coding-job-strip")).getByText("implemented")).toBeInTheDocument()
    })
    expect(within(screen.getByTestId("attached-coding-job-strip")).getByText("uncommitted changes")).toBeInTheDocument()
    expect(within(screen.getByTestId("attached-coding-job-strip")).getByRole("button", { name: "Submit" })).toBeDisabled()
  })

...
export function linkifySlugs(text: string, options: LinkifySlugOptions = {}): ReactNode[] {
  return text.split(slugPattern).map((part, index) => {
      ...
      return (
        <SlugHoverCard key={index} kind="job" id={Number(job[1])}>
          {options.jobStyle === "copyable" ? (
            ...
            <Link className="text-brand hover:underline dark:text-brand-emphasis" to={`/jobs/${job[1]}`}>
              {part}
            </Link>
          )}
        </SlugHoverCard>
    ...
    if (epic) {
      return (
        <SlugHoverCard key={index} kind="epic" id={Number(epic[1])}>
          ...
          <Link className="text-brand hover:underline dark:text-brand-emphasis" to={`/epics/${epic[1]}`}>
            {part}
          </Link>
        </SlugHoverCard>
      )
    ...
    if (doc) {
      return (
        <Link className="text-brand hover:underline dark:text-brand-emphasis" key={index} to={`/design_docs/${doc[1]}`}>
          {part}
        </Link>
      )
    }

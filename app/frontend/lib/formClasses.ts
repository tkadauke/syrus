...
// Uses semantic border/brand focus tokens so authenticated app surfaces follow
// the signed-in user's selected color theme. Consolidated from several
...
export function inputClass({ fullWidth = true }: { fullWidth?: boolean } = {}) {
  return `block ${fullWidth ? "w-full" : "w-auto"} rounded border border-border bg-white px-3 py-2 text-sm text-gray-900 shadow-sm focus:border-brand focus:outline-none focus:ring-1 focus:ring-brand dark:bg-gray-950 dark:text-gray-100 dark:placeholder:text-gray-500`
}

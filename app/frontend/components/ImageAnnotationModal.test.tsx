...
describe("ImageAnnotationModal", () => {
  ...
  it("uses measured canvas text bounds for selecting text annotations", async () => {
    ...
    vi.mocked(contexts[1].measureText).mockReturnValue({ width: 80 } as TextMetrics)
    fireEvent.click(screen.getByRole("button", { name: "Select" }))
    fireEvent.pointerDown(screen.getByLabelText("Annotation canvas"), { clientX: 85, clientY: 25, pointerId: 1 })
    fireEvent.pointerUp(screen.getByLabelText("Annotation canvas"), { clientX: 85, clientY: 25, pointerId: 1 })

    await waitFor(() => {
      expect(screen.getByRole("radiogroup", { name: "Text size" })).toBeVisible()
    })
  })
  ...
  it("Escape closes annotation shortcut help without also closing the annotation editor", async () => {
    const onClose = vi.fn()
    renderModal({ onClose })
    await waitForLoaded()

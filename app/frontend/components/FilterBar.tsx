    ...
    return (
      <span className={negated ? "inline-flex flex-wrap items-center gap-1 rounded border border-rose-300 bg-rose-50 px-1.5 py-0.5 text-sm dark:border-rose-800 dark:bg-rose-950" : "inline-flex flex-wrap items-center gap-1 rounded border border-indigo-300 bg-indigo-50 px-1.5 py-0.5 text-sm dark:border-indigo-800 dark:bg-indigo-950"}>
        ...
        {inner.or.map((child, childIndex) => (
          <span className="inline-flex flex-nowrap items-center gap-1 whitespace-nowrap" key={childIndex}>
            ...
            {isFilterChip(child) ? (
              <span className="inline-flex flex-nowrap items-center gap-1 whitespace-nowrap rounded border border-gray-300 bg-gray-50 px-2 py-1 dark:border-gray-700 dark:bg-gray-800">
...
function FilterChipButton({ chip, controls, negated = false, onClick }: { chip: FilterChip; controls: FilterSchemaField[]; negated?: boolean; onClick: () => void }) {
  ...
  return (
    <button aria-label={label} className="inline-flex flex-nowrap items-baseline gap-1 whitespace-nowrap text-left" onClick={onClick} type="button">
      ...
      {isPredicateOp(chip.op) ? null : dateRange && dateRange.from && dateRange.to ? (
        <span className="inline-flex flex-nowrap items-baseline gap-1 whitespace-nowrap">
        ...
        <>
          <span className="whitespace-nowrap text-xs text-gray-500 dark:text-gray-400">{opLabel}</span>
          <span className="whitespace-nowrap font-mono text-gray-900 dark:text-white">{formattedValue}</span>
        </>
      )}
    </button>
  )

...
function FilterChipButton({ chip, controls, negated = false, onClick }: { chip: FilterChip; controls: FilterSchemaField[]; negated?: boolean; onClick: () => void }) {
  ...
  return (
    <button aria-label={label} className="inline-flex flex-nowrap items-baseline gap-1 text-left" onClick={onClick} type="button">
      <span className="whitespace-nowrap font-medium text-gray-700 dark:text-gray-200">{fieldLabel}</span>

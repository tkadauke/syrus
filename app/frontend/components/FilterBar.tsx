  ...
  return (
    <div className={className}>
      <div className="relative flex flex-wrap items-center gap-2">
        ...
        <button
          className="inline-flex items-center gap-1 rounded border border-dashed border-gray-300 dark:border-gray-600 px-2 py-1.5 text-sm text-gray-600 dark:text-gray-300 hover:border-gray-400 hover:text-gray-900 dark:text-gray-100"
          onClick={() => openAddMenu()}
          type="button"
        >
          + Add filter
        ...
        {hasFilters ? (
          <Link className="text-sm text-gray-500 dark:text-gray-400 underline hover:text-gray-700 dark:text-gray-200" to={clearFiltersLink(pathname, search, legacyFilterKeys, buildLink)}>
            Clear filters
          </Link>
        ) : null}
        {addMenuOpen ? (
          <div className="absolute left-0 top-full z-20 mt-1 w-72 rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-lg" ref={addMenuRef}>
            <input
              autoFocus
              className="block w-full rounded-t border-b border-gray-200 dark:border-gray-700 px-3 py-2 text-sm focus:outline-none"
...
function FilterNodeChip({
  ...
  if (isFilterChip(inner)) {
    return (
      <span className={filterChipClass(negated)}>
        ...
        <button aria-label={`Remove ${filterChipLabel(inner, controls)} filter`} className="inline-flex h-5 w-5 items-center justify-center rounded text-gray-400 dark:text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-700 dark:text-gray-200" onClick={() => onRemove([index])} type="button">
  ...
  if (inner && "or" in inner && Array.isArray(inner.or)) {
    return (
      <span className={negated ? "inline-flex flex-wrap items-center gap-1 rounded border border-rose-300 dark:border-rose-700 bg-rose-50 dark:bg-rose-950/40 px-1.5 py-0.5 text-sm" : "inline-flex flex-wrap items-center gap-1 rounded border border-indigo-300 dark:border-indigo-700 bg-indigo-50 dark:bg-indigo-950/40 px-1.5 py-0.5 text-sm"}>
        <button aria-label={negated ? "Remove NOT" : "Wrap in NOT"} className={filterNotClass(negated)} onClick={() => onToggleNegation(index)} title={negated ? "Remove NOT" : "Wrap in NOT"} type="button">¬</button>
        <span className={negated ? "text-xs font-semibold text-rose-700" : "text-xs font-semibold text-indigo-700 dark:text-indigo-300"}>(</span>
        {inner.or.map((child, childIndex) => (
          <span className="inline-flex items-center gap-1" key={childIndex}>
            ...
            {isFilterChip(child) ? (
              <span className="inline-flex items-center gap-1 rounded border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 px-2 py-1">
                <FilterChipButton chip={child} controls={controls} onClick={() => onEdit([index, childIndex])} />
                <button aria-label={`Remove ${filterChipLabel(child, controls)} filter`} className="inline-flex h-5 w-5 items-center justify-center rounded text-gray-400 dark:text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-700 dark:text-gray-200" onClick={() => onRemove([index, childIndex])} type="button">
                  <CloseIcon className="h-3.5 w-3.5" />
                </button>
              </span>
            ) : (
              <span className="rounded border border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-950/40 px-2 py-1 text-amber-800 dark:text-amber-200">complex filter</span>
            )}
          </span>
        ))}
        <span className={negated ? "text-xs font-semibold text-rose-700" : "text-xs font-semibold text-indigo-700 dark:text-indigo-300"}>)</span>
  ...
  return <span className="rounded border border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-950/40 px-2 py-1 text-sm text-amber-800 dark:text-amber-200">complex filter</span>
}
...
function FilterChipButton({ chip, controls, negated = false, onClick }: { chip: FilterChip; controls: FilterSchemaField[]; negated?: boolean; onClick: () => void }) {
  ...
  return (
    <button aria-label={label} className="inline-flex items-baseline gap-1 text-left" onClick={onClick} type="button">
      ...
      {isPredicateOp(chip.op) ? null : <span className="font-mono text-gray-900 dark:text-gray-100">{formattedValue}</span>}
    </button>
  )
}
...
function FilterChipEditor({ chip, editorRef, meta, onAddAlternative, onChange }: { chip: FilterChip; editorRef: RefObject<HTMLDivElement>; meta: FilterSchemaField; onAddAlternative: () => void; onChange: (chip: FilterChip) => void }) {
  ...
  return (
    <div aria-label={`${meta.label} filter settings`} className="absolute left-0 top-full z-30 mt-2 w-[min(36rem,calc(100vw-3rem))] space-y-3 rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-3 shadow-lg" ref={editorRef} role="dialog">
      <div className="text-xs font-semibold uppercase text-gray-500 dark:text-gray-400">{meta.label}</div>
      <div className="space-y-3">
        <label className="block text-xs font-medium uppercase text-gray-500 dark:text-gray-400" htmlFor={`filter-op-${meta.field}`}>
          Operator
          <select className="mt-1 block rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 px-2 py-1.5 text-sm normal-case text-gray-700 dark:text-gray-200" id={`filter-op-${meta.field}`} onChange={(event) => updateOp(event.target.value)} value={chip.op}>
            {meta.operators.map((op) => <option key={op} value={op}>{humanizeOp(op)}</option>)}
          </select>
        </label>
        <FilterValueEditor chip={chip} meta={meta} onChange={onChange} />
      </div>
      <button className="rounded border border-dashed border-indigo-300 dark:border-indigo-700 px-2 py-1 text-sm font-medium text-indigo-700 dark:text-indigo-300 hover:bg-indigo-50 dark:hover:bg-indigo-950/40" onClick={onAddAlternative} type="button">
        + OR alternative
      </button>
    </div>
  )
...
function FilterValueEditor({ chip, meta, onChange }: { chip: FilterChip; meta: FilterSchemaField; onChange: (chip: FilterChip) => void }) {
  ...
  if (options.length > 0 && !meta.typeahead) {
    ...
    return (
      <label className="block text-xs font-medium uppercase text-gray-500 dark:text-gray-400" htmlFor={`filter-value-${meta.field}`}>
        Value
        <select
          className="mt-1 block min-w-44 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 px-2 py-1.5 text-sm normal-case text-gray-700 dark:text-gray-200"
  ...
  return (
    <label className="block text-xs font-medium uppercase text-gray-500 dark:text-gray-400" htmlFor={`filter-value-${meta.field}`}>
      Value
      <input
        className="mt-1 block w-56 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 px-2 py-1.5 text-sm normal-case text-gray-700 dark:text-gray-200"
        id={`filter-value-${meta.field}`}
...
function TypeaheadFilterValueEditor({ chip, meta, multi, onChange }: { chip: FilterChip; meta: FilterSchemaField; multi: boolean; onChange: (chip: FilterChip) => void }) {
  ...
  return (
    <div className="block text-xs font-medium uppercase text-gray-500 dark:text-gray-400">
      ...
      <div className="relative mt-1 normal-case text-gray-700 dark:text-gray-200">
        <div className="flex max-h-[10rem] min-h-11 w-full flex-wrap items-center gap-1.5 overflow-y-auto rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 px-2 py-2 text-sm" onClick={() => inputRef.current?.focus()}>
          {selected.length > 0 ? (
            selected.map((value) => {
              const option = selectedOptions.find((candidate) => String(candidate.value) === value) || { value, label: value }
              return (
                <span className="inline-flex items-center gap-1 rounded bg-indigo-100 dark:bg-indigo-950/60 px-2 py-0.5 text-indigo-800" key={value}>
                  {option.label}
        ...
        {query.trim() ? (
          <div className="absolute left-0 right-0 top-full z-10 mt-1 max-h-56 overflow-y-auto rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 py-1 shadow-lg">
            {options.length > 0 ? (
              options.map((option) => (
                <button
...
function MultiFilterValueEditor({ chip, meta, onChange, options }: { chip: FilterChip; meta: FilterSchemaField; onChange: (chip: FilterChip) => void; options: FilterOption[] }) {
  ...
  return (
    <label className="block text-xs font-medium uppercase text-gray-500 dark:text-gray-400" htmlFor={`filter-value-${meta.field}-search`}>
      <span className="sr-only">Value</span>
      <div className="mt-1 w-72 overflow-hidden rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 normal-case text-gray-700 dark:text-gray-200">
        <div className="flex min-h-11 flex-wrap items-center gap-1.5 px-2 py-2 text-sm">
          {selectedOptions.length > 0 ? (
            selectedOptions.map((option) => (
              <span className="inline-flex items-center gap-1 rounded bg-indigo-100 dark:bg-indigo-950/60 px-2 py-0.5 text-indigo-800" key={String(option.value)}>
                {option.label}
        ...
        <input
          className="block w-full border-t border-gray-200 dark:border-gray-700 px-2 py-2 text-sm focus:outline-none"
          id={`filter-value-${meta.field}-search`}
          onChange={(event) => setQuery(event.target.value)}
          placeholder="Search..."
          type="search"
          value={query}
        />
        <div className="max-h-56 overflow-y-auto border-t border-gray-200 dark:border-gray-700 py-1">
...
function DateFilterValueEditor({ chip, onChange }: { chip: FilterChip; onChange: (chip: FilterChip) => void }) {
  if (chip.op === "within_last" || chip.op === "more_than_ago") {
    ...
    return (
      <div className="flex items-end gap-2">
        <label className="block text-xs font-medium uppercase text-gray-500 dark:text-gray-400" htmlFor="filter-date-amount">
          Amount
          <input className="mt-1 block w-20 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 px-2 py-1.5 text-sm normal-case text-gray-700 dark:text-gray-200" id="filter-date-amount" min="0" onChange={(event) => onChange({ ...chip, value: { ...value, n: Number(event.target.value || 0) } })} type="number" value={Number(value.n || 0)} />
        </label>
        <label className="block text-xs font-medium uppercase text-gray-500 dark:text-gray-400" htmlFor="filter-date-unit">
          Unit
          <select className="mt-1 block rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 px-2 py-1.5 text-sm normal-case text-gray-700 dark:text-gray-200" id="filter-date-unit" onChange={(event) => onChange({ ...chip, value: { ...value, unit: event.target.value } })} value={String(value.unit || "days")}>
  ...
  if (chip.op === "between") {
    ...
    return (
      <div className="flex items-end gap-2">
        <label className="block text-xs font-medium uppercase text-gray-500 dark:text-gray-400" htmlFor="filter-date-from">
          From
          <input className="mt-1 block rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 px-2 py-1.5 text-sm normal-case text-gray-700 dark:text-gray-200" id="filter-date-from" onChange={(event) => onChange({ ...chip, value: [event.target.value, value[1] || ""] })} type="date" value={String(value[0] || "").slice(0, 10)} />
        </label>
        <label className="block text-xs font-medium uppercase text-gray-500 dark:text-gray-400" htmlFor="filter-date-to">
          To
          <input className="mt-1 block rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 px-2 py-1.5 text-sm normal-case text-gray-700 dark:text-gray-200" id="filter-date-to" onChange={(event) => onChange({ ...chip, value: [value[0] || "", event.target.value] })} type="date" value={String(value[1] || "").slice(0, 10)} />
        </label>
      </div>
    )
  }
  ...
  return (
    <label className="block text-xs font-medium uppercase text-gray-500 dark:text-gray-400" htmlFor="filter-date-value">
      Value
      <input className="mt-1 block rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 px-2 py-1.5 text-sm normal-case text-gray-700 dark:text-gray-200" id="filter-date-value" onChange={(event) => onChange({ ...chip, value: event.target.value })} type="date" value={String(chip.value || "").slice(0, 10)} />
    </label>
  )
}
...
function NumberFilterValueEditor({ chip, onChange }: { chip: FilterChip; onChange: (chip: FilterChip) => void }) {
  if (chip.op === "between") {
    ...
    return (
      <div className="flex items-end gap-2">
        <label className="block text-xs font-medium uppercase text-gray-500 dark:text-gray-400" htmlFor="filter-number-min">
          Min
          <input className="mt-1 block w-28 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 px-2 py-1.5 text-sm normal-case text-gray-700 dark:text-gray-200" id="filter-number-min" onChange={(event) => onChange({ ...chip, value: [event.target.value === "" ? null : Number(event.target.value), value[1] ?? null] })} type="number" value={typeof value[0] === "number" ? value[0] : ""} />
        </label>
        <label className="block text-xs font-medium uppercase text-gray-500 dark:text-gray-400" htmlFor="filter-number-max">
          Max
          <input className="mt-1 block w-28 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 px-2 py-1.5 text-sm normal-case text-gray-700 dark:text-gray-200" id="filter-number-max" onChange={(event) => onChange({ ...chip, value: [value[0] ?? null, event.target.value === "" ? null : Number(event.target.value)] })} type="number" value={typeof value[1] === "number" ? value[1] : ""} />
        </label>
      </div>
    )
  }
  ...
  return (
    <label className="block text-xs font-medium uppercase text-gray-500 dark:text-gray-400" htmlFor="filter-number-value">
      Value
      <input className="mt-1 block w-32 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 px-2 py-1.5 text-sm normal-case text-gray-700 dark:text-gray-200" id="filter-number-value" onChange={(event) => onChange({ ...chip, value: event.target.value === "" ? null : Number(event.target.value) })} type="number" value={typeof chip.value === "number" ? chip.value : ""} />
    </label>
  )
}
...
function filterChipClass(negated: boolean) {
  return negated
    ? "inline-flex flex-wrap items-center gap-1 rounded border border-rose-300 dark:border-rose-700 bg-rose-50 dark:bg-rose-950/40 px-1.5 py-1 text-sm"
    : "inline-flex flex-wrap items-center gap-1 rounded border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 px-2 py-1 text-sm"
}
...
function filterNotClass(negated: boolean) {
  return negated
    ...
    : "inline-flex h-5 w-5 items-center justify-center rounded border border-gray-300 dark:border-gray-600 text-sm font-bold leading-none text-gray-400 dark:text-gray-500 hover:border-rose-300 dark:hover:border-rose-700 hover:bg-rose-50 dark:hover:bg-rose-950/40 hover:text-rose-700"
}

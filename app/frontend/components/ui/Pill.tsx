...
export function pillClasses(tone: SemanticTone = "neutral", className = "", wrap = false) {
  return classes("inline-flex items-center gap-1.5 rounded-[var(--radius-pill)] px-2 py-0.5 text-xs font-medium ring-1", wrap ? false : "whitespace-nowrap", TONE_CHIP_CLASSES[tone], className)
}
...
export function Pill({ active = false, children, className = "", tone = "neutral", wrap = false, ...props }: PillProps) {
  return (

...
export function pillClasses(tone: SemanticTone = "neutral", className = "", wrap = false) {
  return classes(
    "inline-flex items-center gap-1.5 rounded-[var(--radius-pill)] px-2 py-0.5 text-xs font-medium ring-1",
    wrap ? "max-w-full flex-wrap whitespace-normal break-words text-left" : "whitespace-nowrap",
...
export function Pill({ active = false, children, className = "", contentClassName, tone = "neutral", wrap = false, ...props }: PillProps) {
  return (
    <span className={pillClasses(tone, className, wrap)} {...props}>

      ...
      <div className="space-y-5 p-5 sm:p-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">{t("configure_agent.description")}</p>
          </div>
          <button
        ...
        <div className="flex border-b border-gray-200 dark:border-gray-700" role="tablist">
          <button aria-selected={tab === "claude"} className={tabClass(tab === "claude")} onClick={() => setTab("claude")} role="tab" type="button">
          ...
          >
            ...
            <span className="ml-2 rounded bg-gray-100 px-1.5 py-0.5 text-2xs font-semibold uppercase text-gray-400 dark:bg-gray-800 dark:text-gray-500">
              {t("configure_agent.soon")}
            </span>
          </button>
          <button
        ...
        {tab === "gemini" ? (
          <div className="space-y-4">
            <p className="text-sm text-gray-600 dark:text-gray-400">{t("configure_agent.gemini_tab_intro")}</p>
            {geminiConfigured ? (
              <>
                <StatusBox tone="ok">{t("configure_agent.gemini_tab_configured")}</StatusBox>
                <div className="flex justify-end">
                  <Button onClick={onClose}>{t("configure_agent.done")}</Button>
                </div>
              </>
            ) : (
              <div className="flex items-center justify-end gap-2">
                ...
                <Button onClick={() => setGeminiSheetOpen(true)}>{t("configure_agent.gemini_tab_add_key")}</Button>
              </div>
            )}
          </div>
        ) : null}
...
function tabClass(active: boolean) {
  ...
  return active ? `${base} border-brand text-brand dark:text-brand-emphasis` : `${base} border-transparent text-gray-500 dark:text-gray-400`
}

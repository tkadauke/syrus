module SyrusChatMcp
  module LocalToolDispatch
    ...
    def self.call(tool_name, arguments, chat_session:)
      ...
      return SyrusChatMcp.tool_error(DISCONNECTED_ERROR) unless session&.connected?

      call = session.dispatch_tool_call!(tool_name, arguments)
      outcome = call.wait_for_result

      if outcome[:error]
        SyrusChatMcp.tool_error(outcome[:error].to_s)
      else
        SyrusChatMcp.success(outcome[:result])
      end
    end
  end
end

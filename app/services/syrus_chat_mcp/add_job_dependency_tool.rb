  ...
  class AddJobDependencyTool < MCP::Tool
    ...
    class << self
      def call(job_id:, depends_on_job_id: nil, depends_on_epic_id: nil, server_context:)
        ...
        return SyrusChatMcp.invalid("exactly one of depends_on_job_id or depends_on_epic_id must be supplied") if
      ...
      def add_epic_target(job, depends_on_epic_id, user)
        ...
        return SyrusChatMcp.invalid("epic not found: #{depends_on_epic_id}") unless epic

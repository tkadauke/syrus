  ...
  class AdminQueueDetailTool < MCP::Tool
    ...
    class << self
      def call(tab:, server_context:)
        ...
        tab = tab.to_s
        return SyrusChatMcp.invalid("tab must be one of #{TABS.join(', ')}") unless TABS.include?(tab)

        user = server_context.fetch(:chat_session).user
        payload = ::Admin::Queue::Payload.new(params: { tab: tab }, user: user).public_send(tab)
        SyrusChatMcp.success(tab: tab, **payload)
      rescue ActiveRecord::StatementInvalid,
             ActiveRecord::ConnectionNotEstablished,
             ActiveRecord::ActiveRecordError => e
        SyrusChatMcp.invalid("SolidQueue tables unreachable from this connection: #{e.message}")
      end

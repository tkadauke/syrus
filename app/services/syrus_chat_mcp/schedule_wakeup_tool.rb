  ...
  class ScheduleWakeupTool < MCP::Tool
    ...
    class << self
      def call(prompt:, delay_minutes:, server_context:)
        ...
        return SyrusChatMcp.invalid("prompt is required") if prompt.blank?
        unless delay_minutes && delay_minutes.between?(MIN_DELAY_MINUTES, MAX_DELAY_MINUTES)
          return SyrusChatMcp.invalid("delay_minutes must be between #{MIN_DELAY_MINUTES} and #{MAX_DELAY_MINUTES}")
        end

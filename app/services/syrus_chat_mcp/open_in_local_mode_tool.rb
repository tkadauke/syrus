    ...
    class << self
      def call(job_id:, server_context:)
        ...
        if job.linked_chat_id.present?
          return SyrusChatMcp.invalid("Job #{job_id} is already linked to a local coding session.")
        end
        unless job.implemented? || job.approved?
          return SyrusChatMcp.invalid("Job #{job_id} must be in implemented or approved state (current: #{job.state}).")
        end

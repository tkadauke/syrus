  ...
  class ResumeScheduledTaskTool < MCP::Tool
    ...
    class << self
      def call(scheduled_task_id:, server_context:)
        ...
        return SyrusChatMcp.invalid("scheduled task is already enabled") if scheduled_task_enabled?(task)
        return SyrusChatMcp.invalid("scheduled task cannot be resumed from state: #{task.state}") unless task.paused? || task.auto_paused?

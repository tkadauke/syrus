class StepWorkspace
  class PinnedWorkflowWorkspace
    def self.build(step, git:, log:, run: nil)
  ...
  class ImmutableSource
    def self.build(step, git:, log:, run: nil)
      ...
      return PinnedWorkflowWorkspace.build(step, git: git, log: log, run: run) unless Feature.distributed_workflow_dag_enabled?(repository)

      ImmutableSourceCheckout.new(step, git: git, log: log, run: run)
    end
  end
  ...
  def self.for(step, git: nil, log: nil, run: nil)
    POLICIES.fetch(step.placement_policy).build(step, git: git, log: log, run: run)
  end
end

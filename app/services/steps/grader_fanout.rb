module Steps
  ...
  class GraderFanout < Base
    ...
    def dependency_source_scope_for(grader)
      ...
      log("[grader_fanout] target dependency resolution failed for #{target_label}: #{e.message}")
      []
    end
    ...
    def prepare_commands_for(grader)
      ...
      log("[grader_fanout] target prepare dependency resolution failed for #{target_label_for(grader)}: #{e.message}")
      []
    end
    ...
    rescue TargetGraph::Error, SyrusYml::ParseError => e
      log("[grader_fanout] warning: target graph unavailable: #{e.message}")
      @target_graph = nil
    end

module App
  class DashboardPayload
      ...
      <<~SQL.squish
        (
          ...
          WHERE child_jobs.epic_id = epics.id
            AND (
              #{epic_release_blocking_sql}
              OR (
                child_jobs.dependencies_overridden_at IS NULL
                AND #{job_dependency_blocking_exists_sql("child_jobs")}
              )
            )
        )
      SQL
    ...
    def epic_release_blocking_sql
      <<~SQL.squish
        NOT (
          epics.state IN ('in_progress', 'done')
          AND NOT #{epic_dependency_blocking_exists_sql("epics")}
        )
      SQL
    end

    def epic_dependency_blocking_exists_sql(epic_table)
      <<~SQL.squish
        EXISTS (
          SELECT 1
          FROM epic_dependencies release_dependencies
          LEFT JOIN jobs release_dependency_jobs ON release_dependency_jobs.id = release_dependencies.depends_on_job_id
          LEFT JOIN epics release_dependency_epics ON release_dependency_epics.id = release_dependencies.depends_on_epic_id
          WHERE release_dependencies.epic_id = #{epic_table}.id
            AND (
              (
                release_dependencies.depends_on_job_id IS NULL
                AND release_dependencies.depends_on_epic_id IS NULL
              )
              OR (
                release_dependencies.depends_on_job_id IS NOT NULL
                AND NOT #{job_success_sql("release_dependency_jobs")}
              )
              OR (
                release_dependencies.depends_on_epic_id IS NOT NULL
                AND (
                  release_dependency_epics.id IS NULL
                  OR release_dependency_epics.state != 'done'
                )
              )
            )
        )
    ...
    def job_dependency_blocking_exists_sql(job_table)
      <<~SQL.squish
        EXISTS (
          SELECT 1
          FROM job_dependencies child_dependencies
          LEFT JOIN jobs dependency_jobs ON dependency_jobs.id = child_dependencies.depends_on_job_id
          LEFT JOIN epics dependency_epics ON dependency_epics.id = child_dependencies.depends_on_epic_id
          WHERE child_dependencies.job_id = #{job_table}.id
            AND (
              (
                child_dependencies.depends_on_job_id IS NULL
                AND child_dependencies.depends_on_epic_id IS NULL
                AND NOT #{pending_job_dependency_succeeded_sql(job_table, "child_dependencies")}
              )
              OR (
                child_dependencies.depends_on_job_id IS NOT NULL
                AND NOT #{job_dependency_succeeded_sql(job_table, "child_dependencies", "dependency_jobs")}
              )
              OR (
                child_dependencies.depends_on_epic_id IS NOT NULL
                AND (
    ...
    def job_dependency_succeeded_sql(job_table, dependency_table, dependency_job_table)
      <<~SQL.squish
        (
          (
          ...
          OR (
            ...
            AND (
              ...
              OR (
                #{job_table}.epic_id IS NOT NULL
                AND #{dependency_job_table}.epic_id = #{job_table}.epic_id
                AND #{dependency_job_table}.state IN ('approved', 'landing')
              )
            )
          )
        )
    ...
    def pending_job_dependency_succeeded_sql(job_table, dependency_table)
      github_issue_url_sql = if ActiveRecord::Base.connection.adapter_name.downcase.include?("mysql")
                               "CONCAT('https://github.com/', #{dependency_table}.unresolved_owner, '/', #{dependency_table}.unresolved_repo, '/issues/', #{dependency_table}.unresolved_number)"
      else
                               "('https://github.com/' || #{dependency_table}.unresolved_owner || '/' || #{dependency_table}.unresolved_repo || '/issues/' || #{dependency_table}.unresolved_number)"
      end

      <<~SQL.squish
        EXISTS (
          SELECT 1
          FROM repositories referenced_repositories
          INNER JOIN epics referenced_epics ON referenced_epics.repository_id = referenced_repositories.id
          WHERE referenced_repositories.user_id = #{job_table}.user_id
            AND referenced_repositories.owner = #{dependency_table}.unresolved_owner
            AND referenced_repositories.name = #{dependency_table}.unresolved_repo
            AND referenced_epics.github_issue_url = #{github_issue_url_sql}
            AND referenced_epics.state = 'done'
        )
      SQL
    end

    def job_success_sql(table_name)
      sanitized_success_reasons = Job::SUCCESSFUL_CLOSURE_REASONS.map { |reason| ActiveRecord::Base.connection.quote(reason) }.join(", ")
      "(#{table_name}.state = 'closed' AND #{table_name}.closure_reason IN (#{sanitized_success_reasons}))"
    end
    ...
    def blocked_epic_child_counts(epic_ids)
      Job
        ...
        .where("#{epic_release_blocking_sql} OR (jobs.dependencies_overridden_at IS NULL AND #{job_dependency_blocking_exists_sql('jobs')})")
        .group(:epic_id)
        .count
    end
  end

  ...
  class JobDetailPayload
    module WorkflowSerializers
      ...
      def latest_admission_details_for(step, workflow:)
        admission = admission_artifacts_for(workflow).find { |candidate| admission_applies_to_step?(candidate, step) }
      ...
      def admission_applies_to_step?(admission, step)
        step_id = admission["step_id"].presence
        return step_id.to_i == step.id if step_id

        run_id = admission["run_id"].presence
        return false unless run_id

        ordered_runs_for(step).any? { |run| run.id == run_id.to_i }
      end
        ...
        @job_log_stats ||= begin
          ...
          if ids.empty?
            ...
            JobLog
              ...
              .pluck(
                :run_id,
                Arel.sql("MAX(sequence)"),
                Arel.sql("MAX(CASE WHEN kind = 'rate_limited' THEN 1 ELSE 0 END)")
              )
              .to_h do |run_id, max_sequence, rate_limited|
                [
                  run_id,
                  {
                    count: max_sequence.to_i + 1,
                    rate_limited: rate_limited.to_i.positive?
                  }
                ]
              end
          end

module Workflows
  ...
  class MainBranchRepair < Base
    ...
    # when the PR merges.
    #
    # Preflight path (graders passed → no fix needed): update the main-branch
    # health signals represented by the CI-phase preflight plan, notify
    # MainHealthChangedService of the transition so landing can resume, and
    # close the anchor job.
    def self.after_success(workflow)
      ...
      updates = { ci_health: "healthy", grader_health: "healthy" }

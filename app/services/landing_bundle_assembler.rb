...
class LandingBundleAssembler
  ...
  def self.for_repository(repository, exclude_active: true)
    scope = if exclude_active
      Scopes::PriorityTier.new(repository)
    else
      Scopes::PriorityTier.new(repository, exclude_active: false)
    end

  ...
  class AdminListProcessesTool < MCP::Tool
    ...
    class << self
      def call(state: "running", kind: nil, limit: DEFAULT_LIMIT, server_context:)
        ...
        return Mcp::Tools.invalid("state must be one of #{STATES.join(', ')}") unless STATES.include?(state)
        if kind.present? && !SpawnedProcess::KINDS.include?(kind)
          return Mcp::Tools.invalid("kind must be one of #{SpawnedProcess::KINDS.join(', ')}")
        end
        ...
        Mcp::Tools.success(processes: processes)
      end

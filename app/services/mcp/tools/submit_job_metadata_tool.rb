  ...
  class SubmitJobMetadataTool < MCP::Tool
    ...
    class << self
      ...
      def call(changed:, title: nil, summary: nil, pr_body: nil, test_plan: nil, intent_revision_reason: nil, server_context:)
        ...
        Mcp::Tools.write_log(run, "[mcp] submit_job_metadata received: changed=#{changed}")
      ...
      def normalize_payload(changed:, title:, summary:, pr_body:, test_plan:, intent_revision_reason:)
        {
          "changed" => changed,
          "title" => Mcp::Tools.utf8(title).strip.presence,
          "summary" => Mcp::Tools.utf8(summary).strip.presence,
          "pr_body" => Mcp::Tools.utf8(pr_body).strip.presence,
          "test_plan" => normalize_test_plan(test_plan),
          "intent_revision_reason" => Mcp::Tools.utf8(intent_revision_reason).strip.presence,
      ...
      def normalize_test_plan(test_plan)
        ...
        steps = Array(test_plan["steps"] || test_plan[:steps]).map { |step| Mcp::Tools.utf8(step).strip }.reject(&:empty?)
        notes = Mcp::Tools.utf8(test_plan["notes"] || test_plan[:notes]).strip.presence

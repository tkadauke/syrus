  ...
  class CompleteImplementStepTool < MCP::Tool
    ...
    input_schema(
      properties: {
        ...
        branch_name: {
          type: "string",
          description: "Branch name to hand off. In Local Mode, required for Jobs without an existing PR and replaces the stored branch when supplied for a rerun. In Coding Mode this names the local checkout branch, not a branch you pushed yourself; omit it to use the active coding checkout branch."
        }
      },
      required: %w[job_id]
    )
    ...
    class << self
      def call(job_id:, branch_name: nil, server_context:)
        ...
        if chat_session.local? && job.pr_number.blank? && normalized_branch.blank?

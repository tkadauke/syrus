  ...
  class SubmitSummaryTool < MCP::Tool
    ...
    class << self
      ...
      def call(pr_title:, pr_body:, summary:, server_context:)
        ...
        return Mcp::Tools.not_authorized unless McpToolPolicy.capability_permitted?(context, :submit_summary)

        title   = Mcp::Tools.utf8(pr_title).strip
        body    = Mcp::Tools.utf8(pr_body).strip
        summary = Mcp::Tools.utf8(summary).strip

        return Mcp::Tools.invalid("pr_title is required")                          if title.empty?
        return Mcp::Tools.invalid("pr_title too long (#{title.length} chars)")     if title.length > MAX_TITLE_LENGTH
        return Mcp::Tools.invalid("pr_body is required")                           if body.empty?
        return Mcp::Tools.invalid("summary is required")                           if summary.empty?
        ...
        Mcp::Tools.write_log(run, "[mcp] submit_summary received: #{title.inspect}")

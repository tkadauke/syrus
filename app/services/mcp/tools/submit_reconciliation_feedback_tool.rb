  ...
  class SubmitReconciliationFeedbackTool < MCP::Tool
    ...
    class << self
      def call(job_id:, feedback:, server_context:)
        ...
        return Mcp::Tools.not_authorized unless McpToolPolicy.capability_permitted?(context, :submit_chat_feedback)

        job_id_int = Integer(job_id, exception: false)
        return Mcp::Tools.invalid("job_id must be an integer") unless job_id_int

        feedback_text = Mcp::Tools.utf8(feedback.to_s).strip
        return Mcp::Tools.invalid("feedback is required") if feedback_text.blank?

        target_job = Job.find_by(id: job_id_int)
        return Mcp::Tools.invalid("Job #{job_id_int} not found") unless target_job
        ...
        return Mcp::Tools.invalid(result.error) unless result.success?

        Mcp::Tools.write_log(run, "[mcp] submit_chat_feedback: submitted feedback for #{target_job.slug}")
        MCP::Tool::Response.new([ { type: "text", text: "Feedback submitted to #{target_job.slug}." } ])
      rescue StandardError => e
        Rails.logger.error("[Mcp::Tools::SubmitReconciliationFeedbackTool] #{e.class}: #{e.message}")

...
module Mcp::Tools
  class SendChatMessageTool < MCP::Tool
    ...
    class << self
      def call(server_context:, text:, target_chat_session_id: nil, thread_id: nil, max_hops: nil)
        ...
        thread =
          if thread_id.present?
            find_reply_thread!(thread_id, chat_session)
          else
            open_thread!(chat_session, target_chat_session_id, max_hops, server_context)
          end
        return thread if thread.is_a?(MCP::Tool::Response)
      ...
      def open_thread!(chat_session, target_chat_session_id, max_hops, server_context)
        ...
        thread = ChatBridgeThread.new(
          origin_chat_session: chat_session,
          target_chat_session: target,
      ...
      def success_payload(thread, chat_session, result)
        to_chat_session = thread.origin_chat_session_id == chat_session.id ? thread.target_chat_session : thread.origin_chat_session

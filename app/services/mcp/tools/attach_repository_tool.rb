  ...
  class AttachRepositoryTool < MCP::Tool
    ...
    class << self
      def call(slug:, server_context:)
        ...
        return Mcp::Tools.invalid("repository #{owner}/#{name} is not configured for this user") unless repository
        ...
        Mcp::Tools.invalid("could not authenticate to #{slug} for cloning: #{e.message}. Check the repository's GitHub App installation or your GitHub token in credentials.")
        ...
        Rails.logger.warn("[Mcp::Tools] attach_repository failed for #{slug}: #{e.class}: #{e.message}")
        Mcp::Tools.invalid("failed to attach #{slug}: #{e.message}")
      end

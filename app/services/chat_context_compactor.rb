class ChatContextCompactor
  ...
  def record_compaction_metrics!(checkpoint:, compacted:, previous:)
    kept = exclude_current_unanswered_user(
      messages_scope.where("id > ?", checkpoint.compacted_through_message_id).order(:id)
    ).pluck(:content)
    ...
    payload = {
      ...
      estimated_kept_context_bytes: estimated_content_bytes(kept),
    ...
    OperationalLogging.ingest(
      ...
      message: "Compacted #{compacted.size} messages for chat #{@chat_session.id} (kept #{kept.size} raw)",
      context: payload
    )
  rescue StandardError => e
    Rails.logger.warn("[ChatContextCompactor] failed to record compaction metrics for chat #{@chat_session.id}: #{e.class}: #{e.message}")
    nil
  end

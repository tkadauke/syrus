...
class AgentEnvironmentSnapshot
  ...
  def run_design_doc_lines
    docs = DesignDoc.visible_to(run.job.user)
    ...
    rendered = docs.map do |doc|
      "#{doc.display_id} #{doc.title} (#{doc.visibility}, #{doc.state}, v#{doc.current_version&.version_number || 'unknown'})"
    end
    [
      "- Design docs: #{rendered.join('; ')}",
      "- Design docs access: worker agents may call `list_design_docs` and `read_design_doc` for readable DOC-<id> references; no design doc mutation tools are available in workflow runs."
    ]

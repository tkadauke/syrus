...
module WorkEngine
  class RepeatedFailureCircuit
    THRESHOLD = 3
    TOP_STACK_FRAMES = 5
    PROBLEM_CODE = "repeated_failure_circuit_open".freeze
    ...
    def recent_workflows
      job.workflows
        ...
        .terminal
        .where("finished_at IS NULL OR finished_at <= ?", run.workflow&.finished_at || run.finished_at || Time.current)
        .order(Arel.sql("COALESCE(finished_at, updated_at) DESC"), id: :desc)
    end

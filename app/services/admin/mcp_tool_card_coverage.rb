module Admin
  class McpToolCardCoverage
    ...
    def aggregate_dashboard_rows(scope)
      scope
        .then { |relation| strong_card_names.empty? ? relation : relation.where.not(normalized_tool_name: strong_card_names) }
    ...
    def strong_card_names
      @strong_card_names ||= cards.values.select(&:strong?).map(&:tool_name)
    end

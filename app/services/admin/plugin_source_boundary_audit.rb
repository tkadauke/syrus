module Admin
  class PluginSourceBoundaryAudit
    ...
    def manifest_for(dir)
      files = Dir.glob(dir.join("lib/**/*.rb").to_s).sort
      source = files.map { |file| File.read(file) }.join("\n")
      ...
      Manifest.new(
        ...
        constants: top_level_constants(files),
    ...
    def top_level_constants(files)
      files.flat_map do |file|
        File.readlines(file).filter_map do |line|
          line[/\A\s*(?:module|class)\s+([A-Z][A-Za-z0-9_]*)\b/, 1]
        end
      end.uniq
    end
    ...
    def reference_pattern_for(manifests)
      tokens = manifests.flat_map do |manifest|
        [ "plugins/#{manifest.dir_name}/", "#{manifest.dir_name}/", manifest.gem_name, *manifest.constants.map { |constant| "#{constant}::" } ]
    ...
    def reference_reason(line, manifest)
      ...
      constant = manifest.constants.find { |name| unquoted_line.match?(/\b#{Regexp.escape(name)}::/) }
    ...
    def allowed_core_reference?(path, manifest, line)
      ...
      guarded.any? do |constant|
        manifest.constants.include?(constant) &&
          line.match?(/\b#{Regexp.escape(constant)}::/) &&

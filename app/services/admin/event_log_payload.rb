module Admin
  # Shared pagination/sort/filter/revision-scope plumbing for the admin
  # event-log payloads (backend exceptions, browser errors). Subclasses
  # supply the model, SORTS map, filter definition, and per-event JSON.
  class EventLogPayload
    ...
    def filtered_scope
      filter_definition.apply(model_class.all, params)
    end

    def sorted_scope
      columns = self.class::SORTS.fetch(sort_column)
    ...
    def sort_column
      ...
      self.class::SORTS.key?(value) ? value : "time"
    end

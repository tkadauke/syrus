...
module MaintenanceTasks
  module Definitions
    class SearchDatabaseRebuild < Base
      ...
      def index_jobs(task)
        indexer = job_search_source_provider
      ...
      def index_epics(task)
        indexer = epic_search_source_provider
      ...
      def jobs_need_rebuild?
        job_search_source_provider && indexed_count("job_fts", "job_id") < Job.count
      rescue StandardError
        job_search_source_provider && Job.exists?
      end

      def epics_need_rebuild?
        epic_search_source_provider && indexed_count("epic_fts", "epic_id") < Epic.count
      rescue StandardError
        epic_search_source_provider && Epic.exists?
      end
      ...
      def jobs_count = job_search_source_provider ? Job.count : 0
      def epics_count = epic_search_source_provider ? Epic.count : 0
      def operational_logs_count = OperationalLogging.configured_for_instance? ? OperationalLogEvent.count : 0

      def job_search_source_provider = search_source_providers.find { |provider| provider.respond_to?(:index_job) }

      def epic_search_source_provider = search_source_providers.find { |provider| provider.respond_to?(:index_epic) }

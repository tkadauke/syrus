class TargetGraph
  ...
  Project = Data.define(:id, :label, :kind, :path, :owner_config_path, :preview, :visual_review, :adversarial_review) do
    ID_PATTERN = /\A[A-Za-z0-9_-]+\z/

    def initialize(id:, label: nil, kind: nil, path: "", owner_config_path: nil, preview: nil, visual_review: nil, adversarial_review: nil)
      id = id.to_s.strip

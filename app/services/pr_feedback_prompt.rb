class PrFeedbackPrompt
  # `comments` is a chronologically-sorted array containing a mix of
  # GitHub issue_comments (no `path` attr) and review_comments (have
  # `path`, `line`, `diff_hunk`). The composer formats each kind so the
  # agent can tell where the feedback is anchored.

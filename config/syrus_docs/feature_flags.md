...
- `syrus.performance.slow_phase` for instrumented application phases slower than `SYRUS_PERFORMANCE_SLOW_PHASE_MS` (default 250 ms), including request context, sanitized metadata, phase-local SQL counters, and top SQL fingerprints observed while that phase was active.

...
This Job intentionally ships only the backend capability, flag, and permission gate — no MCP tool or pending-action confirmation wiring calls `EmergencyLand::Lander` yet, and no UI surfaces it. Those are separate follow-up Jobs in the same Epic.

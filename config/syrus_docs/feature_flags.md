...
Supervisor agent turns receive the `chat:admin` role. Admin users keep the normal admin MCP tool set, but the Supervisor prompt directs the agent to treat `supervisor_event` system messages as operational context, summarize incidents, inspect live Syrus state, read current state before acting, and keep risky side effects behind proposals or pending-action confirmation. Missing repository attachment is normal for Supervisor operations triage; the agent should ask for a repository only when the operator explicitly requests code inspection, proposal drafting, or another repository-context-dependent task. Chat-history fallback preserves Supervisor event messages and pending-action outcome notices so provider resume failures do not drop the audit trail that motivated an action.

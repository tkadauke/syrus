...
**Degrades gracefully today.** Hotfix-sync and upstream-export ref-movement workflows don't exist yet (later Epic Jobs), so no repository has `hotfix_sync`/`upstream_export` enabled and no Job ever has an `upstream_export`-role `JobPrLink` row yet. Promotion itself now exists (below), but `Workflows::Promotion` writes its `promotion`-role `JobPrLink` onto its own synthetic anchor Job, not onto the individual dev Jobs that landed the commits being promoted — so `DeliveryStatus.for(job:)` for an ordinary dev Job still can't observe "my track's promotion happened," and resolves to `:waiting_for_promotion` (or one of the other pre-existing buckets) exactly as before. Wiring promotion status back onto every constituent Job is left for a later Job in this Epic, same as the hotfix-sync/upstream-export gap.

...
grader, desktop unit tests still use the root Vitest dependency set, and the
root Go-workspace backstop still covers release/desktop packaging paths that
bundle the CLI.
...
dependency in its dependency closure is also healthy for its own current
fingerprints. Unknown, stale, failed, timed-out, cancelled, or inconclusive
target health is treated as a miss, so required targets still run. Health-skip
and forced-rerun decisions are recorded in workflow logs and artifacts:
`format_target_health_skips`,
...
Skipped and cached are different decisions:

- `skipped` means the target was considered during selection but was not
  affected by the current change. The reason is usually `no matching files
  changed`, meaning neither the target's own source scope nor any dependency in
  its transitive `deps:` closure matched the diff. A plain skipped target does
  not need, and usually will not have, a target-health proof.
- `cached` means the target was selected as affected, but Syrus found a healthy
  reusable target-health proof for the current target, command, and environment
  fingerprints, and every executable dependency in its closure also had healthy
  current health.

The Job Target Graph cached-target explanation includes the reused health
record id, producing commit SHA, checked timestamp, and any referenced
dependency health records. The skipped-target explanation instead focuses on
selection evidence: target label, project, grader name, requiredness, and the
reason it did not match the diff.

Fixes:

- If an ordinary skip is wrong, add or correct `sources`, `when_files_changed`,
  or `deps:` so the changed file reaches the intended executable target.
- If the skip is correct but surprising, rename the project or target so the UI
  makes the selection result easier to recognize.

...
Grade carry-forward records a fresh `LandingValidationCache` artifact for the post-rebase head, tree, base ref, base SHA, and required-grader fingerprint. It only carries forward from a successful required-grader validation, not from another carried validation, and the current required-grader fingerprint must match the source validation. The fingerprint includes grader selection inputs such as `when_files_changed`, so changing the required grader set or its file globs rejects carry-forward. Later reuse reports this as `clean_rebase_carry_forward` rather than as a normal exact-head hit. It is never applied speculatively.

...
`relation(base_id, head_id)`, `history(base_id, head_id)`,
`tree_sha(revision_id)`, and `divergence(base_id, head_id)`. Relation
...
what it got, the same as `changes` does for GitHub's 300-file cap. `tree_sha`
answers a commit's root tree object id -- two revisions share one exactly
when they would produce identical content, which is how the landing path
recognizes an empty merge without diffing files itself. Divergence is the
numeric form of the relation comparison: a `RepositoryContent::Divergence`
with `ahead` (commits `head` has that `base` does not) and `behind` (commits
`base` has that `head` does not) -- GitHub compare's `ahead_by`/`behind_by`.
`git_mirror` answers it from its already-synchronized local bare mirror
(`git rev-list --left-right --count`); `github_host` answers it from GitHub's
compare API. `CommitsBehindCalculator` is the reference caller: it asks the
provider chain first and falls back to a local `RepositoryBareClone` only
when no provider can answer. Replicas refresh movable refs within `max_age`;
`max_age: 0` requests a current authoritative answer.
...
- `syrus_rails` (registered manifest name `syrus-rails`) — installed but
  ...
  diff renderers), and `:prompt_injector`. The plugin engine registers these
  providers through its manifest at boot; core does not call the plugin
  namespace directly.
...
- `browser` — default-enabled. Provides `:mcp_tool_set`
  ...
  `browser_evaluate`, `browser_file_upload`, `browser_drop`, `browser_drag`,

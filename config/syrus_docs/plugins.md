...
returning `{error: "lease_required"}` with no active agent input lease). Every
acquire/release/cancel/expire and delivered input event is audited via
`JobLog` (when the session has a `run`) and broadcast live over

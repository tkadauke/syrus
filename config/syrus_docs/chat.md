...
chat provider such as Muse or Antigravity. Each chat stores a concrete
...
delivery path: it posts a `role: "system"` outbound-bridge message in the
sending chat, creates the matching inbound turn in the other chat via a
`ChatWakeup` (`metadata: { "requested_by" => "cross_chat", ... }`, so it
flows through the existing `ChatWakeup -> ChatWakeupFireJob -> ChatTurnJob`
path and inherits that job's per-chat concurrency limiting), increments the
thread's hop count, and posts a closure notice to both chats once the hop
cap is reached. `from:` defaults to the thread's origin chat but accepts
either side, since a reply can come from either chat once a thread is open.
Delivery on an already-closed thread raises
`ChatSession::CrossChatMessage::ClosedThreadError`.
...
itself always delivered through a `cross_chat` wakeup. The tool returns the
thread's id, `state`, `hop_count`, `max_hops`, `hops_remaining`, and the
recipient chat session id.

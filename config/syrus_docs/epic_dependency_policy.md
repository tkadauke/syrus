...
`ChatEpicProposalMaterializer` also rejects confirming any Epic proposal — bare `propose_epic` or `propose_epic_with_jobs` — that would leave the resulting Epic with zero child Jobs: no proposed children on the card, and (when `epic.epic_id` targets an existing Epic) that Epic has no Jobs of its own either. A zero-child Epic would confirm successfully and then implement nothing, which surprises the operator far more than a rejected confirmation does. One child Job is allowed. `propose_epic` (the Epic-only tool) can still *create* a zero-child proposal card — "propose the Epic now, add child Jobs in a follow-up `propose_epic_with_jobs` card before confirming" is a supported way to draft an Epic incrementally — only *confirming* it while it (and any existing target Epic) still has no children is blocked.

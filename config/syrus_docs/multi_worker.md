...
`complete_implement_step` hands committed Coding Mode work to the
`coding_handoff` workflow after operator confirmation. Once the workflow starts,
it owns any further repair and publication work from a normal workflow checkout,
so it runs safely on any compute pod.

...
KNOWN_VOLUMES="syrus_syrus-data syrus_syrus-search syrus_syrus-mise-cache"
...
if [ "$docker_ready" = "1" ]; then
  if [ "$KEEP_DATA" = "1" ]; then
    ...
    info "        syrus_syrus-mise-cache as a fallback) — verified by re-listing after teardown"
  fi

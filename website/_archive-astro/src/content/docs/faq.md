...
Yes. Syrus has an agent-provider abstraction and supports Codex and
OpenCode in addition to Claude. Users choose a default provider for Jobs and
future chats, chat can use Claude or Codex, repositories can override Job
provider selection, and retry actions can use any provider the user has
configured.

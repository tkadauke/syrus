...
- **Design System** — when the bundled `syrus_dev` plugin is enabled, Admin → Design System (`/admin/design_system`) renders Syrus's real shared UI components alongside the current color token values, including the DOC-27 semantic page, section, notice, text, surface, toolbar, chip, and layout helpers. The gallery is development/internal tooling, not a normal account setting. The hidden `/design_system` path remains as the render target the chat `preview_theme` tool opens in a popup; appending `?theme_id=<id>` previews that theme's token values against the same real component gallery, scoped to the page's own content only — it never changes the theme of the surrounding app chrome or any other open tab.

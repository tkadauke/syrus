...
- **Design System** — a settings page (`/design_system`) rendering Syrus's real shared UI components (buttons, form controls, cards, a modal, status pills, headings) alongside the current color token values, doubling as living documentation and a way to spot-check components that haven't migrated onto the shared design-system primitives yet. Any signed-in user can visit it. Appending `?theme_id=<id>` to the URL previews that theme's token values against the same component gallery, scoped to the page's own content only — it never changes the theme of the surrounding app chrome or any other open tab.

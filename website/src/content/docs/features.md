...
`/epic [id]`, `/prs`, `/issues`, and `/proposals`; ID commands without an ID
open a picker scoped to the attached repository when possible. Job action
commands such as `/cancel [id]`, `/retry [id]`, `/approve [id]`, `/discard`,
and `/clear-canvas` show an inline confirmation before they mutate anything.
`/approve` accepts `JOB-123`, `job-123`, or `123`, opens a picker of
implemented Jobs when no ID is provided, and approves the selected Job for
landing after confirmation. Skill commands such as `/canvas` and `/feedback`
are sent through the normal chat message path for the agent to interpret. The
`/propose` skill command starts a guided wizard: the agent asks for a Job
title, description, and optional Epic, then creates a proposal card for

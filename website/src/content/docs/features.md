...
Pending chat proposals for a Job or Epic show up in that chat's own Jobs tab
(see below) rather than on the dashboard, so proposed-but-unconfirmed work
stays scoped to the conversation that's actually driving it.
...
Once the current chat session has a pending, confirmed, or linked Job or
Epic proposal, a Jobs tab appears in the workspace panel. A "Proposed"
section at the top lists proposals still awaiting confirmation — both
direct Job proposals and Epic bundles, each showing its kind, a relative
timestamp, and (for an Epic bundle) its child count — and clicking a card
jumps straight to the chat message where it was posted. Below that, confirmed
...
list. Each confirmed card displays the Job state, the active workflow step or
PR link, and a red blocker banner when operator action is required (awaiting
review, landing failed, or a failed dependency); clicking it navigates to the
Job detail page. Both sections update in real time as proposals are created,
confirmed, rejected, or withdrawn, and as any Job originating from this chat
session changes. PDFs are passed to the agent without an inline preview.
...
the bounded top SQL fingerprints captured for both the request and each phase;
these samples are diagnostic context, not an exhaustive query log. The SQL tab
can run EXPLAIN for captured slow-SQL samples and shows a visual plan, raw
table rows, raw JSON, and normalized SQL.
EXPLAIN ANALYZE is available only when the backend classifies the statement as
safe: MySQL, a single read-only SELECT/CTE, no comments or user variables, and
a short statement timeout because analyze executes the query. Implementation
...
- **Design System** — when the bundled `syrus_dev` plugin is enabled, Admin → Design System (`/admin/design_system`) renders Syrus's real shared UI components alongside the current token values — both the color tokens and the non-color shape/shadow/spacing/density/typography token groups — including the DOC-27 semantic page, section, notice, text, surface, toolbar, chip, and layout helpers. The gallery is development/internal tooling, not a normal account setting. The hidden `/design_system` path remains as the render target the chat `preview_theme` tool opens in a popup; appending `?theme_id=<id>` previews that theme's full token set (color and non-color) against the same real component gallery, scoped to the page's own content only — it never changes the theme of the surrounding app chrome or any other open tab.

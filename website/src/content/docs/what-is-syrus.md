...
| Workflow | One attempt to move a Job forward. Trigger kinds include `initial`, `pr_comment`, `chat_feedback`, `ci_failure`, `retry`, `manual`, `rebase`, `stack_rebase`, `auto_merge`, `merge_train`, `coding_handoff`, `local_mode_handoff`, `main_branch_repair`, `main_grader`, and `agent_insight`. |

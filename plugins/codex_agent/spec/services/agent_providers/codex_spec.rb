...
RSpec.describe AgentProviders::Codex do
  ...
  describe "#run" do
    ...
    it "launches the sidecar for non-implement steps too" do
      ...
      review_run = review_step.runs.create!(job: job, trigger_kind: run.trigger_kind, agent_provider: "codex")
      ...
      result = review_adapter.run(prompt: "review the diff", log_sink: ->(*, **) { })

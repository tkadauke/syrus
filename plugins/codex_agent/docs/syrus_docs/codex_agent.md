...
Runs and chat turns to the Codex CLI. It is customer-facing but **disabled by
default** (`default_enabled: false`, `disableable: true`) — a brand-new
install starts with every agent-provider plugin off, so an operator enables
at least one from Admin -> Plugins before Jobs or chats can run. It provides
both an `agent_provider` (`AgentProviders::Codex`) and a `chat_provider`
(`ChatProviders::Codex`), category `agent_provider`. It runs independently of
`claude_agent` — either or both can be enabled, and a Job/chat picks a

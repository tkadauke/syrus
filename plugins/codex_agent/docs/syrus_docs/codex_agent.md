...
Runs and chat turns to the Codex CLI. It is **default-disabled**,
customer-facing, and provides both an `agent_provider`
(`AgentProviders::Codex`) and a `chat_provider` (`ChatProviders::Codex`),
category `agent_provider`, `disableable: true`. A new install ships with no
agent provider enabled; an operator turns one on from Admin -> Plugins once
its credentials are configured (see `config/syrus_docs/plugins.md`). It runs
independently of `claude_agent` — either or both can be enabled, and a

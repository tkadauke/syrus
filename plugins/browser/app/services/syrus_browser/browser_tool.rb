...
module SyrusBrowser
  ...
  class BrowserTool < MCP::Tool
    class << self
      ...
      # fill, hover, drag, drop, file_upload, and evaluate (arbitrary JS can
      # dispatch synthetic click/keyboard events or set form field values,
      # simulating the same input the other gated tools perform). These are
      # the only working way to drive pointer/keyboard input today
      # (RuntimeSessionProvider#input itself still answers

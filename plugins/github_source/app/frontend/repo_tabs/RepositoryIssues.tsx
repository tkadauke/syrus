...
export function RepositoryIssues({
  ...
  onRefresh: () => void
  payload: RepositoryIssuesPayload
  prefix: string
}) {
  ...
  const command = useMutation({
    mutationFn: (action: IssueCommand) => {
      switch (action.kind) {
        ...
        case "comment":
          return commentRepositoryIssue(payload.paths.app_comment_issue_path, {
            issueNumber: action.issueNumber,
            commentBody: action.commentBody,
            state: payload.state
          })
        case "bulk":
          return bulkRepositoryIssues(payload.paths.app_bulk_issues_path, {
            issueNumbers: action.issueNumbers,
  ...
  return (
    <>
      <div className="flex flex-wrap gap-x-5 gap-y-1 text-sm text-gray-600 dark:text-gray-400">
        ...
        <span>
          <strong>{payload.issue_count}</strong> {payload.issue_count === 1 ? "issue" : "issues"}
        </span>
        {payload.repository.github_rate_limit ? (
          <span>
            {t("repository.github_quota_prefix")} <strong>{payload.repository.github_rate_limit.remaining.toLocaleString()}</strong> /{" "}
            {payload.repository.github_rate_limit.limit.toLocaleString()} ({payload.repository.github_rate_limit.resource})
          </span>
        ) : null}
        <a className="text-brand hover:underline dark:text-brand-emphasis" href={payload.paths.github_issues_path} rel="noopener" target="_blank">
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <div className="flex gap-1">
            <Link className={stateFilterClass(payload.state === "open")} to={withRoutePrefix(payload.state_paths.open, prefix)}>
              {t("repository.state_open")}
            </Link>
            <Link className={stateFilterClass(payload.state === "closed")} to={withRoutePrefix(payload.state_paths.closed, prefix)}>
              {t("repository.state_closed")}
            </Link>
          </div>
          <Button className="disabled:text-gray-400 dark:disabled:text-gray-500" disabled={isRefreshing} onClick={onRefresh} variant="secondary">
      ...
      {payload.issues.length > 0 ? (
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead className="bg-gray-50 dark:bg-gray-800 text-left text-xs uppercase text-gray-500 dark:text-gray-400">
              <tr>
                ...
                <th className="w-36 px-4 py-2 text-right">{t("repository.col_actions")}</th>
              </tr>
            </thead>
        ...
        <OnboardingEmptyState
          ...
          fallbackDescription={
            payload.state === "open"
              ? `No open issues are available to delegate. Create or label an issue with ${payload.repository.trigger_label}, or use a direct job for first-run work.`
              : "No closed issues are available in this repository view."
          }
        ...
        <section className="rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4">
          ...
          <form className="mt-3 space-y-3" onSubmit={submitComment}>
            ...
            <div className="flex justify-end gap-2">
              <Button onClick={() => setCommentingOn(null)} variant="secondary">
                {t("repository.cancel")}
              </Button>
              <button className={buttonClass("blue")} disabled={command.isPending} type="submit">
                {t("repository.post_comment")}
              </button>
            </div>
          </form>
        </section>
    ...
    <tr>
      ...
      <td className="px-4 py-3 align-top">
        <div className="flex flex-wrap items-baseline gap-2">
          <a className="font-mono text-gray-500 dark:text-gray-400 hover:underline" href={issue.html_url} rel="noopener" target="_blank">
            #{issue.number}
          </a>
          {issue.labels.map((label) => (
            <IssueLabel color={label.color} key={label.name} name={label.name} />
          ))}
          <a className="font-medium text-gray-900 dark:text-gray-100 hover:underline" href={issue.html_url} rel="noopener" target="_blank">
            {issue.title}
          </a>
        </div>
        <div className="mt-1 text-xs text-gray-500 dark:text-gray-400">
          {issue.user_login ? `${issue.user_login} · ` : ""}
          {issue.created_at ? <RelativeTimestamp value={issue.created_at} /> : ""}
        </div>
      ...
      <td className="px-4 py-3 align-top">
        <div className="flex flex-col items-stretch gap-1.5">
          ...
          {state === "open" ? (
            <button
              className="rounded bg-amber-50 dark:bg-amber-950/40 px-2 py-1 text-xs font-medium text-amber-700 dark:text-amber-300 hover:bg-amber-100 dark:hover:bg-amber-950/60"
              disabled={commandPending}
              onClick={onClose}
              type="button"
            >
          ...
          {issue.delegated ? (
            <span className="rounded border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-950/40 px-2 py-1 text-center text-xs font-medium text-green-700 dark:text-green-300">
              {t("repository.delegated")}
            </span>
          ) : (
            <button
              className="rounded bg-brand/10 px-2 py-1 text-xs font-medium text-brand hover:bg-brand/20"
              disabled={commandPending}
              onClick={onDelegate}
              type="button"
            >
...
export default function RepositoryIssuesTab() {
  ...
  return (
    ...
    >
      ...
      {payload ? <RepositoryIssues isRefreshing={issues.isFetching} onRefresh={() => issues.refetch()} payload={payload} prefix={prefix} /> : null}
    </RepositoryPageShell>
  )
}

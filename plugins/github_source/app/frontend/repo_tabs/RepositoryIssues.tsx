  ...
  const command = useMutation({
    mutationFn: (action: IssueCommand) => {
      ...
      switch (action.kind) {
        ...
        case "comment":
          return commentRepositoryIssue(payload.paths.app_comment_issue_path, { issueNumber: action.issueNumber, commentBody: action.commentBody, folder, filterParam, sort })
        case "bulk":
...
function buildIssueColumns({
  ...
  onComment: (issue: RepositoryIssue) => void
    ...
    },
    {
      ...
      renderCell: (issue) => (
        <div className="flex flex-wrap justify-end gap-1.5">
          <Button disabled={commandPending} onClick={() => onComment(issue)} size="sm" variant="secondary">

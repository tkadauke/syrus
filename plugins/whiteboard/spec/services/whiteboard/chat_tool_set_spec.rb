...
RSpec.describe Whiteboard::ChatToolSet do
  let(:user) { Factories.user }
  let(:repository) { Factories.repository(user: user) }
  let(:chat_session) { ChatSession.create!(user: user, repository: repository) }
  let(:tool_set) { Whiteboard::ChatToolSet.new }
  ...
  it "read_scene returns the empty scene without rasterization or mutation logging" do
    ...
    expect(Whiteboard::Board.for(chat_session.reload)).to be_nil
  ...
  it "draw_shape appends a server-id shape, increments the version, and broadcasts" do
    ...
    whiteboard = Whiteboard::Board.for(chat_session.reload)
  ...
  it "draw_text appends a text element" do
    ...
    element = Whiteboard::Board.for(chat_session.reload).elements.first
    expect(response.error?).to be_falsey
  ...
  it "draw_arrow binds both arrow endpoints and both endpoint shapes" do
    ...
    elements = Whiteboard::Board.for(chat_session.reload).elements
  ...
  it "delete_element removes the element" do
    ...
    expect(response.error?).to be_falsey
    expect(Whiteboard::Board.for(chat_session.reload).elements).to be_empty
  end
  ...
  it "save_canvas returns an empty-canvas result without creating a snapshot" do
    ...
    }.not_to change(Whiteboard::Snapshot, :count)
  ...
  it "save_canvas saves a non-empty canvas as a manual snapshot" do
    ...
    }.to change(Whiteboard::Snapshot, :count).by(1)

    snapshot = Whiteboard::Snapshot.first
    expect(response.error?).to be_falsey
  ...
  it "load_canvas merges snapshot elements with fresh ids and merged files" do
    ...
    snapshot = Whiteboard::Snapshot.create_from_scene!(
      chat_session: chat_session,
      kind: "manual",
      scene: {
        "elements" => [
  ...
  it "load_canvas replace mode auto-saves the current canvas and replaces scene and files" do
    ...
    auto_saved_snapshot = Whiteboard::Snapshot.where(snapshot_kind: "auto_before_load").first
    whiteboard = Whiteboard::Board.for(chat_session.reload)
    expect(response.error?).to be_falsey
    expect(payload(response)).to include(
      loaded: true,
  ...
  it "load_canvas enforces the element limit before merging" do
    Whiteboard::Board.create!(chat_session: chat_session,
      scene_json: { "elements" => Array.new(Whiteboard::Board::MAX_ELEMENTS) { |index| minimal_element(index) } }
    )
    snapshot = Whiteboard::Snapshot.create_from_scene!(
      chat_session: chat_session,
      kind: "manual",
    ...
    expect(response.error?).to be(true)
    expect(response.content.first[:text]).to eq(Whiteboard::Board.element_limit_message)
    expect(Whiteboard::Board.for(chat_session.reload).elements.size).to eq(Whiteboard::Board::MAX_ELEMENTS)
  end
  ...
  it "update_scene replaces the elements array" do
    ...
    expect(response.error?).to be_falsey
    expect(Whiteboard::Board.for(chat_session.reload).elements).to eq(replacement)
    expect(Whiteboard::Board.for(chat_session.reload).app_state).to eq("viewBackgroundColor" => "#fff")
    expect(Whiteboard::Board.for(chat_session.reload).files).to eq(files)
  end
  ...
  it "rejects raw scene replacements beyond the element limit" do
    response = call_tool(
      "update_scene",
      elements: Array.new(Whiteboard::Board::MAX_ELEMENTS + 1) { |index| minimal_element(index) }
    )
  ...
  describe Whiteboard::Canvas do
    describe ".text_element" do
      it "sets height for a single line based on font_size" do
        element = Whiteboard::Canvas.text_element(content: "Hello", x: 0, y: 0, font_size: 20)
      ...
      it "treats empty string as a single line with minimum width" do
        element = Whiteboard::Canvas.text_element(content: "", x: 0, y: 0, font_size: 20)
        expect(element["height"]).to eq((20 * 1.25).round(2))
    ...
    it "ELEMENT_TYPES does not include sticky" do
      expect(Whiteboard::Canvas::ELEMENT_TYPES).not_to include("sticky")
    end

    it "validate_elements! raises on a sticky element" do
      elements = [ { "id" => "s1", "type" => "sticky" } ]
      expect {
        Whiteboard::Canvas.validate_elements!(elements)

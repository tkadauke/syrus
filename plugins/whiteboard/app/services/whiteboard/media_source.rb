module Whiteboard
  ...
  module MediaSource
    ...
    def self.chat_media_exists?(chat_session:, id:)
      Whiteboard::Snapshot.where(chat_session_id: chat_session.id).exists?(id)
    end

    def self.attach_chat_media(chat_session:, job:, ref:, id:)
      snapshot = Whiteboard::Snapshot.where(chat_session_id: chat_session.id).find_by(id: id)
    ...
    def self.list_chat_media(chat_session:)
      Whiteboard::Snapshot.where(chat_session_id: chat_session.id).limit(10).map do |snapshot|
        {
    ...
    def self.chat_media_panel(chat_session:)
      board = Whiteboard::Board.find_by(chat_session_id: chat_session.id)
      elements = board ? Array(board.scene_json&.dig("elements")).reject { |el| el["isDeleted"] } : []
      snapshots = Whiteboard::Snapshot.where(chat_session_id: chat_session.id).order(created_at: :desc)

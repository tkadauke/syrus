...
module Whiteboard
  class LoadCanvasTool < MCP::Tool
    ...
    input_schema(
      properties: {
        snapshot_id: { type: "integer", description: "ID of the Whiteboard::Snapshot to load." },
    ...
    class << self
      def call(snapshot_id:, server_context:, mode: "merge")
        ...
        snapshot = Whiteboard::Snapshot.for(chat_session).find(snapshot_id)
        snapshot_scene = Whiteboard::Board.normalize_scene!(snapshot.scene_json)
      ...
      def replace_canvas(chat_session, snapshot_scene)
        ...
        auto_saved_snapshot = if current_scene.fetch("elements").any?
          Whiteboard::Snapshot.create_from_scene!(
      ...
      def merge_canvas(chat_session, snapshot_scene)
        Canvas.mutate(chat_session, tool_name, { "mode" => "merge" }) do |elements, scene|
          ...
          if elements.size + snapshot_elements.size > Whiteboard::Board::MAX_ELEMENTS
            raise Canvas::ElementLimitExceeded, Whiteboard::Board.element_limit_message
          end

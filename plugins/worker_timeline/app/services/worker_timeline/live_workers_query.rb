module WorkerTimeline
  class LiveWorkersQuery
    ...
    def pools_for(worker_key, host_processes, slots)
      host_processes.map do |process|
        queues = queue_names(process.metadata&.dig("queues"))
        pool_slots = slots.select { |slot| slot_matches_pool?(slot, queues) }
        {
          key: "#{process.hostname}:#{process.pid}",
          hostname: process.hostname,
          pid: process.pid,
          queues: queues,
          capacity: pool_capacity(process),
          used: pool_slots.length,
          status: pool_status(used: pool_slots.length, capacity: pool_capacity(process)),
          last_heartbeat_at: process.last_heartbeat_at&.iso8601,
          slots: pool_slots
        }
      end.then do |pools|
        assigned_ids = pools.flat_map { |pool| pool[:slots].map { |slot| slot[:id] } }.to_set
        unassigned = slots.reject { |slot| assigned_ids.include?(slot[:id]) }
        next pools if unassigned.empty?
    ...
    def pool_capacity(process)
      value = process.metadata&.dig("thread_pool_size")
      Integer(value, exception: false).to_i.clamp(1, 10_000)
    end
    ...
    def slot_for_process(process)
      ...
      run = process.run
      step = run&.step || workflow&.steps&.detect(&:running?)
      job = workflow&.job || run&.job
      {
        ...
        confidence: "inferred",
        hostname: process.hostname,
        worker_storage_key: workflow&.worker_storage_key.presence || host_key_for(hostname: process.hostname),
        ...
        elapsed_seconds: elapsed_seconds(process.started_at),
        command: command_excerpt(process.redacted_command),
        command_excerpt: command_excerpt(process.redacted_command)
      }
    end
    ...
    def slot_for_run(run)
      ...
      job = run.job
      {
        ...
        worker_storage_key: workflow&.worker_storage_key.presence || host_key_for(hostname: workflow&.worker_hostname),
        queue_role: queue_role_for(workflow),
        job_id: job.id,
        job_slug: job.slug,
        job_title: job.title,
        ...
        elapsed_seconds: elapsed_seconds(run.started_at),
        command: nil,
        command_excerpt: nil
      }
    end
    ...
    def slot_for_step(step)
      ...
      job = workflow.job
      {
        ...
        elapsed_seconds: elapsed_seconds(step.started_at),
        command: nil,
        command_excerpt: nil
      }
    end
    ...
    def slot_for_workflow(workflow)
      job = workflow.job
      {
        ...
        elapsed_seconds: elapsed_seconds(workflow.started_at),
        command: nil,
        command_excerpt: nil
      }
    end
    ...
    def running_processes
      @running_processes ||= SpawnedProcess.running
        .includes(:workflow, run: [ :job, { step: :workflow } ])
    ...
    def instance_versions
      @instance_versions ||= InstanceVersion.fresh
        .where(role: "worker")
        .then { |scope| hostname_filter ? scope.where(hostname: hostname_filter) : scope }
        .order(:hostname)
        .to_a
    ...
    def sparklines_for(key)
      ...
      {
        cpu: sparkline_points(host_samples, :cpu_used_percent),
        memory: sparkline_points(host_samples, :memory_used_percent),
        io: sparkline_points(host_samples, :io_pressure_some)
      }
    end

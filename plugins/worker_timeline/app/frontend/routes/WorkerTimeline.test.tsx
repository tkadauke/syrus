...
function livePayload(overrides: Partial<WorkerTimelineLivePayload> = {}): WorkerTimelineLivePayload {
  return {
    ...
    range: { from: "2026-01-01T00:00:00Z", to: "2026-01-01T01:00:00Z" },
    filter: { and: [] },
    filter_schema: liveFilterSchema(),
    attribution: {
    ...
    workers: [
      {
        ...
        sparklines: {
          cpu: [ { observed_at: "2026-01-01T00:58:00Z", value: 30 }, { observed_at: "2026-01-01T00:59:00Z", value: 42 } ],
          memory: [ { observed_at: "2026-01-01T00:58:00Z", value: 60 }, { observed_at: "2026-01-01T00:59:00Z", value: 64 } ],
          io: [ { observed_at: "2026-01-01T00:58:00Z", value: 3 }, { observed_at: "2026-01-01T00:59:00Z", value: 5 } ]
        },
        pools: [
          {
            key: "worker-a:101",
...
describe("WorkerTimeline macro view", () => {
  ...
  it("switches to the scoped Live Workers tab without fetching the macro timeline again", async () => {
    const calls = setupFetchMock()
    renderTimeline()

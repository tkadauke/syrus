...
RSpec.describe "API: /api/v1/admin/worker_timeline", type: :request do
  ...
  describe "GET /live" do
    ...
    it "returns the live workers payload for an admin token" do
      enable_plugin!
      SolidQueue::Process.create!(kind: "Worker", name: "worker-101", hostname: "worker-a", pid: 101, last_heartbeat_at: Time.current, created_at: Time.current, metadata: { "queues" => "runs", "thread_pool_size" => 1 })
      WorkerHostHealthSample.create!(hostname: "worker-a", worker_storage_key: "storage-a", role: "worker", version: "abc123", observed_at: 1.minute.ago, cpu_used_percent: 10, memory_used_percent: 20, io_pressure_some: 0)

      get "/api/v1/admin/worker_timeline/live", headers: auth

      expect(response).to have_http_status(:ok)
      expect(response.parsed_body.fetch("workers").first).to include("hostname" => "worker-a", "worker_storage_key" => "storage-a", "status" => "idle")
      expect(response.parsed_body.dig("attribution", "exact_thread_ownership")).to be(false)
    end
  end
end

...
RSpec.describe "API: /api/v1/app/admin/worker_timeline", type: :request do
  ...
  describe "GET /live" do
    ...
    it "returns multi-slot hosts, idle hosts, health states, and redacted commands" do
      ...
      busy_workflow = Workflow.create!(
        job: busy_job, trigger_kind: "initial", state: "running",
        started_at: 12.minutes.ago, worker_hostname: "worker-a", worker_storage_key: "storage-a"
      )
      busy_step = busy_workflow.steps.create!(kind: "implement", position: 1, state: "running", started_at: 11.minutes.ago)
      busy_run = Run.create!(
        job: busy_job, user: admin, step: busy_step, trigger_kind: "initial", agent_provider: "codex",
      ...
      running_process = solid_queue_process(hostname: "worker-a", pid: 101, metadata: { "queues" => "runs,maintenance", "thread_pool_size" => 2 })
      idle_process = solid_queue_process(hostname: "worker-idle", pid: 102, metadata: { "queues" => [ "runs" ], "thread_pool_size" => 1 })
      overloaded_process = solid_queue_process(hostname: "worker-hot", pid: 103, metadata: { "queues" => "runs", "thread_pool_size" => 1 })

      2.times do |index|
        hot_job = Factories.job_record(user: admin, repository: repository, state: "running", issue_title: "Hot job #{index}")
        hot_workflow = Workflow.create!(
          job: hot_job, trigger_kind: "retry", state: "running",
          started_at: (8 - index).minutes.ago, worker_hostname: "worker-hot", worker_storage_key: "storage-hot"
        )
        hot_step = hot_workflow.steps.create!(kind: "implement", position: 1, state: "running", started_at: (7 - index).minutes.ago)
        Run.create!(
          job: hot_job, user: admin, step: hot_step, trigger_kind: "retry", agent_provider: "codex",
          state: "running", started_at: (6 - index).minutes.ago
        )
      end
      ...
      InstanceVersion.create!(hostname: "worker-a", role: "worker", version: "abc123", started_at: 1.hour.ago, last_heartbeat_at: 1.minute.ago)
      InstanceVersion.create!(hostname: "worker-idle", role: "worker", version: "abc123", started_at: 1.hour.ago, last_heartbeat_at: 1.minute.ago)
      InstanceVersion.create!(hostname: "worker-hot", role: "worker", version: "abc123", started_at: 1.hour.ago, last_heartbeat_at: 1.minute.ago)

      get "/api/v1/app/admin/worker_timeline/live"

      expect(response).to have_http_status(:ok)
      body = parse_body
      expect(body.fetch("filter_schema").map { |field| field.fetch("field") }).to eq(%w[ hostname status window ])
      expect(body.dig("attribution", "exact_thread_ownership")).to be(false)

      workers = body.fetch("workers").index_by { |worker| worker.fetch("hostname") }
      expect(workers.fetch("worker-a").dig("occupancy")).to eq("used" => 1, "total" => 2)
      expect(workers.fetch("worker-a").fetch("pools").first.fetch("queues")).to eq(%w[ runs maintenance ])
      expect(workers.fetch("worker-idle").fetch("status")).to eq("idle")
      expect(workers.fetch("worker-idle").dig("occupancy")).to eq("used" => 0, "total" => 1)
      ...
      expect(running_process.reload).to be_present
      expect(idle_process.reload).to be_present
      expect(overloaded_process.reload).to be_present
    end

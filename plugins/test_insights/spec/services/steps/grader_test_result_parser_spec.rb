...
RSpec.describe "Steps::Grader :test_result_parser plugin integration" do
  ...
  context "when a plugin parser matches" do
    it "uses the plugin parser's output instead of JUnit XML" do
      ...
      allow(plugin).to receive(:can_parse?).with(hash_including(output_path: output_path)).and_return(true)
      allow(plugin).to receive(:call).with(hash_including(output_path: output_path)).and_return(parsed)
      ...
      expect { handler.call rescue nil }
        ...
        .and change(TestInsights::TestCase, :count).by(1)

      tr = TestInsights::TestRun.find_by!(run: run, grader_name: "go-tests")
  ...
  context "when a plugin's can_parse? returns false" do
    it "falls through to JUnit XML parsing" do
      ...
      allow(plugin).to receive(:can_parse?).with(hash_including(output_path: output_path)).and_return(false)
      ...
      expect(TestInsights::TestRun.find_by!(run: run, grader_name: "go-tests").total_count).to eq(1)
    end
  end
  ...
  context "when a plugin's return value violates the TestResultParser contract" do
    it "logs the parser class name and the exception, and does not fail the grader" do
      ...
      allow(plugin).to receive(:can_parse?).with(hash_including(output_path: output_path)).and_return(true)
      allow(plugin).to receive(:call).with(hash_including(output_path: output_path)).and_return(bad_return_value)
      ...
      expect(TestInsights::TestRun.where(run: run, grader_name: "go-tests")).not_to exist
  ...
  context "when multiple plugins are registered" do
    it "uses the first matching plugin and skips the rest" do
      ...
      allow(plugin_a).to receive(:can_parse?).with(hash_including(output_path: output_path)).and_return(false)
      allow(plugin_b).to receive(:can_parse?).with(hash_including(output_path: output_path)).and_return(true)
      allow(plugin_b).to receive(:call).with(hash_including(output_path: output_path)).and_return(parsed)
      ...
      expect { handler.call rescue nil }.to change(TestInsights::TestRun, :count).by(1)
      expect(TestInsights::TestRun.find_by!(run: run, grader_name: "go-tests").total_count).to eq(5)
    end
  end
  ...
  context "when no plugins are registered" do
    it "falls back to JUnit XML parsing" do
      ...
      expect { handler.call }.to change(TestInsights::TestRun, :count).by(1)
      expect(TestInsights::TestRun.find_by!(run: run, grader_name: "go-tests").total_count).to eq(2)
    end
  end
end

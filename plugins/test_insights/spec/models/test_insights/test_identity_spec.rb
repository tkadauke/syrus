...
RSpec.describe TestInsights::TestIdentity do
  ...
  describe ".interesting_for_repository" do
    ...
    it "uses test identity summaries instead of scanning test cases" do
      ...
      expect(TestInsights::TestCase).not_to receive(:where)
  ...
  describe ".refresh_many!" do
    ...
    it "selects latest test cases without reserved window-function alias collisions" do
      ...
      subscriber = ActiveSupport::Notifications.subscribe("sql.active_record") do |_name, _started, _finished, _id, payload|
        sql = payload[:sql].to_s
        sql_statements << sql if sql.include?("test_insight_cases")
      end
    ...
    it "updates identity summaries in bulk" do
      ...
      subscriber = ActiveSupport::Notifications.subscribe("sql.active_record") do |_name, _started, _finished, _id, payload|
        sql = payload[:sql].to_s
        test_identity_updates << sql if sql.match?(/\AUPDATE [`"]?test_insight_identities[`"]?/i)
      end

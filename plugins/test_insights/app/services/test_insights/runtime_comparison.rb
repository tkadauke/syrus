module TestInsights
  class RuntimeComparison
    ...
    def candidate_ids_from_summary(scope, repository)
      grader_name = @grader_name || RuntimeSummary::ALL_GRADERS
      scope
        .joins(:runtime_summaries)
        .where(test_insight_runtime_summaries: {
          repository_id: repository.id,
          grader_name: grader_name,
          window: RuntimeSummary::RECENT_100_WINDOW
        })
        .order(
          Arel.sql("test_insight_runtime_summaries.p95_duration_ms DESC"),
          Arel.sql("test_insight_runtime_summaries.avg_duration_ms DESC"),
          Arel.sql("test_insight_identities.id DESC")
        )
        .limit(@limit)
        .to_a
    end
    ...
    class Source
      ...
      def base_case_scope(identity_ids, grader_name:)
        ...
        scope = scope.where(test_insight_runs: { grader_name: grader_name }) if grader_name.present?
    ...
    class RunSource < Source
      ...
      def duration_rows(identity_ids, grader_name:)
        rows_from(base_case_scope(identity_ids, grader_name: grader_name).where(test_insight_runs: { run_id: @run.id }))
      end

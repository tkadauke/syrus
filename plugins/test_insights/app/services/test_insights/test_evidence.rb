module TestInsights
  ...
  class TestEvidence
    ...
    # TestCase.flakiness_score, which already excludes self-repaired loop
    # failures (`.scored`) and can omit the workflow currently being
    # adjudicated -- this must not reimplement that filtering.
    def self.flakiness_score(repository:, suite_name:, name:, exclude_workflow: nil)
      return nil if repository.nil? || suite_name.blank? || name.blank?

      TestCase.flakiness_score(repository: repository, suite_name: suite_name, name: name, exclude_workflow: exclude_workflow)
    end

module TestInsights
  ...
  class TestEvidence
    ...
    # TestCase.flakiness_score, which already excludes in-loop repair
    # failures (`.scored`) and, when provided, the current workflow's own
    # attempts -- this must not reimplement that filtering.
    def self.flakiness_score(repository:, suite_name:, name:, workflow: nil)
      return nil if repository.nil? || suite_name.blank? || name.blank?

      TestCase.flakiness_score(repository: repository, suite_name: suite_name, name: name, exclude_workflow: workflow)
    end

  ...
  class Query
    ...
    def call
      ...
      Result.new(
        ...
        query: @query,
        limit: @limit,
        summary_window: RuntimeSummary::RECENT_100_WINDOW,
        tests: tests
      )
    end
    ...
    def apply_grader_filter(scope, repository)
      ...
      matching_identity_ids = TestCase
        .joins(:test_run)
        .where(repository_id: repository.id, test_insight_runs: { grader_name: @grader_name })
    ...
    def recent_stats_by_identity_id(identities)
      ...
      existing.merge(TestInsights::RecentStats.load(missing, lookback: @filters.lookback))
    end
    ...
    def summary_grader_name
      @grader_name || RuntimeSummary::ALL_GRADERS
    end
    ...
    class RuntimeSummarySort < Sort
      ...
      def apply(scope, direction)
        scope.order(
          ...
          Arel.sql("test_insight_identities.last_seen_at #{direction.name.upcase}"),
          Arel.sql("test_insight_identities.id #{direction.name.upcase}")
        )
      end
      ...
      def apply_runtime_summary_join(scope, grader_name)
        ...
        quoted_window = connection.quote(RuntimeSummary::RECENT_100_WINDOW)
        summary_selects = SUMMARY_COLUMNS.map do |column|
          "test_insight_runtime_summaries.#{column} AS runtime_#{column}"
        end

        scope
          .joins(<<~SQL.squish)
            INNER JOIN test_insight_runtime_summaries
              ON test_insight_runtime_summaries.test_identity_id = test_insight_identities.id
             AND test_insight_runtime_summaries.grader_name = #{quoted_grader}
             AND test_insight_runtime_summaries.window = #{quoted_window}
          SQL
          .select("test_insight_identities.*", "test_insight_runtime_summaries.#{@column} AS runtime_sort_value", *summary_selects)
      end
    end

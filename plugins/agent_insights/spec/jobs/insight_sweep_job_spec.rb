...
RSpec.describe InsightSweepJob do
  ...
  context "when agent_insights is enabled" do
    ...
    it "skips repos whose AgentInsights::ScheduleConfig is disabled" do
      AgentInsights::ScheduleConfig.create!(repository: repository, enabled: false, min_jobs_since_last_run: 1, max_jobs_since_last_run: 5)
      3.times { closed_coding_job(repository) }
    ...
    it "processes multiple repositories independently" do
      ...
      expect(AgentInsights::Scheduler).to have_received(:enqueue_if_idle!).with(repository)
      expect(AgentInsights::Scheduler).not_to have_received(:enqueue_if_idle!).with(repo2)
    end
    ...
    it "isolates one repo's failure from the rest" do
      ...
      call_count = 0
      allow(AgentInsights::Scheduler).to receive(:enqueue_if_idle!) do |repo|
        call_count += 1

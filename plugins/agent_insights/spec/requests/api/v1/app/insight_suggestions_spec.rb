...
RSpec.describe "App API insight suggestions", type: :request do
  ...
  def disable_feature
    PluginRecord.find_or_create_by!(name: "agent_insights").update!(enabled: false, disableable: true)
  end

  describe "feature flag off" do
    before { disable_feature }

    # Authentication runs before the plugin-disabled check, so an anonymous
    # request to a disabled plugin's route is a 401 rather than a 404. That is
    # the host dispatcher's ordering, not this plugin's business.
    it "still requires authentication when the plugin is disabled" do
  ...
  describe "GET /api/v1/app/repositories/:repository_id/insight_suggestions" do
    ...
    it "returns tabs including the plugin insights tab" do
      get "/api/v1/app/repositories/#{repository.id}/insight_suggestions"

      expect(response).to have_http_status(:ok)
      tab_keys = parse_body["tabs"].map { |t| t["key"] }
      expect(tab_keys).to include("overview", "github_source.issues", "documents", "scheduled_tasks.repository", "agent_insights.repository")
    end
    ...
    it "filters suggestions by created_at through the shared filter query" do
      old = create_suggestion(title: "Old")
      old.update_column(:created_at, Time.zone.parse("2026-06-01 12:00:00"))
      fresh = create_suggestion(title: "Fresh")
      fresh.update_column(:created_at, Time.zone.parse("2026-07-15 12:00:00"))
      ...
      body = parse_body
      expect(body["suggestions"].map { |s| s["id"] }).to eq([ fresh.id ])
      expect(body.dig("meta", "counts")).to include("pending" => 1, "all" => 1)
    ...
    it "combines an active smart folder with additional filters and returns filter-aware counts" do
      ...
      expect(body["smart_folders"].find { |folder| folder["name"] == "Accepted" }["count"]).to eq(1)
      expect(body["suggestions"].map { |s| s["id"] }).not_to include(accepted_low.id, pending_high.id)
    end
  ...
  describe "PATCH /api/v1/app/insight_suggestions/:id — dismiss" do
    ...
    it "returns 404 for a suggestion belonging to another user" do
      ...
      other_suggestion = AgentInsights::Suggestion.create!(

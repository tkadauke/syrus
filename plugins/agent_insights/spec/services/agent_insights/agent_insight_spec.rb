...
RSpec.describe AgentInsights::Prompt do
  ...
  describe "known insights" do
    context "when known_insights are provided" do
      let(:known_insights) do
        [
          instance_double(AgentInsights::Suggestion, id: 101, title: "Agent repeatedly fails at rebase step", state: "accepted", effective_proposal_type: "create_job"),
          instance_double(AgentInsights::Suggestion, id: 102, title: "Missing memory about test runner config", state: "dismissed", effective_proposal_type: "save_memory")
        ]
      end

...
RSpec.describe AgentInsights::Tools::SubmitInsightTool do
  ...
  describe "successful submission" do
    it "creates an AgentInsights::Suggestion record" do
      expect { call }.to change(AgentInsights::Suggestion, :count).by(1)
    end
    ...
    it "redacts GitHub credentials before persisting operator-facing insight copy" do
      ...
      serialized = JSON.generate(AgentInsights::Suggestion.last.attributes)
    ...
    it "returns a success response with the suggestion id" do
      ...
      expect(response.content.first[:text]).to match(/Suggestion #\d+ saved\./)
    end
    ...
    it "accepts evidence referencing jobs owned by the user" do
      ...
      expect(response).not_to be_error
      expect(AgentInsights::Suggestion.last.evidence).to be_an(Array)
    end
    ...
    it "stores valid job-only evidence" do
      ...
      expect(response).not_to be_error
      expect(AgentInsights::Suggestion.last.evidence).to eq([
  ...
  describe "feature flag gating" do
    it "is withdrawn when the plugin is disabled" do
      ...
      expect(Syrus::PluginRegistry.providers_for(:mcp_tool_set)).not_to include(AgentInsights::McpToolSet)
    end

    it "is advertised by the plugin tool set while the plugin is enabled" do
      context = McpToolContext.from_run(run)
      expect(AgentInsights::McpToolSet::TOOL_CLASSES).to include(described_class)
    end
  end

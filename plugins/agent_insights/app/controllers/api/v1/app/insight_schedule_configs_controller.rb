    ...
    module App
      class InsightScheduleConfigsController < BaseController
        ...
        def require_agent_insights_feature
          render_error("agent_insights_disabled", "Agent Insights is not enabled.", status: :forbidden) unless AgentInsights.enabled?
        end
        ...
        def config_json(config)
          if config
            ...
            {
              enabled: false,
              min_jobs_since_last_run: AgentInsights::ScheduleConfig.column_defaults["min_jobs_since_last_run"] || 5,
              max_jobs_since_last_run: AgentInsights::ScheduleConfig.column_defaults["max_jobs_since_last_run"] || 10
            }
          end
        end
      end

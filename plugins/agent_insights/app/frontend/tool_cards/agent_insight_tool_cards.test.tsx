...
describe("Agent Insights tool cards", () => {
  ...
  it("renders pending, accepted, and retired insights with metadata and source links", () => {
    const insights = [
      baseInsight,
  ...
  it("renders read_insight details with evidence, recommended action, links, and retirement outcome", () => {
    ...
    const cardContext = context({
      ...
      parsedResult: {
        insight: {
          ...baseInsight,
          evidence: [
            { job_id: 5, run_id: 7, kind: "prepare_failure", job_path: "/jobs/5", run_transcript_path: "/admin/runs/7/transcript" }
          ],
    ...
    expect(screen.getByRole("link", { name: "JOB-5" })).toHaveAttribute("href", "/jobs/5")
    expect(screen.getByRole("link", { name: "RUN-7" })).toHaveAttribute("href", "/admin/runs/7/transcript")
  })
  ...
  it("renders retire_insight outcome details from the text response and call input", () => {
    ...
    expect(terms.map((term) => term.textContent)).toContain("Superseding insight")

...
module AgentInsights
  module Tools
    ...
    class UpdateInsightTool < MCP::Tool
      ...
      description <<~DESC
        Updates an existing unaccepted Suggestion in place. Use this when
      ...
      input_schema(
        properties: {
          target_insight_id: {
            type: "integer",
            description: "Required Suggestion id to update."
          },
          reason: {
            type: "string",
      ...
      class << self
        ...
        def call(target_insight_id:, reason:, server_context:,
          ...
          if insight.accepted?
            return Mcp::Tools.invalid(
              "Suggestion ##{insight.id} is accepted and cannot be updated. Submit a new insight instead and cite the accepted insight as context."
            )
          end
          ...
          MCP::Tool::Response.new([
            { type: "text", text: "Suggestion ##{insight.id} updated." }
          ])
        ...
        def visible_insight(insight_id, run)
          ...
          Suggestion.for_repository(run.job.repository).find_by(id: id)
        end

...
RSpec.describe Ruby::FocusedTestCommand do
  it "synthesizes a focused RSpec command when the grader opts into plugin strategy" do
    ...
    expect(command).to eq('export TEST_ENV_NUMBER="${TEST_ENV_NUMBER:-_focused_$$}"; if [ -x bin/rails ] && [ -f config/database.yml ]; then bin/rails db:test:prepare; fi && bundle exec rspec spec/models/widget_spec.rb')
  end
  ...
  it "isolates and prepares the Rails test database before running focused specs" do
    ...
    expect(command).to start_with('export TEST_ENV_NUMBER="${TEST_ENV_NUMBER:-_focused_$$}"; if [ -x bin/rails ] && [ -f config/database.yml ]; then bin/rails db:test:prepare; fi && ')
  end

...
RSpec.describe Ruby::FocusedTestCommand do
  it "synthesizes a focused RSpec command when the grader opts into plugin strategy" do
    ...
    expect(command).to eq("RAILS_ENV=test COVERAGE=false bundle exec rspec spec/models/widget_spec.rb")
  end
  ...
  it "forces test env defaults so workflow repeat checks do not inherit development Rails env" do
    ...
    expect(command).to start_with("RAILS_ENV=test COVERAGE=false bundle exec rspec ")
  end

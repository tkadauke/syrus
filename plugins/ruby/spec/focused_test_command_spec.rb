...
RSpec.describe Ruby::FocusedTestCommand do
  ...
  it "preserves ci_only inclusion for ci-only RSpec graders" do
    ...
    expect(command).to eq("RAILS_ENV=test RUN_CI_ONLY_SPECS=true COVERAGE=false bundle exec rspec --tag ci_only spec/migrations/widget_spec.rb")
  end
  ...
  it "preserves RSpec tag filters passed through RSPEC_TAG_ARGS" do
    ...
    expect(command).to eq("RAILS_ENV=test RUN_CI_ONLY_SPECS=false COVERAGE=false bundle exec rspec --tag \\~ci_only plugins/muse_agent/spec/services/muse_invocation_spec.rb")
  end

...
RSpec.describe Ruby::FocusedTestCommand do
  ...
  it "forces test env defaults so workflow repeat checks do not inherit development Rails env" do
    ...
    expect(command).to start_with("if [ -x bin/rails ] && [ -f config/database.yml ]; then RAILS_ENV=test bin/rails db:test:prepare; fi && RAILS_ENV=test COVERAGE=false bundle exec rspec ")
  end

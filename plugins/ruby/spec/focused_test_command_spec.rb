...
RSpec.describe Ruby::FocusedTestCommand do
  ...
  it "synthesizes a focused RSpec command when the grader opts into plugin strategy" do
    ...
    expect(command).to eq("#{setup_prefix}RAILS_ENV=${RAILS_ENV:-test} RUN_CI_ONLY_SPECS=false COVERAGE=false bundle exec rspec --tag ~ci_only spec/models/widget_spec.rb")
  end
  ...
  it "preserves ci_only inclusion for ci-only RSpec graders" do
    ...
    expect(command).to eq("#{setup_prefix}RAILS_ENV=${RAILS_ENV:-test} RUN_CI_ONLY_SPECS=true COVERAGE=false bundle exec rspec --tag ci_only spec/migrations/widget_spec.rb")
  end
  ...
  it "preserves RSpec tag filters passed through RSPEC_TAG_ARGS" do
    ...
    expect(command).to eq("#{setup_prefix}RAILS_ENV=${RAILS_ENV:-test} RUN_CI_ONLY_SPECS=false COVERAGE=false bundle exec rspec --tag \\~ci_only plugins/muse_agent/spec/services/muse_invocation_spec.rb")
  end

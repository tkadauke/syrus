...
RSpec.describe Ruby::FocusedTestCommand do
  it "synthesizes a focused RSpec command when the grader opts into plugin strategy" do
    ...
    expect(command).to eq("if [ -x bin/rails ] && [ -f config/database.yml ]; then bin/rails db:test:prepare; fi && bundle exec rspec spec/models/widget_spec.rb")
  end

  it "prepares the Rails test database before running focused specs" do
    command = described_class.command_for(
      grader_name: "rspec",
      grader_command: "bin/rspec",
    ...
    expect(command).to start_with("if [ -x bin/rails ] && [ -f config/database.yml ]; then bin/rails db:test:prepare; fi && ")
  end

...
module Ruby
  class FocusedTestCommand
    ...
    def command
      ...
      "#{rails_prepare_command} RAILS_ENV=${RAILS_ENV:-test} COVERAGE=false #{rspec_command} #{Shellwords.join(files)}"
    end
    ...
    def rails_prepare_command
      "if [ -x bin/rails ] && [ -f config/database.yml ] && [ ! -f .syrus/flaky-gate-db-prepared ]; then mkdir -p .syrus && bin/rails db:test:prepare && touch .syrus/flaky-gate-db-prepared; fi &&"
    end

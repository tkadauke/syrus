...
module BuildCache
  class ClearRequest < ApplicationRecord
    self.table_name = "build_cache_clear_requests"
    ...
    def confirm!(user:)
      ...
      return false unless BuildCache::Client.configured?
    ...
    def bucket_configured
      errors.add(:base, "build cache bucket is not configured") unless BuildCache::Client.configured?
    end

    def no_other_pending_request
      errors.add(:base, "another clear request is already pending") if BuildCache::ClearRequest.pending.exists?
    end
  end
end

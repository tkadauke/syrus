...
RSpec.describe ScheduledTasks::Prompt do
  ...
  describe "footer" do
    ...
    it "shows the last PR number and 'open' state when the job is open" do
      task.jobs.create!(
        user: user, repository: repository,
        kind: "cron", scheduled_task_id: task.id, issue_number: nil, pr_number: 42
      )
      output = described_class.new(scheduled_task: task, fired_at: fired_at).to_s
    ...
    it "shows 'closed' when the last PR job was closed with pr_closed" do
      job = task.jobs.create!(
        user: user, repository: repository,
        kind: "cron", scheduled_task_id: task.id, issue_number: nil, pr_number: 17
      )
    ...
    it "picks the newest PR job when multiple exist" do
      task.jobs.create!(
        user: user, repository: repository,
        kind: "cron", scheduled_task_id: task.id, issue_number: nil, pr_number: 10
      ).tap { |j| j.close_with_reason!("pr_merged") }
      task.jobs.create!(
        user: user, repository: repository,
        kind: "cron", scheduled_task_id: task.id, issue_number: nil, pr_number: 20
      )
      output = described_class.new(scheduled_task: task, fired_at: fired_at).to_s
  ...
  describe "{{last_pr_number}} template variable" do
    ...
    it "renders the PR number when a PR job exists" do
      task.jobs.create!(
        user: user, repository: repository,
        kind: "cron", scheduled_task_id: task.id, issue_number: nil, pr_number: 55
      )
      task.update!(prompt: "Last PR: {{last_pr_number}}.")
      output = described_class.new(scheduled_task: task, fired_at: fired_at).to_s

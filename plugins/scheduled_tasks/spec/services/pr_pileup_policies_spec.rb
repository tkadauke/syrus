...
RSpec.describe ScheduledTasks::PrPileupPolicies do
  describe ".for" do
    it "returns SkipPolicy for 'skip'" do
      expect(described_class.for("skip")).to eq(ScheduledTasks::PrPileupPolicies::SkipPolicy)
    end

    it "returns ReplacePolicy for 'replace'" do
      expect(described_class.for("replace")).to eq(ScheduledTasks::PrPileupPolicies::ReplacePolicy)
    end

    it "returns PilePolicy for 'pile'" do
      expect(described_class.for("pile")).to eq(ScheduledTasks::PrPileupPolicies::PilePolicy)
    end

    it "raises ConfigurationError for unknown policy names" do
      expect { described_class.for("queue") }
        .to raise_error(ScheduledTasks::PrPileupPolicies::ConfigurationError, /Unknown pr_pileup_policy/)
    end
  end

  describe ScheduledTasks::PrPileupPolicies::SkipPolicy do
    ...
    let(:fire_service) { instance_double(ScheduledTasks::Fire, now: Time.current) }
  ...
  describe ScheduledTasks::PrPileupPolicies::ReplacePolicy do
    it "delegates to fire_service.close_prior_open_prs and returns nil" do
      fire_service = instance_double(ScheduledTasks::Fire, close_prior_open_prs: nil)
      task = instance_double(ScheduledTasks::Task)
      policy = described_class.new(task, fire_service: fire_service)
  ...
  describe ScheduledTasks::PrPileupPolicies::PilePolicy do
    it "always returns nil (fire proceeds)" do
      fire_service = instance_double(ScheduledTasks::Fire)
      task = instance_double(ScheduledTasks::Task)
      policy = described_class.new(task, fire_service: fire_service)

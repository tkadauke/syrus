...
RSpec.describe ScheduledTasks::Fire do
  ...
  def make_open_pr_job(pr_number: 99)
    task.jobs.create!(
      user: user, repository: repository,
      kind: "cron", scheduled_task_id: task.id,
  ...
  describe "#call" do
    it "spawns a cron Job with an explicit initial Run carrying the rendered prompt" do
      ...
      expect([ result.job.origin, result.job.origin_id ]).to eq([ "scheduled_tasks", task.id.to_s ])
    ...
    it "does not spawn a second Job in the same hourly fire window" do
      ...
      first_copy = ScheduledTasks::Task.find(task.id)
      second_copy = ScheduledTasks::Task.find(task.id)
    ...
    context "skill tasks" do
      ...
      it "spawns a cron Job carrying skill_name/skill_args and dispatches a Workflows::Skill" do
        ...
        expect([ job.origin, job.origin_id ]).to eq([ "scheduled_tasks", task.id.to_s ])

...
RSpec.describe ScheduledTasks::Schedules::CadencePreview do
  ...
  it "does not consult the LLM when a cron-shaped expression is deterministically invalid" do
    expect(ScheduledTasks::Schedules::CadenceLlmFallback).not_to receive(:call)
  ...
  it "returns the original deterministic error when the LLM fallback is unusable" do
    allow(ScheduledTasks::Schedules::CadenceLlmFallback).to receive(:call)
      .and_return(ScheduledTasks::Schedules::CadenceLlmFallback::Result.new(usable?: false, structured_intent: nil, error: "nope"))
  ...
  it "fails closed when the LLM produces malformed structured intent instead of saving a bogus schedule" do
    allow(ScheduledTasks::Schedules::CadenceLlmFallback).to receive(:call)
      .and_return(ScheduledTasks::Schedules::CadenceLlmFallback::Result.new(usable?: true, structured_intent: { frequency: "weekly" }, error: nil))

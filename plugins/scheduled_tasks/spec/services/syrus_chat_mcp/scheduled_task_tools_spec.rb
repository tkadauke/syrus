...
RSpec.describe "Mcp::Tools scheduled task tools" do
  ...
  def server
    MCP::Server.new(
      ...
      tools: [
        ScheduledTasks::McpTools::ListScheduledTasksTool,
        ScheduledTasks::McpTools::ReadScheduledTaskTool,
        ScheduledTasks::McpTools::UpdateScheduledTaskTool,
        ScheduledTasks::McpTools::PauseScheduledTaskTool,
        ScheduledTasks::McpTools::ResumeScheduledTaskTool,
        ScheduledTasks::McpTools::DeleteScheduledTaskTool
      ],
  ...
  def create_scheduled_task(attrs = {})
    ...
    ScheduledTasks::Task.create!({ repository: task_repository,
      user: user,
      kind: "cron",
      name: "Daily review",
  ...
  it "lists non-archived scheduled tasks across the user's repositories" do
    ...
    ScheduledTasks::Task.create!(repository: outsider_repository,
  ...
  it "does not list scheduled tasks owned by another user" do
    ...
    ScheduledTasks::Task.create!(repository: other_repository,
  ...
  it "rejects scheduled tasks owned by another user" do
    other_repository = Factories.repository
    other_task = ScheduledTasks::Task.create!(repository: other_repository,
      user: other_repository.user,
      kind: "cron",
      name: "Other repo",
      prompt: "Ignore this.",
  ...
  it "allows admin to read any scheduled task" do
    ...
    admin_server = MCP::Server.new(
      name: "syrus-chat-sidecar",
      tools: [ ScheduledTasks::McpTools::ReadScheduledTaskTool ],
  ...
  it "allows admin to update any scheduled task" do
    ...
    admin_server = MCP::Server.new(
      name: "syrus-chat-sidecar",
      tools: [ ScheduledTasks::McpTools::UpdateScheduledTaskTool ],
      server_context: { chat_session: admin_session }
    )
    raw = admin_server.handle_json({
      jsonrpc: "2.0", id: 1, method: "tools/call",

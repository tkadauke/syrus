...
RSpec.describe ScheduledTasks::CronTemplate do
  ...
  describe "validation" do
    ...
    it "accepts all valid pileup policies" do
      ScheduledTasks::CronTemplate::PR_PILEUP_POLICIES.each do |policy|
  ...
  describe "associations" do
    it "has many scheduled_tasks that nullify on destroy" do
      ...
      task = ScheduledTasks::Task.create!(repository: repo, user: user, name: "T", kind: "cron",
  ...
  describe ".seed_defaults_for" do
    ...
    it "creates the default templates for the given user" do
      ...
      }.to change { ScheduledTasks::CronTemplate.where(user_id: user.id).count }.by(described_class::DEFAULT_TEMPLATES.size)

      expect(ScheduledTasks::CronTemplate.where(user_id: user.id).pluck(:name)).to contain_exactly(
    ...
    it "seeds templates that are individually valid" do
      ...
      expect(ScheduledTasks::CronTemplate.where(user_id: user.id)).to all(be_valid)
    end
  end
end

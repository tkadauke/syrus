...
RSpec.describe "API: /api/v1/app/cron_templates", type: :request do
  ...
  it "lists only the current user's cron templates" do
    ...
    ScheduledTasks::CronTemplate.create!(valid_attrs.merge(name: "Their template", user: other_user))
    ...
    expect(parse_body["pr_pileup_policies"]).to eq(ScheduledTasks::CronTemplate::PR_PILEUP_POLICIES)
  ...
  it "shows a template with active repositories and applied tasks" do
    ...
    expect(body["repositories"]).to contain_exactly(
      include(
        "id" => repository.id,
        "slug" => "acme/widgets",
        "new_scheduled_task_path" => "/repositories/#{repository.id}/scheduled_tasks/new?from_template=#{template.id}"
      )
    )
  ...
  it "creates a template" do
    ...
    }.to change { ScheduledTasks::CronTemplate.where(user_id: user.id).count }.by(1)

    expect(response).to have_http_status(:created)
    template = ScheduledTasks::CronTemplate.where(user_id: user.id).last
  ...
  it "returns validation errors" do
    ...
    }.not_to change { ScheduledTasks::CronTemplate.where(user_id: user.id).count }
  ...
  it "does not allow managing another user's template" do
    sign_in_as(user)
    template = ScheduledTasks::CronTemplate.create!(valid_attrs.merge(user: other_user))

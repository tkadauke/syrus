module ScheduledTasks
  # Fires one schedule: renders its prompt and creates the Job.
  class Fire
    ...
    def call
      @task.with_lock do
        ...
        policy = ScheduledTasks::PrPileupPolicies.for(@task.pr_pileup_policy).new(@task, fire_service: self)
    ...
    def fire_freeform_job!
      prompt_renderer = ScheduledTasks::Prompt.new(scheduled_task: @task, fired_at: @now)
      ...
      job = Job.create!(
        ...
        origin: "scheduled_tasks",
        origin_id: @task.id.to_s,
      ...
      WorkUnits::Launcher.start!(workflow, prompt: rendered_prompt)
      job
    end
    ...
    def fire_skill_job!
      ...
      WorkUnits::Launcher.start!(workflow, prompt: nil)
      job
    end
    ...
    def close_prior_open_prs
      @task.open_pr_jobs.find_each do |old_job|
        Rails.logger.info("[ScheduledTasks::Fire] task ##{@task.id} closing prior PR ##{old_job.pr_number} (#{old_job.slug}) per replace policy")
        begin
          GithubClient.for(repository: @task.repository, user: @task.user).close_pull_request(@task.repository.slug, old_job.pr_number)
        rescue StandardError => e
          Rails.logger.warn("[ScheduledTasks::Fire] failed to close PR ##{old_job.pr_number}: #{e.class}: #{e.message}")
        end

...
describe("ScheduledTaskFormRoute cadence preview", () => {
  ...
  it("previews the pre-filled placeholder cadence successfully on load", async () => {
    ...
    expect(promptRadio.closest("label")).toHaveClass("w-auto")
    expect(promptRadio.closest("label")).not.toHaveClass("w-full")
    expect(skillRadio.closest("label")).toHaveClass("w-auto")
    expect(skillRadio.closest("label")).not.toHaveClass("w-full")
    await waitFor(() => {

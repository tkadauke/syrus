    ...
    module App
      class ScheduledTasksController < BaseController
        ...
        def preview_schedule
          result = ScheduledTasks::Schedules::CadencePreview.call(
        ...
        def create
          repository = find_repository
          task = ScheduledTasks::Task.new(scheduled_task_params.merge(repository: repository))
          ...
          if params[:from_template].present?
            template = ScheduledTasks::CronTemplate.where(user_id: Current.user.id).find_by(id: params[:from_template])
        ...
        def scheduled_tasks_index_payload
          tasks = ScheduledTasks::Task.alive.where(user: Current.user).includes(:repository)
          {
            ...
            archived_tasks: ScheduledTasks::Task.archived.where(user: Current.user).includes(:repository).order(archived_at: :desc).limit(20).map { |task| task_summary_json(task) },
        ...
        def repository_scheduled_tasks_payload(repository)
...

          {
            ...
            new_scheduled_task_path: "/repositories/#{repository.id}/scheduled_tasks/new",
            options: scheduled_task_options
          }
        end
        ...
        def cron_template_json(template)
          {
            ...
            cron_template_path: "/cron_templates/#{template.id}"
          }
        end
        ...
        def scheduled_task_options
          {
            kinds: ScheduledTasks::Task::KINDS,
            pr_pileup_policies: ScheduledTasks::Task::PR_PILEUP_POLICIES,
        ...
        def find_task
          ScheduledTasks::Task.where(user: Current.user).find(params[:id])
        end

        def find_repository_task(repository)
          ScheduledTasks::Task.where(repository_id: repository.id).find(params[:id])
        end

module ScheduledTasks
  # A reusable prompt + schedule an operator can apply to several Tasks.
  # Applying copies the values; later template edits do not rewrite the Tasks
  # that already used it.
  class CronTemplate < ApplicationRecord
    self.table_name = "scheduled_task_cron_templates"
    ...
    belongs_to :user
    has_many :scheduled_tasks, class_name: "ScheduledTasks::Task",
             foreign_key: :cron_template_id, inverse_of: :cron_template, dependent: :nullify
    ...
    def self.seed_defaults_for(user)
      DEFAULT_TEMPLATES.each do |attrs|
        where(user_id: user.id).find_or_create_by!(name: attrs.fetch(:name)) do |template|

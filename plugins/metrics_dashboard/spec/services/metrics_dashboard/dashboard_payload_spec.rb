...
RSpec.describe MetricsDashboard::DashboardPayload do
  ...
  describe "gauge panels" do
    ...
    it "keeps retained worker-storage-key samples separate until retention prunes them" do
      now = Time.current.change(sec: 0)
      sample(metric: "syrus_worker_disk_percent", labels: { "worker_storage_key" => "storage-a" }, value: 55, at: now - 10.minutes)
      sample(metric: "syrus_worker_disk_percent", labels: { "worker_storage_key" => "storage-b" }, value: 63, at: now - 1.minute)

      series = panel(described_class.build(window: "6h"), "worker_disk")[:series]

      expect(series.map { |entry| entry[:key] }).to contain_exactly("storage-a", "storage-b")
      expect(series.map { |entry| entry[:name] }).to contain_exactly("storage-a", "storage-b")
    end
  end

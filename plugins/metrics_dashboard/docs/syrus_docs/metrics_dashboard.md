...
Samples recorded before this label existed are intentionally not backfilled;
retained `worker_storage_key` rows and older hostname-only rows continue to
render as separate historical series until the sample-retention window prunes
them.

module WhiteboardTools
  ...
  module PayloadContributor
    ...
    def self.chat_payload_counts(chat_session:)
      { whiteboard_snapshot_count: WhiteboardTools::Snapshot.where(chat_session_id: chat_session.id).count }
    end

    def self.scene_for(chat_session, include_scene:)
      return WhiteboardTools::Board.default_state.merge("loaded" => false) unless include_scene

      row = payload_scope(chat_session.id).pick(:scene_json, :version)
      return WhiteboardTools::Board.default_state.merge("loaded" => true) unless row

      scene_json, version = row
      WhiteboardTools::Board.normalize_scene!(scene_json).merge("version" => version, "loaded" => true)
    end

    # MySQL picks a worse plan without the hint on chats with many rows.
    def self.payload_scope(chat_session_id)
      scope = WhiteboardTools::Board.where(chat_session_id: chat_session_id)
      return scope unless ActiveRecord::Base.connection.adapter_name.downcase.include?("mysql")

      scope.from(Arel.sql("#{WhiteboardTools::Board.quoted_table_name} FORCE INDEX (index_whiteboard_tools_boards_on_chat_session_id)"))
    end
  end
end

module WhiteboardTools
  class Board < ApplicationRecord
    self.table_name = "whiteboard_tools_boards"

    MAX_ELEMENTS = 1000
    ELEMENT_LIMIT_MESSAGE = "WhiteboardTools::Board at element limit (1000). Operator must clear or remove some shapes before adding more."
    ...
    # ChatSession used to carry `has_one :whiteboard`, injected onto the core
    # model by this plugin. The query lives here now instead.
    def self.for(chat_session) = find_by(chat_session_id: chat_session.id)
    def self.for!(chat_session) = find_or_create_by!(chat_session_id: chat_session.id)

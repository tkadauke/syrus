  ...
  class WorkspaceCandidates
    ...
    def workflow_candidates
      user.workflows
        ...
        .limit(80)
        .map { |workflow| WorkflowCandidate.new(workflow) }
    end
    ...
    class WorkflowCandidate < Candidate
      ...
      def queue_name
        ...
        @queue_name = @workflow.runs.order(created_at: :desc).first&.resume_worker_queue
      end
    ...
    class ChatCandidate < Candidate
      ...
      def working_directory = repository ? ChatWorkspace.repo_path_for(@chat, repository).to_s : ChatWorkspace.path_for(@chat).to_s
      def state = @chat.mode
      def queue_name = "chat"

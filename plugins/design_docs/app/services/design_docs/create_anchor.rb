  ...
  class CreateAnchor
    ...
    def call
      ...
      anchor = design_doc.anchors.create!(
        ...
        selected_text: selected_text(inserted),
    ...
    def resolve_range!(anchor_kind)
      ...
      expected = expected_markdown_selection
    ...
    def expected_markdown_selection
      attributes[:selected_markdown].presence || attributes[:selected_text].presence
    end

    def selected_text(inserted)
      provided = attributes[:selected_text].presence
      return provided if provided.present? && rendered_text(inserted.selected_markdown) == provided

      inserted.selected_markdown
    end

    def rendered_text(markdown)
      markdown.to_s
        .gsub(/\[([^\]]+)\]\([^)]+\)/, "\\1")
        .gsub(/`([^`]+)`/, "\\1")
        .gsub(/\*\*([^*]+)\*\*/, "\\1")

...
function DesignDocsIndexTable({ controls, docs, loading, onSelect, preferences }: {
  ...
  return (
    <section className="min-w-0 space-y-3" aria-label={t("index_table_label")}>
      {isDesktop ? (
                  ...
                  {menuColumns.map((column) => {
                    ...
                    return (
                      <div className="grid grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-2 text-sm text-gray-700 dark:text-gray-200" key={column.key}>
                        ...
                        <button aria-label={t("columns_move_left", { title: column.title })} className="rounded px-1 text-xs text-gray-500 hover:bg-gray-100 disabled:text-gray-300 dark:text-gray-400 dark:hover:bg-gray-800" disabled={!checked || updatePreferences.isPending} onClick={() => moveColumn(column.key, -1)} type="button">{t("columns_move_earlier")}</button>
                        <button aria-label={t("columns_move_right", { title: column.title })} className="rounded px-1 text-xs text-gray-500 hover:bg-gray-100 disabled:text-gray-300 dark:text-gray-400 dark:hover:bg-gray-800" disabled={!checked || updatePreferences.isPending} onClick={() => moveColumn(column.key, 1)} type="button">{t("columns_move_later")}</button>
                      </div>
                    )
                  })}
                </fieldset>
                {updatePreferences.isError ? <p className="mt-2 text-xs text-red-700 dark:text-red-300" role="alert">{errorMessage(updatePreferences.error, t("columns_update_error"))}</p> : null}
              </div>
            ) : null}
          </div>
        </div>

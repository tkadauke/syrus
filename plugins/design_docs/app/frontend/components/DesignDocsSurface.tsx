...
type DesignDocsTranslate = (key: string, options?: Record<string, string | number>) => string
...
function DesignDocsIndexTable({ controls, docs, loading, onSelect, preferences }: {
  ...
  const requiredColumns = controls?.columns.required ?? [{ key: "title", title: t("column_title") }]
                  ...
                  {menuColumns.map((column) => {
                    ...
                    return (
                      <div className="grid grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-2 text-sm text-gray-700 dark:text-gray-200" key={column.key}>
                        ...
                        <button aria-label={t("move_column_up", { title: column.title })} className="rounded px-1 text-xs text-gray-500 hover:bg-gray-100 disabled:text-gray-300 dark:text-gray-400 dark:hover:bg-gray-800" disabled={!checked || updatePreferences.isPending} onClick={() => moveColumn(column.key, -1)} type="button">{t("move_up")}</button>
                        <button aria-label={t("move_column_down", { title: column.title })} className="rounded px-1 text-xs text-gray-500 hover:bg-gray-100 disabled:text-gray-300 dark:text-gray-400 dark:hover:bg-gray-800" disabled={!checked || updatePreferences.isPending} onClick={() => moveColumn(column.key, 1)} type="button">{t("move_down")}</button>
                      </div>
                    )
                  })}
                </fieldset>
                {updatePreferences.isError ? <p className="mt-2 text-xs text-red-700 dark:text-red-300" role="alert">{errorMessage(updatePreferences.error, t("error_update_columns"))}</p> : null}
              </div>
            ) : null}
          </div>
        </div>
        ...
        <div className="overflow-hidden rounded border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900" data-testid="design-docs-mobile-list">
          {docs.map((doc) => (
            ...
            >
              ...
              <p className="mt-2 truncate text-xs text-gray-500 dark:text-gray-400">{repositoryLabel(doc, t)}</p>
              <p className="mt-1 truncate text-xs text-gray-500 dark:text-gray-400">{doc.owner?.name ?? t("unassigned")}</p>
            </button>
          ))}
        </div>
      )}
...
function DesignDocTableCell({ column, doc, onSelect, t }: { column: string; doc: DesignDocSummary; onSelect: (id: number) => void; t: DesignDocsTranslate }) {
  return (
...
function designDocCellContent(column: string, doc: DesignDocSummary, onSelect: (id: number) => void, t: DesignDocsTranslate): ReactNode {
  if (column === "title") {
    return (
      <div className="min-w-0">
        <button
  ...
  if (column === "owner") return <span className="block truncate text-gray-700 dark:text-gray-200">{doc.owner?.name ?? t("unassigned")}</span>
  ...
  if (column === "latest_version") return <span className="tabular-nums text-gray-700 dark:text-gray-200">{doc.current_version_number ? t("preview_version", { number: doc.current_version_number }) : t("none")}</span>
...
function defaultDesignDocOptionalColumns(t: DesignDocsTranslate) {
  return [
    { key: "doc_slug", title: "DOC" },
    { key: "state", title: t("column_state") },
    { key: "repository", title: t("column_repository") },
    { key: "owner", title: t("column_owner") },
    { key: "collaborators", title: t("column_collaborators") },
    { key: "comments", title: t("column_comments") },
    { key: "latest_version", title: t("column_latest_version") },
    { key: "updated_at", title: t("column_updated") },
    { key: "actions", title: t("column_actions") }
  ]
}
...
function repositoryLabel(doc: DesignDocSummary, t: DesignDocsTranslate) {
  ...
  useEffect(() => {
    ...
    function recomputeNow() {
      ...
      const measurements = railEntries.map((entry) => {
        ...
        return {
          id: entry.id,
          anchorTop: marker ? marker.getBoundingClientRect().top - containerTop : fallbackAnchorTop({ anchorStart, containerTop, draft, markerRoot }),
...
function DesignDocTitleBar({ archiveDisabled, canArchive, collaborators, doc, repoIds, repositories, repositoryPickerOpen, selectedRepositories, selectedVersionId, setCollaborators, setRepoIds, setRepositoryPickerOpen, setShareOpen, setTitle, shareOpen, title, versions, versionsLoading, versionsOpen, canManageMetadata, isArchived, onArchive, onMetadataSave, onSave, saveLabel, saveDisabled, onVersionChange, onVersionsOpen, onVisibilityChange }: {
  ...
  function positionShareMenu() {
    ...
    const containerRect = shareButtonRef.current?.parentElement?.getBoundingClientRect()
    ...
    const menuWidth = Math.min(320, Math.max(0, window.innerWidth - edgePadding * 2))
    const boundary = titleBarRef.current?.getBoundingClientRect()
    const viewportBoundaryLeft = edgePadding
    const viewportBoundaryRight = window.innerWidth - edgePadding
    const clampedBoundaryLeft = Math.max(boundary?.left ?? viewportBoundaryLeft, viewportBoundaryLeft)
    const clampedBoundaryRight = Math.min(boundary?.right ?? viewportBoundaryRight, viewportBoundaryRight)
    const boundaryLeft = clampedBoundaryLeft < clampedBoundaryRight ? clampedBoundaryLeft : viewportBoundaryLeft
    const boundaryRight = clampedBoundaryLeft < clampedBoundaryRight ? clampedBoundaryRight : viewportBoundaryRight
    const minLeft = boundaryLeft
    const maxLeft = Math.max(minLeft, boundaryRight - menuWidth)
    const viewportLeft = Math.min(Math.max(rect.left, minLeft), maxLeft)
...
function fallbackAnchorTop({ anchorStart, containerTop, draft, markerRoot }: {
  ...
  if (anchorStart == null || !markerRoot) return null

  const sourceElement = Array.from(markerRoot.querySelectorAll("[data-source-start][data-source-end]")).find((element) => {
    const start = Number((element as HTMLElement).dataset.sourceStart)
    const end = Number((element as HTMLElement).dataset.sourceEnd)
    return Number.isFinite(start) && Number.isFinite(end) && start <= anchorStart && end > anchorStart
  }) as HTMLElement | undefined
  if (sourceElement) return sourceElement.getBoundingClientRect().top - containerTop

  const rootTop = markerRoot.getBoundingClientRect().top - containerTop
  const lineHeight = 24
  const lineIndex = draft.slice(0, anchorStart).split("\n").length - 1
  return rootTop + 16 + lineIndex * lineHeight
}
...
function renderWholeSuggestionBlockHtml(highlight: AnchorHighlight, focusedThreadId: number | null, focusedSuggestionId: number | null, context: WysiwygRenderContext) {
  ...
  return [
    `<div class="${className}" data-anchor-highlight="${highlight.id}" data-anchor-status="${escapeHtml(highlight.status)}" data-block-suggestion-state="${escapeHtml(highlight.suggestionState || "")}" data-inline-suggestion-state="${escapeHtml(highlight.suggestionState || "")}" data-original-markdown="${escapeHtml(original)}"${suggestionAttrs}>`,
    ...
    .map((segment) => {
      ...
      if (segment.highlight.kind === "suggestion") {
        ...
        if (segment.highlight.renderMode === "block") {
          ...
          return [
            `<mark class="${className}" data-anchor-highlight="${segment.highlight.id}" data-anchor-status="${escapeHtml(segment.highlight.status)}" data-block-suggestion-state="${escapeHtml(segment.highlight.suggestionState || "")}" data-inline-suggestion-state="${escapeHtml(segment.highlight.suggestionState || "")}" data-original-markdown="${escapeHtml(original)}"${suggestionAttrs}>`,

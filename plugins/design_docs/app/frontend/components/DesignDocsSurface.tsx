...
type SelectionRange = { start: number; end: number; text: string; rect: SelectionRect | null }
...
function DesignDocEditor({ doc, mode, repositories, onDocChange }: { doc: DesignDocDetail; mode: SurfaceMode; repositories: Array<{ id: number; slug: string }>; onDocChange: (doc: DesignDocDetail, message: string) => void }) {
  ...
  const [selection, setSelection] = useState<SelectionRange>({ start: 0, end: 0, text: "", rect: null })
  ...
  const commentMutation = useMutation({
    ...
    onSuccess: (payload) => {
      setCommentBody("")
      setSelection({ start: 0, end: 0, text: "", rect: null })
  ...
  function updateSelection(event?: ChangeEvent<HTMLTextAreaElement>) {
    ...
    setSelection({ start, end, text: target.value.slice(start, end), rect: textareaSelectionRect(target, start, end) })
  }
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-xs text-gray-500 dark:text-gray-400">Selected {selection.end - selection.start} characters</p>
      </div>

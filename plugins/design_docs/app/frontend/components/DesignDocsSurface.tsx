...
export function DesignDocsSurface({ chatId, compact = false, designDocIds, initialDesignDocId, initialDesignDocs = [], mode, repositoryId }: {
  ...
  if (mode === "repository") {
    return (
      <RepositoryPageShell
        ...
        ariaLabel={t("design_docs:page.aria_label")}
  ...
  return (
    <main aria-label={t("design_docs:page.aria_label")} className={compact ? "space-y-4" : "mx-auto max-w-[96rem] space-y-6 p-6"}>
      {content}
    </main>
  )
}
...
function DesignDocEditor({ doc, mode, repositories, onDocChange }: { doc: DesignDocDetail; mode: SurfaceMode; repositories: Array<{ id: number; slug: string }>; onDocChange: (doc: DesignDocDetail, message?: string) => void }) {
  ...
  const reviewMutation = useMutation({
    ...
    onSuccess: (payload) => {
      ...
      onDocChange(payload.design_doc, payload.message || t("editor.suggestion_reviewed"))
    }
  })
  ...
  return (
    ...
    >
      ...
      <div className="relative shrink-0" ref={moreMenuRef}>
        <ToolbarIconButton ariaExpanded={moreOpen} disabled={readOnly} icon="..." label={t("toolbar.more_formatting_label")} onClick={() => setMoreOpen((open) => !open)} />
        {moreOpen ? (
  ...
  return (
    <Panel className="relative min-h-[36rem]">
      ...
      <div className="relative mt-3 space-y-3">
        ...
        {!historicalVersionLoading && railEntries.length === 0 ? <p className="text-sm text-gray-500 dark:text-gray-400">{viewingHistory ? "No threads existed as of this version." : t("threads.empty")}</p> : null}
        <div

...
type SurfaceMode = "index" | "show" | "repository" | "chat"
...
export function DesignDocsSurface({ chatId, compact = false, designDocIds, initialDesignDocId, initialDesignDocs = [], mode, repositoryId }: {
  ...
  const indexQuery = useQuery({
    ...
    enabled: mode === "index" || mode === "repository"
  })
  ...
  const isFocusedSurface = mode === "show" || mode === "chat"
  const showIndexControls = mode === "index" || mode === "repository"
  const showDocList = mode === "index" || mode === "repository"
  ...
  const layoutClass = mode === "index"
    ? "grid-cols-1"
    : compact && showDocList
      ? "xl:grid-cols-[18rem_minmax(0,1fr)]"
      : showDesktopInlineFolders
        ? "lg:grid-cols-[16rem_20rem_minmax(0,1fr)]"
        : showDocList
  ...
  return (
    <main aria-label="Design docs" className={compact ? "space-y-4" : "mx-auto max-w-[100rem] space-y-6 p-6"}>
      {!isFocusedSurface ? <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
      ...
      <div className={`grid min-h-0 gap-4 ${layoutClass}`}>
        ...
        /> : null}
        {mode === "index" ? null : <section className="min-w-0">

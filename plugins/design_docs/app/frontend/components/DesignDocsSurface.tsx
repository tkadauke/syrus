      ...
      <section className="min-w-0 space-y-4">
        <div className="overflow-hidden rounded border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900">
          ...
          <SelectionCommentAffordance
            ...
            selection={selection}
            onOpenCommentComposer={() => setCommentComposerOpen(true)}
          />
          </div>
        </div>
      </section>
...
function SelectionCommentAffordance({ disabled, selection, onOpenCommentComposer }: {
  disabled: boolean
...
function ThreadPanel({ commentBody, commentComposerOpen, commentComposerRef, doc, focusedThreadId, replyBodies, selection, threadRefs, onComment, onCommentChange, onFocus, onReply, onReplyChange, onResolve }: {
  commentBody: string
  commentComposerOpen: boolean
  commentComposerRef: React.MutableRefObject<HTMLInputElement | null>
  doc: DesignDocDetail
  ...
  return (
    <Panel className="relative min-h-[36rem]">
      ...
      <div className="relative mt-3 space-y-3">
        {commentComposerOpen && hasSelection ? (
          <div className="rounded border border-brand/40 bg-brand/5 p-3">
            <p aria-label="Selected text for new thread" className="rounded bg-white/70 p-2 text-xs text-gray-600 dark:bg-gray-950/40 dark:text-gray-300">{selection.selectedText || selection.text}</p>
            <div className="mt-3 flex gap-2">
              <Input
                ...
                value={commentBody}
              />
              <Button disabled={commentBody.trim().length === 0} onClick={onComment} size="sm" variant="secondary">
                Comment
              </Button>
            </div>
          </div>
...
function CommentBubbleIcon() {
  return (
    <span aria-hidden="true" className="relative block h-4 w-4 rounded-[0.2rem] border-2 border-current">
      <span className="absolute -bottom-1 left-1 h-1.5 w-1.5 border-b-2 border-l-2 border-current bg-surface" />
    </span>
  )
}

...
describe("applyDesignDocFormattingCommand", () => {
  ...
  it("leaves fenced code content unchanged for block formatting commands", () => {
    ...
    expect(applyDesignDocFormattingCommand(markdown, { start: codeStart, end: codeEnd }, "heading_2")).toMatchObject({
      markdown,
      applied: true
    })
    expect(applyDesignDocFormattingCommand(markdown, { start: codeStart, end: codeEnd }, "paragraph")).toMatchObject({
      markdown,
      applied: true
    })
    expect(applyDesignDocFormattingCommand(markdown, { start: codeStart, end: codeEnd }, "horizontal_rule")).toMatchObject({
      markdown,
      applied: false
    })
  })

...
describe("DesignDocsSurface", () => {
  ...
  it("switches editor tabs without losing unsaved local edits", async () => {
    ...
    expect(screen.getByRole("textbox", { name: "WYSIWYG editor" })).toHaveValue("Unsaved **local** edits")
    fireEvent.change(screen.getByRole("textbox", { name: "WYSIWYG editor" }), { target: { value: "Edited from WYSIWYG" } })
  ...
  it("renders title-bar controls for repositories, sharing, and far-right versions", async () => {
    ...
    fireEvent.focus(within(titleBar).getByRole("combobox", { name: "Version selection" }))

...
export default function WorkspaceDesignDocs({ payload, tab }: PluginWorkspaceTabProps) {
  const designDocIds = Array.isArray(tab?.data?.design_doc_ids) ? tab.data.design_doc_ids.filter((id): id is number => typeof id === "number") : []
  const designDocId = typeof tab?.data?.design_doc_id === "number" ? tab.data.design_doc_id : designDocIds[0]
  const designDocs = Array.isArray(tab?.data?.design_docs) ? tab.data.design_docs.filter((doc): doc is DesignDocSummary => isDesignDocSummary(doc)) : []

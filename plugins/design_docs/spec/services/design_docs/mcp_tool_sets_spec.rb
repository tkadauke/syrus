...
RSpec.describe "DesignDocs MCP tool sets" do
  ...
  it "keeps non-overflow database failures inside the tool error response" do
    server = chat_server
    allow(DesignDocs::Create).to receive(:call).and_raise(
    ...
    response = call_tool(server, "propose_design_doc", title: "Database issue", markdown: "x")

    expect(response.dig(:result, :isError)).to be true
    expect(response.dig(:result, :content, 0, :text)).to eq("Could not propose design doc: database is temporarily unavailable")
  end

...
RSpec.describe "Design Docs @syrus agent mentions" do
  ...
  it "includes the triggering reply and full discussion for long threads" do
    doc = create_design_doc
    thread = create_thread(doc)
    45.times do |index|
      thread.comments.create!(author_kind: "user", author_user: collaborator, body: "Earlier comment #{index}")
    end

    trigger = thread.comments.create!(author_kind: "user", author_user: owner, body: "@syrus summarize the latest decision")

    comments = DesignDocs::DesignDocAgentRun.last.context_snapshot.dig("thread", "comments")
    expect(comments.length).to eq(47)
    expect(comments.map { |comment| comment.fetch("id") }).to include(trigger.id)
    expect(comments.last.fetch("body")).to eq("@syrus summarize the latest decision")
  end

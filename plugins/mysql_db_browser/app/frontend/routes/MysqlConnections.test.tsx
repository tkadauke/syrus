...
describe("MysqlConnections", () => {
  ...
  it("lists connections without ever rendering a password", async () => {
    ...
    expect(screen.getByRole("main", { name: "MySQL DB Browser" })).toHaveClass("flex", "h-full", "flex-col", "gap-6", "overflow-hidden", "p-3", "sm:p-6")
  ...
  describe("schema browsing", () => {
    it("lists databases (including system schemas) and shows the table's Content grid by default", async () => {
      ...
      expect(screen.getByRole("tab", { name: "Browse" }).closest('[role="tablist"]')).toHaveClass("shrink-0")
      ...
      expect(schemaTree.parentElement).toHaveClass("grid", "min-h-0", "flex-1")
    ...
    it("constrains the content column so a wide results table scrolls instead of widening the page", async () => {
      ...
      expect(column).toHaveClass("min-w-0", "overflow-hidden")

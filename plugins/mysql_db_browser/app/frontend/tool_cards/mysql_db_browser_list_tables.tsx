...
function parseCard(context: ToolCardContext): TablesCard | null {
  ...
  return {
    ...
    tables: parsed.tables.flatMap((entry) => {
      const row = parseTableSummary(entry)
      return row ? [row] : []
    })
  }
}
      ...
      <TableShell>
        <table className="w-full text-left text-xs">
          ...
          <tbody className="divide-y divide-gray-100 bg-white dark:divide-gray-800 dark:bg-gray-950">
            {card.tables.map((row) => (
              <tr key={row.name}>
                <td className="px-2 py-1 font-mono text-gray-800 dark:text-gray-200" title={row.comment ?? undefined}>
                  {row.name}
                </td>
                ...
                <td className="whitespace-nowrap px-2 py-1 text-gray-600 dark:text-gray-300">
                  {row.approxRowCount != null ? row.approxRowCount.toLocaleString() : "—"}
                </td>

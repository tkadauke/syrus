...
RSpec.describe K8sCluster::DaemonSetsTool do
  ...
  it "lists daemon sets" do
    payload = { available: true, daemon_sets: [] }
    service = instance_double(K8sCluster::DaemonSets, list: payload)
    allow(K8sCluster::DaemonSets).to receive(:new).with(cluster).and_return(service)
  ...
  it "describes a single daemon set when name and namespace are given" do
    payload = { available: true, daemon_set: { "metadata" => { "name" => "monitoring" } } }
    service = instance_double(K8sCluster::DaemonSets, describe: payload)
    allow(K8sCluster::DaemonSets).to receive(:new).with(cluster).and_return(service)

    response = call(cluster_id: cluster.id, namespace: "default", name: "monitoring")

    expect(response.error?).to be(false)
    expect(service).to have_received(:describe).with("monitoring", namespace: "default")
  end

  it "refuses to describe without a namespace" do
    response = call(cluster_id: cluster.id, name: "monitoring")

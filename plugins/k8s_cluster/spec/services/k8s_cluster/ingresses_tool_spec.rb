...
RSpec.describe K8sCluster::IngressesTool do
  ...
  it "lists ingresses" do
    payload = { available: true, ingresses: [] }
    service = instance_double(K8sCluster::Ingresses, list: payload)
    allow(K8sCluster::Ingresses).to receive(:new).with(cluster).and_return(service)
  ...
  it "describes a single ingress when name and namespace are given" do
    payload = { available: true, ingress: { "metadata" => { "name" => "web" } } }
    service = instance_double(K8sCluster::Ingresses, describe: payload)
    allow(K8sCluster::Ingresses).to receive(:new).with(cluster).and_return(service)

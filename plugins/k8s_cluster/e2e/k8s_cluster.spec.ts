...
test("K8s Cluster Viewer registers a cluster and browses it read-only, with no write-action controls", async ({ page }) => {
  ...
  const pluginCard = page.getByRole("region", { name: "Registered plugins" })
    .locator("article")
    .filter({ has: page.getByRole("heading", { name: "Kubernetes Cluster Viewer", exact: true }) })
  await expect(pluginCard).toBeVisible()
  const enableButton = pluginCard.getByRole("button", { name: "Enable" })
  if (await enableButton.isVisible()) {
    await enableButton.click()
    await page.waitForURL("**/admin/plugins/k8s_cluster")
  }
  ...
  const addDialog = page.getByRole("dialog", { name: "Add cluster" })
  await addDialog.getByLabel("Label", { exact: true }).fill(clusterLabel)
  await addDialog.getByLabel("Kubeconfig", { exact: true }).fill(KUBECONFIG)
  await addDialog.getByRole("button", { name: "Add cluster", exact: true }).click()

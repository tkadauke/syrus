...
function Metric({ context, title, tone = "idle", value }: { context?: string; title: string; tone?: "idle" | "ok" | "warn"; value: ReactNode }) {
  const toneClass = tone === "ok" ? "border-success/30 bg-success/10" : tone === "warn" ? "border-warning/30 bg-warning/10" : ""
  ...
  return (
    <Section.Root className={toneClass}>
      ...
      <Text className="mt-2 text-2xl font-semibold" tone={valueTone}>
        {value}
      </Text>
      {context ? (

...
export type CatalogFeedbackDialogProps<Metadata extends CatalogFeedbackMetadata> = {
  ...
  createJob?: (input: { prompt: string; screenshot: { name: string; mimeType: string; dataUrl: string } | null }) => Promise<unknown>
...
export function CatalogFeedbackDialog<Metadata extends CatalogFeedbackMetadata>({
  ...
  const createJobMutation = useMutation({
    mutationFn: (input: { prompt: string; screenshot: { name: string; mimeType: string; dataUrl: string } | null }) => {
      if (!createJob) throw new Error("No job creation handler configured")
      return createJob(input)
    },
    onSuccess: (payload) => {
      setOpen(false)
      const redirectTo = typeof payload === "object" && payload && "redirect_to" in payload ? String(payload.redirect_to) : null
      if (redirectTo) navigate(withRoutePrefix(redirectTo, routePrefix(location.pathname)))
    }
  })
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (createJob) {
      ...
      const prompt = buildPrompt(trimmedPrompt, metadata)
  ...
  function startDiscussChat() {
    ...
    const attachments: ChatMessageAttachmentInput[] = screenshot ? [{ dataUrl: screenshot.dataUrl, mimeType: "image/png", name: attachmentName }] : []
    startChat.mutate({ attachments, text })
  }

  const promptIsBlank = promptText.trim().length === 0
  const busy = startChat.isPending || createJobMutation.isPending || capturing
  let submitLabel = copy.submit
  if (createJob) {
    submitLabel = createJobMutation.isPending ? (copy.creatingJob ?? copy.starting) : (copy.createJob ?? copy.submit)
  } else if (startChat.isPending) {
    submitLabel = copy.starting
  }
          ...
          <form className="space-y-4" onSubmit={submit}>
            ...
            {screenshot ? (
              <div className="space-y-2">
                <img alt={copy.screenshotAlt} className="max-h-64 w-full rounded border border-border object-contain" src={screenshot.dataUrl} />
            ...
            <details className="group rounded border border-border">
              ...
              <pre className="max-h-48 overflow-auto border-t border-border px-3 py-2 text-xs text-text-secondary">{JSON.stringify(metadata, null, 2)}</pre>
            </details>
            ...
            <div className="flex justify-end gap-2 border-t border-border pt-4">
              ...
              {createJob ? (
                <Button disabled={busy} onClick={startDiscussChat} type="button" variant="secondary">
                  {startChat.isPending ? copy.starting : copy.submit}
                </Button>
              ) : null}
              <Button
                disabled={busy || (createJob ? promptIsBlank : false)}

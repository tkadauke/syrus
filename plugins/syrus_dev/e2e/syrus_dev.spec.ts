...
test("Syrus Dev Performance UI drills into a seeded request/run and runs a real SQL explain", async ({ page }) => {
  ...
  const pluginCard = page.getByRole("region", { name: "Registered plugins" })
    .locator("article")
    .filter({ has: page.getByRole("heading", { name: "Syrus Dev", exact: true }) })
  await expect(pluginCard).toBeVisible()
  const enableButton = pluginCard.getByRole("button", { name: "Enable" })
  if (await enableButton.isVisible()) {
    await enableButton.click()
    await expect(page).toHaveURL(/\/admin\/plugins\/syrus_dev$/)
    await expect(page.getByRole("main", { name: "Plugin detail" }).getByRole("heading", { level: 1, name: "Syrus Dev" })).toBeVisible()
  }

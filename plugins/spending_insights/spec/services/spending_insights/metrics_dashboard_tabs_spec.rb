...
RSpec.describe SpendingInsights::MetricsDashboardTabs do
  it "declares one tab with a panel per metric it actually exports" do
    tab = described_class.metrics_dashboard_tabs.sole

    expect(tab[:id]).to eq("spending_insights")
    expect(tab[:label]).to be_present
    expect(tab[:panels]).to be_present
    expect(tab[:panels].map { |panel| panel[:key] }).to match_array(%w[run_cost_by_provider run_cost_by_trigger_kind])
  end

...
module AgentMemory
  module Tools
    class WriteMemoryTool < MCP::Tool
      ...
      input_schema(
        properties: {
          content:  { type: "string", maxLength: AgentMemory::Entry::CONTENT_MAX_LENGTH, description: "Memory content (max #{AgentMemory::Entry::CONTENT_MAX_LENGTH} characters)." },
          kind:     { type: "string", enum: AgentMemory::Entry::KIND, description: "Memory kind." },
          scope:    { type: "string", enum: AgentMemory::Entry::TOOL_SCOPES, description: "Memory scope: global or repository. Omit to default to repository in workflow runs." },
      ...
      class << self
        def call(content:, kind:, server_context:, scope: nil, scope_id: nil)
          ...
          if content.length > AgentMemory::Entry::CONTENT_MAX_LENGTH
            return invalid_response(
              "content is #{content.length} characters; write_memory content must be " \
              "#{AgentMemory::Entry::CONTENT_MAX_LENGTH} characters or fewer. Store one concise " \
              "durable fact per memory, or split independent facts into separate memories."
            )
          end
          return invalid_response("kind must be one of #{AgentMemory::Entry::KIND.join(', ')}") unless AgentMemory::Entry::KIND.include?(kind)
          ...
          if (duplicate = duplicate_memory_for(context, content, kind, effective_scope, repository&.id))
            return invalid_response(
              "similar #{kind} memory already exists as memory ##{duplicate.id}; " \

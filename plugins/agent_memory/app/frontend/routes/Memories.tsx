      ...
      <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
        ...
        <tbody className="divide-y divide-gray-100 text-sm dark:divide-gray-800">
          {payload.memories.length === 0 ? (
            <tr>
              <td className="px-4 py-6 text-center text-gray-500 dark:text-gray-400" colSpan={columnCount}>
    ...
    <tr className="align-top">
      ...
      <td className="px-4 py-3">
        {showDeleted ? (
          ...
          <span className={memory.published ? "text-green-700 dark:text-green-300" : "text-gray-500 dark:text-gray-400"}>
            {memory.published ? t("published_label") : t("unpublished_label")}
          </span>
        )}
      </td>
...
function PanelMessage({ children }: { children: string }) {
  const { t } = useT("agent_memory")
  return (
    <section className="rounded border border-gray-200 bg-white p-6 text-sm text-gray-600 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400">
      {children}
    </section>
  )
}
...
function MemoriesError({ error }: { error: unknown }) {
  const { t } = useT("agent_memory")
  return (
    <section className="rounded border border-red-200 bg-red-50 p-4 text-sm text-red-800 dark:border-red-900 dark:bg-red-950 dark:text-red-200">
      {errorMessage(error, "Unable to load memories.")}
    </section>
  )
}

...
RSpec.describe AgentMemory::Entry do
  ...
  describe "audit events" do
    it "emits a created event on save" do
      memory = build_memory
      expect { memory.save! }.to change(AgentMemory::AuditEvent, :count).by(1)

      event = AgentMemory::AuditEvent.last
    ...
    it "emits an updated event when content changes" do
      ...
      expect { memory.update!(content: "New content.") }
        .to change(AgentMemory::AuditEvent, :count).by(1)
    ...
    it "does not emit an updated event for non-audited field changes" do
      ...
      expect { memory.update!(published: true) }
        .not_to change(AgentMemory::AuditEvent, :count)
    end

    it "emits a deleted event via soft_delete_by! with user actor" do
      memory = build_memory
      memory.save!
      initial_count = AgentMemory::AuditEvent.count

      memory.soft_delete_by!(owner)

      new_events = AgentMemory::AuditEvent.where("id > ?", AgentMemory::AuditEvent.order(:id).offset(initial_count - 1).pick(:id) || 0)
      delete_event = AgentMemory::AuditEvent.order(:id).last
    ...
    it "does not emit an updated event when only deleted_at changes" do
      ...
      events_after = AgentMemory::AuditEvent.where(entry: memory).order(:id)

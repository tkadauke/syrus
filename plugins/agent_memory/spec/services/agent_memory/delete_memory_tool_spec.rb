...
RSpec.describe AgentMemory::Tools::DeleteMemoryTool do
  ...
  describe ".call from a workflow run context" do
    ...
    it "emits an audit event with actor_run_id" do
      ...
      event = AgentMemory::AuditEvent.where(entry_id: memory.id, event_type: "deleted").last
  ...
  describe ".call from a chat session context" do
    ...
    it "soft-deletes a memory owned by the chat user" do
      owned = AgentMemory::Entry.create!(user: chat_user, kind: "feedback", scope: "repository",
    ...
    it "rejects deletion of another user's memory" do
      other = AgentMemory::Entry.create!(user: Factories.user, kind: "feedback", scope: "repository",

...
RSpec.describe "Mcp::Tools shared memory tools" do
  ...
  describe AgentMemory::Tools::SearchMemoriesTool do
    ...
    context "run surface" do
      it "finds memories matching the query within the job's repository" do
        ...
        expect(AgentMemory::Entry.find(ids.first).content).to include("Alpha")
      end
    ...
    context "chat surface" do
      ...
      it "searches visible memories including global scope" do
        global = AgentMemory::Entry.create!(user: user, kind: "user_pref", scope: "global", scope_id: nil,
      ...
      it "filters by scope when scope is provided" do
        global = AgentMemory::Entry.create!(user: user, kind: "user_pref", scope: "global", scope_id: nil, content: "alpha global")
  ...
  describe AgentMemory::Tools::ListMemoriesTool do
    describe "schema surface" do
    ...
    context "chat surface" do
      ...
      it "lists memories visible to the chat session" do
        global = AgentMemory::Entry.create!(user: user, kind: "user_pref", scope: "global", scope_id: nil, content: "Global pref.")
  ...
  describe "chat surface full tool set via MCP server" do
    ...
    def server
      MCP::Server.new(
        ...
        tools: [
          AgentMemory::Tools::WriteMemoryTool,
          AgentMemory::Tools::ReadMemoryTool,
          AgentMemory::Tools::SearchMemoriesTool,
          AgentMemory::Tools::ListMemoriesTool,
          AgentMemory::Tools::DeleteMemoryTool,
          AgentMemory::Tools::PublishMemoryTool,
          AgentMemory::Tools::UnpublishMemoryTool
        ],
    ...
    it "writes a global memory" do
      ...
      memory = AgentMemory::Entry.find(payload(response)[:id])
    ...
    it "writes a repository-scoped memory only for owned repos" do
      ...
      expect(AgentMemory::Entry.find(payload(good)[:id]).scope_id).to eq(repository.id)
      expect(bad.dig(:result, :isError)).to be true
    end

    it "reads an owned memory by id" do
      m = AgentMemory::Entry.create!(user: user, kind: "feedback", scope: "global", content: "A note.")
      response = call_tool("read_memory", id: m.id)
      expect(payload(response).dig(:memory, :id)).to eq(m.id)
    end

    it "lists visible memories" do
      global = AgentMemory::Entry.create!(user: user, kind: "user_pref", scope: "global", scope_id: nil, content: "Pref.")
      repo_m = create_repo_memory(content: "Repo fact.")

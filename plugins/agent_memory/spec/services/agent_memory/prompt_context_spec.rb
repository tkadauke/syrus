...
RSpec.describe AgentMemory::PromptContext do
  ...
  it "ranks high-confidence memories before low-confidence ones" do
    AgentMemory::Entry.create!(user: user, kind: "feedback", scope: "repository", scope_id: repository.id, content: "Low confidence.", confidence: 0.3)
    AgentMemory::Entry.create!(user: user, kind: "feedback", scope: "repository", scope_id: repository.id, content: "High confidence.", confidence: 0.9)
  ...
  it "ranks repository-scoped memories above global ones" do
    AgentMemory::Entry.create!(user: user, kind: "feedback", scope: "global", content: "Global feedback.")
    AgentMemory::Entry.create!(user: user, kind: "feedback", scope: "repository", scope_id: repository.id, content: "Repository feedback.")

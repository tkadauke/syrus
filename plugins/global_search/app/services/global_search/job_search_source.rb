module GlobalSearch
  class JobSearchSource
    ...
    def self.search_backfill_needed?
      indexed_count(TABLE_NAME, ID_COLUMN) < Job.count
    ...
    def self.search_database_rebuild_batch(task:)
      ...
      MaintenanceTasks::Definitions::Base::Result.new(done: false, processed: batch[:processed].to_i, failed: 0, message: "Indexed #{batch[:processed].to_i} job(s).", level: "progress")
    end

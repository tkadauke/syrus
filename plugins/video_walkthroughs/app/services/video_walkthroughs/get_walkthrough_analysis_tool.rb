    ...
    class << self
      def call(server_context:, walkthrough_id: nil, **)
        ...
        attached_keys = frames.map do |frame|
          VideoWalkthroughs::Prompts::Report.attachment_key(seconds: frame.seconds, title: frame.label)
        end

        report = VideoWalkthroughs::Prompts::Report.new(
      ...
      def extract_frames(walkthrough)
        ...
        with_local_video(walkthrough) do |path|
          VideoWalkthroughs::Gemini::FrameExtractor.extract(video_path: path, timestamps: entries)
        end
      ...
      def frame_entries(walkthrough)
        entries = walkthrough.analysis_issues.filter_map do |issue|
          seconds = VideoWalkthroughs::Gemini::FrameExtractor.parse_timestamp(issue["timestamp"])
          next unless seconds
          ...
          flagged = flagged_issue?(issue)
          {
            ...
            jpeg_quality: flagged ? VideoWalkthroughs::Gemini::FrameExtractor::HIGH_JPEG_QUALITY : VideoWalkthroughs::Gemini::FrameExtractor::JPEG_QUALITY
          }
        end
      ...
      def prioritize_flagged(entries)
        cap = VideoWalkthroughs::Gemini::FrameExtractor::MAX_FRAMES

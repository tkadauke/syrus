...
# The video→Epic pipeline's middle: take an uploaded VideoWalkthroughs::Walkthrough,
...
class VideoWalkthroughs::AnalysisJob < ApplicationJob
  queue_as :videos
  ...
  class << self
    ...
    def client_factory
      @client_factory ||= ->(api_key:) { VideoWalkthroughs::Gemini::Client.new(api_key: api_key) }
    end
  end

  def perform(walkthrough_id)
    walkthrough = VideoWalkthroughs::Walkthrough.find_by(id: walkthrough_id)
    return unless walkthrough&.uploaded?

    # Plugin disabled → land the row in a terminal state (the composer chip
    # polls to a resolution; a silent return would spin it forever). A job can
    # still be in the queue from before an admin turned the plugin off.
    unless VideoWalkthroughs.enabled?
      fail_walkthrough!(walkthrough, "Walkthrough videos are not enabled on this instance — an admin can turn the plugin on under Plugins.")
      return
    end
  ...
  rescue VideoWalkthroughs::Gemini::Client::AuthError => error
    fail_walkthrough!(walkthrough, "Gemini rejected the API key: #{error.message} Check it under Credentials.")
  rescue VideoWalkthroughs::Gemini::Client::RateLimited
    fail_walkthrough!(
      walkthrough,
  ...
  rescue VideoWalkthroughs::Gemini::Client::ConnectionError
    fail_walkthrough!(walkthrough, "Could not reach Gemini — check the connection and retry the analysis.")
  rescue VideoWalkthroughs::Gemini::Client::Error => error
    ...
    Rails.logger.error("[VideoWalkthroughs::AnalysisJob] #{error.class}: #{error.message}")
  ...
  def analyze(client, walkthrough, video_path)
    ...
    raise unless walkthrough.duration_seconds.to_i >= VideoWalkthroughs::Gemini::Client::LOW_RESOLUTION_FALLBACK_SECONDS
    raise if walkthrough.gemini_file_uri.blank?

    Rails.logger.info("[VideoWalkthroughs::AnalysisJob] full-res rate-limited on a #{walkthrough.duration_seconds}s video; retrying at low resolution")
  ...
  def generate(client, walkthrough, file_uri, media_resolution:)
    client.generate_content(
      ...
      prompt: VideoWalkthroughs::Prompts::Analysis.new(repo_context: repo_context_for(walkthrough)).to_s,
      response_schema: VideoWalkthroughs::Prompts::Analysis::RESPONSE_SCHEMA,
  ...
  def compact_for_storage(walkthrough, source_path)
    return unless VideoWalkthroughs::Gemini::VideoTranscoder.available?

    Dir.mktmpdir("syrus-transcode-") do |dir|
      mp4 = File.join(dir, "compact.mp4")
      return unless VideoWalkthroughs::Gemini::VideoTranscoder.to_compact_mp4(input_path: source_path, output_path: mp4)
    ...
    Rails.logger.warn("[VideoWalkthroughs::AnalysisJob] storage transcode failed (keeping original): #{error.class}: #{error.message}")
  end
  ...
  def deliver_chat_turn(walkthrough)
    ...
    Rails.logger.error("[VideoWalkthroughs::AnalysisJob] chat delivery failed: #{error.class}: #{error.message}")
    fail_walkthrough!(
      walkthrough,

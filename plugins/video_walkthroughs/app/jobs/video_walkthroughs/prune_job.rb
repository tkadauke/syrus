...
class VideoWalkthroughs::PruneJob < ApplicationJob
  queue_as :cleanup
  ...
  # Read from the model rather than written as a literal: these three queries
  # have to qualify columns because the blob join also has id/byte_size, and a
  # literal is what let the last table rename slip past them.
  #
  # Fully qualified because `class VideoWalkthroughs::PruneJob` is the compact
  # form, which opens only the innermost constant -- the enclosing module is
  # not in the lexical scope, so a bare `Walkthrough` would not resolve.
  def table = VideoWalkthroughs::Walkthrough.quoted_table_name
  ...
  def log(message)
    Rails.logger.info("[VideoWalkthroughs::PruneJob] #{message}")
  end
end

...
Walkthrough Videos ships as a plugin, off by default. Turn it on from
**Admin -> Plugins**, or with:

```ruby
PluginRecord.find_by(name: "video_walkthroughs").update(enabled: true)
```

The plugin is its own feature flag -- there is no separate Labs toggle. While it
is off, the composer offers no video intake, the upload and retry endpoints
answer `plugin_disabled`, the three walkthrough MCP tools are not advertised,
and a chat turn carrying a walkthrough message reads as a plain note. Threads
that already have analyzed walkthroughs keep their history.

...
# the VideoWalkthroughs::AnalysisJob.client_factory test seam so no Net::HTTP
# stubbing is needed.
class FakeGeminiWalkthroughClient
  ...
  # (VideoWalkthroughs::Gemini::Client#resolve_video_model!); the fake records the call and
...
RSpec.describe VideoWalkthroughs::AnalysisJob do
  before do
  ...
  describe "Gemini errors" do
    ...
    context "auth error" do
      let(:error) { VideoWalkthroughs::Gemini::Client::AuthError.new("API key not valid.") }
    ...
    context "rate limited" do
      let(:error) { VideoWalkthroughs::Gemini::Client::RateLimited.new("429") }
    ...
    context "generic client error" do
      let(:error) { VideoWalkthroughs::Gemini::Client::Error.new("Gemini returned an empty analysis") }
  ...
  describe "no-op guards" do
    ...
    it "returns without raising for a missing walkthrough id" do
      missing_id = VideoWalkthroughs::Walkthrough.maximum(:id).to_i + 1
  ...
  # client now wraps transport failures as VideoWalkthroughs::Gemini::Client::ConnectionError and
  # the job rescues it into a terminal "failed" state.
  describe "transient network failure" do
    let(:fake_client) do
      FakeGeminiWalkthroughClient.new(raise_on_upload: VideoWalkthroughs::Gemini::Client::ConnectionError.new("could not reach Gemini"))
    end
    ...
    context "short video (duration < threshold)" do
      let(:fake_client) do
        FakeGeminiWalkthroughClient.new(
          analysis: analysis,
          raise_on_generate: VideoWalkthroughs::Gemini::Client::RateLimited.new("429")
        )
      end

      it "fails with the quota message and never retries at low resolution" do
        short = (VideoWalkthroughs::Gemini::Client::LOW_RESOLUTION_FALLBACK_SECONDS - 1)
    ...
    context "long video (duration >= threshold)" do
      let(:fake_client) do
        FakeGeminiWalkthroughClient.new(
          ...
          raise_on_generate: [ VideoWalkthroughs::Gemini::Client::RateLimited.new("429"), nil ]
        )
      end

      it "retries at low resolution and succeeds" do
        long = VideoWalkthroughs::Gemini::Client::LOW_RESOLUTION_FALLBACK_SECONDS
  ...
  describe "storage transcode (compact_for_storage)" do
    around do |example|
      original = VideoWalkthroughs::Gemini::VideoTranscoder.runner
      begin
        example.run
      ensure
        VideoWalkthroughs::Gemini::VideoTranscoder.runner = original
      end
    end
    ...
    it "keeps the original webm blob when ffmpeg is unavailable" do
      ...
      allow(VideoWalkthroughs::Gemini::VideoTranscoder).to receive(:available?).and_return(false)
      expect(VideoWalkthroughs::Gemini::VideoTranscoder).not_to receive(:to_compact_mp4)
      walkthrough = create_walkthrough
    ...
    it "replaces the stored blob with the compact mp4 and updates content_type + byte_size" do
      ...
      allow(VideoWalkthroughs::Gemini::VideoTranscoder).to receive(:available?).and_return(true)
      allow(VideoWalkthroughs::Gemini::VideoTranscoder).to receive(:to_compact_mp4) do |input_path:, output_path:|
    ...
    it "keeps the original blob when the transcode fails (returns false)" do
      allow(VideoWalkthroughs::Gemini::VideoTranscoder).to receive(:available?).and_return(true)
      allow(VideoWalkthroughs::Gemini::VideoTranscoder).to receive(:to_compact_mp4).and_return(false)
      walkthrough = create_walkthrough
    ...
    it "still ends analyzed (keeping the original) when the transcode raises" do
      allow(VideoWalkthroughs::Gemini::VideoTranscoder).to receive(:available?).and_return(true)
      allow(VideoWalkthroughs::Gemini::VideoTranscoder).to receive(:to_compact_mp4).and_raise(RuntimeError.new("ffmpeg blew up"))
      walkthrough = create_walkthrough

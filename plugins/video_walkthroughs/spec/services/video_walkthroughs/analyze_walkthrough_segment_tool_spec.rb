...
# VideoWalkthroughs::AnalysisJob.client_factory).
...
RSpec.describe VideoWalkthroughs::AnalyzeWalkthroughSegmentTool do
  ...
  describe "scoping + state guards" do
    ...
    it "maps Gemini rate limiting to a retry-friendly tool error" do
      allow(fake_client).to receive(:analyze_segment).and_raise(VideoWalkthroughs::Gemini::Client::RateLimited.new("429"))
      walkthrough = create_walkthrough
  ...
  describe "rate-limit low-resolution fallback" do
    it "retries the segment once at low resolution when the full-res pass is rate-limited" do
      low_res_client = FakeSegmentClient.new(
        ...
        raise_on_analyze: [ VideoWalkthroughs::Gemini::Client::RateLimited.new("429"), nil ]
      )
    ...
    it "surfaces the quota error only after the low-res retry also rate-limits" do
      exhausted_client = FakeSegmentClient.new(
        raise_on_analyze: [ VideoWalkthroughs::Gemini::Client::RateLimited.new("429"), VideoWalkthroughs::Gemini::Client::RateLimited.new("429") ]
      )
  ...
  describe "registration" do
    # The tool moved out of core's McpToolRegistry when the plugin was
    # extracted, so what it now has to prove is that the plugin's tool set
    # advertises it -- and only at the deferred tier, since a chat with no
    # walkthrough in it should not spend prompt budget on the schema.
    it "is advertised by the plugin's chat tool set at the deferred tier only" do
      names = VideoWalkthroughs::ChatToolSet.tool_definitions(tier: :deferred).map { |tool| tool[:name] }

      expect(names).to include("analyze_walkthrough_segment")
      expect(VideoWalkthroughs::ChatToolSet.available_for?(nil, tier: :deferred)).to be true
      expect(VideoWalkthroughs::ChatToolSet.available_for?(nil, tier: :essential)).to be false
    end
  end
end

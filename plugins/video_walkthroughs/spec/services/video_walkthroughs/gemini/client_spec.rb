...
RSpec.describe VideoWalkthroughs::Gemini::Client do
  ...
  describe "#resolve_video_model!" do
    ...
    it "raises Error when no video-capable model is available to the project" do
      ...
      expect { client.resolve_video_model! }.to raise_error(VideoWalkthroughs::Gemini::Client::Error, /no video-capable Gemini model/)
    end
  end
  ...
  describe "transport failures" do
    it "wraps a list_models transport failure as ConnectionError, not a raw error" do
      ...
      expect { client.list_models }.to raise_error(VideoWalkthroughs::Gemini::Client::ConnectionError, /could not reach Gemini/)
    end
    ...
    it "wraps a generate_content transport failure as ConnectionError, not a raw error" do
      ...
      }.to raise_error(VideoWalkthroughs::Gemini::Client::ConnectionError, /could not reach Gemini/)
    end
  end
  ...
  describe "#upload_file" do
    ...
    it "raises when Gemini does not return a resumable upload URL" do
      ...
      }.to raise_error(VideoWalkthroughs::Gemini::Client::Error, /did not return a resumable upload URL/)
    end
  end
  ...
  describe "#wait_until_active" do
    ...
    it "raises FileProcessingFailed when the file lands in state FAILED" do
      ...
      }.to raise_error(VideoWalkthroughs::Gemini::Client::FileProcessingFailed, /could not process the video/)
    end
    ...
    it "raises when the file never turns ACTIVE before the deadline" do
      ...
      }.to raise_error(VideoWalkthroughs::Gemini::Client::Error, /timed out waiting/)
    end
  end
  ...
  describe "#generate_content" do
    ...
    it "raises AuthError when the key is rejected" do
      [ 401, 403 ].each do |status|
        ...
        expect { generate }.to raise_error(VideoWalkthroughs::Gemini::Client::AuthError, "API key not valid.")
      end
    end
    ...
    it "raises RateLimited on 429" do
      ...
      expect { generate }.to raise_error(VideoWalkthroughs::Gemini::Client::RateLimited, "Resource has been exhausted.")
    end

    it "raises when the reply text is not valid JSON despite the schema" do
      stub_generate(text: "not json {")

      expect { generate }.to raise_error(VideoWalkthroughs::Gemini::Client::Error, /malformed JSON/)
    end
    ...
    it "includes the finish reason when the parts carry no text" do
      ...
      expect { generate }.to raise_error(VideoWalkthroughs::Gemini::Client::Error, /empty analysis.*SAFETY/)
    end
  end
  ...
  describe "#generate_text" do
    ...
    it "raises when the reply text is not valid JSON" do
      ...
      expect { client.generate_text(prompt: "x", response_schema: response_schema) }
        .to raise_error(VideoWalkthroughs::Gemini::Client::Error, /malformed JSON/)
    end
  end
  ...
  describe "#analyze_segment" do
    ...
    it "maps a 429 to RateLimited like the whole-video path" do
      ...
      expect { analyze_segment }.to raise_error(VideoWalkthroughs::Gemini::Client::RateLimited)
    end
  end
end

...
RSpec.describe VideoWalkthroughs::ReadWalkthroughFrameTool do
  ...
  def frame(seconds:, jpeg:)
    VideoWalkthroughs::Gemini::FrameExtractor::Frame.new(seconds: seconds, label: "x", jpeg: jpeg)
  end
  ...
  def stub_extract(return_frames)
    ...
    allow(VideoWalkthroughs::Gemini::FrameExtractor).to receive(:extract) do |video_path:, timestamps:, **|
  ...
  describe "clamping" do
    ...
    it "rejects an unparseable timestamp before touching the video" do
      ...
      expect(VideoWalkthroughs::Gemini::FrameExtractor).not_to have_received(:extract)
    end
  end
  ...
  describe "expired / unreadable video" do
    it "reports the video expired when the stored blob is gone (unattached)" do
      allow(VideoWalkthroughs::Gemini::FrameExtractor).to receive(:extract)
      walkthrough = create_walkthrough
  ...
  describe "registration" do
    ...
    it "is advertised by the plugin's chat tool set at the deferred tier only" do
      ...
      expect(names).to include("read_walkthrough_frame")

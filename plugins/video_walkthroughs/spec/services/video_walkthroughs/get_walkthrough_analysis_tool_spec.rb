...
RSpec.describe VideoWalkthroughs::GetWalkthroughAnalysisTool do
  ...
  def frame(seconds:, label:, jpeg:)
    VideoWalkthroughs::Gemini::FrameExtractor::Frame.new(seconds: seconds, label: label, jpeg: jpeg)
  end

  describe "happy path" do
    it "returns the report text plus a labeled image block per extracted frame" do
      allow(VideoWalkthroughs::Gemini::FrameExtractor).to receive(:extract).and_return([
    ...
    it "prioritizes flagged frames at top JPEG quality" do
      ...
      expect(flagged).to include(flagged: true, jpeg_quality: VideoWalkthroughs::Gemini::FrameExtractor::HIGH_JPEG_QUALITY)
      expect(plain).to include(flagged: false, jpeg_quality: VideoWalkthroughs::Gemini::FrameExtractor::JPEG_QUALITY)
    end
  end
  ...
  describe "degradation" do
    ...
    it "returns text only when the stored video was pruned" do
      allow(VideoWalkthroughs::Gemini::FrameExtractor).to receive(:extract).and_call_original
      walkthrough = create_walkthrough
    ...
    it "degrades to text when extraction raises" do
      allow(VideoWalkthroughs::Gemini::FrameExtractor).to receive(:extract).and_raise(RuntimeError.new("ffmpeg exploded"))
      content = call_tool(walkthrough_id: create_walkthrough.id).dig(:result, :content)
      expect(content.select { |b| b[:type] == "image" }).to be_empty

...
that file is larger than Muse's startup context limit, Syrus refuses the
invocation before launch with `muse_rules_context_too_large` instead of letting
the run degrade into missing MCP tools. MCP server configuration still comes
from the isolated Muse settings file, not from the rules file.

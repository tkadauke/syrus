...
RSpec.describe MuseInvocation do
  ...
  # Optional CI smoke test: the Ruby specs above are deterministic and don't
  # depend on the muse binary being installed. When it IS available, also
  # confirm the real binary accepts the settings file we generate -- this is
  # what actually caught the original bug (`missing field schema_version`),
  # which no amount of pure-Ruby JSON parsing coverage would have caught on
  # its own. Keep it out of focused/review runs so the external CLI does not
  # make touched unit specs look flaky.
  if system("which muse >/dev/null 2>&1")
    it "produces a settings.json the installed muse binary does not reject as malformed", :ci_only do
      Dir.mktmpdir("muse-home") do |muse_home|
        Dir.mktmpdir("muse-workspace") do |workspace|

...
const CONTENT_COLOR_HELPERS = new Set(["diffCoverageBorderClass", "diffLineClass", "diffGutterClass", "diffMarkerClass"])
...
module.exports = {
  ...
  create(context) {
    ...
    const matches = []
    const inspectClassHelpers = isFrontendRouteFile(relative) || isFrontendLibFile(relative)
    ...
    function inspectExportedClassHelper(node) {
      ...
      if (!name || !CLASS_HELPER_NAME_PATTERN.test(name) || CONTENT_COLOR_HELPERS.has(name)) return
...
function isFrontendRouteFile(relative) {
  return /^app\/frontend\/routes\/.*\.tsx?$/.test(relative)
}

...
tester.run("no-legacy-color-tokens", rule, {
  valid: [
    ...
    {
      code: "export function localNavClass(active: boolean) { return active ? 'bg-brand/10 text-brand' : 'text-gray-700' }",
      filename: path.join(ROOT, "app/frontend/routes/chat/workspaceTabs.ts")
    }
  ],
  invalid: [
    {
    ...
    {
      code: "export function workspaceTabClass(active: boolean) { return active ? 'border-blue-600 text-blue-700' : 'text-gray-700' }",
      filename: path.join(ROOT, "app/frontend/routes/chat/workspaceTabs.ts"),
      errors: [{ messageId: "forbidden" }]
    }
  ]

class AddSoftDeletionToChatSessions < ActiveRecord::Migration[8.1]
  def up
    ...
    unless column_exists?(:chat_sessions, :deleted_by_user_id)
      add_reference :chat_sessions, :deleted_by_user, null: true, foreign_key: { to_table: :users }
    end
  ...
  def down
    ...
    remove_reference :chat_sessions, :deleted_by_user, foreign_key: { to_table: :users } if column_exists?(:chat_sessions, :deleted_by_user_id)

class AddOwnerUserToDashboardRecords < ActiveRecord::Migration[8.1]
  def up
    add_reference :jobs, :owner_user, type: :bigint, null: true, foreign_key: { to_table: :users } unless column_exists?(:jobs, :owner_user_id)
    add_reference :epics, :owner_user, type: :bigint, null: true, foreign_key: { to_table: :users } unless column_exists?(:epics, :owner_user_id)
  end

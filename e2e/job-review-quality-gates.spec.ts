...
test("covers review diff, coverage, diff comments, and workflow warning action", async ({ page }) => {
  ...
  await page.getByLabel("Whole-review comment").fill("E2E review note: verify the seeded diff quality gate.")
  await page.getByRole("button", { name: "Comment", exact: true }).click()
  await expect(page.getByText("E2E review note: verify the seeded diff quality gate.")).toBeVisible()

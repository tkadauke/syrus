...
| `app/controllers/api/v1/app/setup_controller.rb` | per-user/private | Setup status and actions are computed for the signed-in user so onboarding reflects that user's credentials, credential checks, repositories, onboarding state, first-run progress, and provider configuration. |

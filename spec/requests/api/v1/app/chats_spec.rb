...
RSpec.describe "API: /api/v1/app/chats", :ci_only, type: :request do
  ...
  describe "DELETE /api/v1/app/chats/:id" do
    it "soft-deletes the chat, keeps dependent rows for audit, cleans search index/workspace/agent homes via the worker job, and hides it from listing/show" do
      ...
      expect(Whiteboard::Board.exists?(whiteboard.id)).to be(true)
  ...
  it "serializes a heavy chat payload without repeated count and goal queries" do
    sign_in_as(user)
    chat = ChatSession.create!(user: user, repository: repository, last_message_at: Time.current)
    3.times do |index|
    ...
    chat.chat_attachments.create!(attachable: repository)
    chat.chat_proposals.create!(slug: "optimize-chat", title: "Optimize chat", body: "Trim payload SQL.", repository: repository)
  ...
  it "forces the active-tail index for MySQL chat message pagination" do
    chat = ChatSession.create!(user: user, repository: repository, last_message_at: Time.current)
    controller = Api::V1::App::ChatsController.new
    ...
    expect(sql).to include("FORCE INDEX (idx_chat_messages_active_tail)")
    expect(sql).to include("\"chat_messages\".\"deleted_at\" IS NULL")
  end
  ...
  it "rejects proposal media refs from another chat on update" do
    ...
    other_snapshot = Whiteboard::Snapshot.create!(
  ...
  describe "GET /api/v1/app/chats/:id/media" do
    it "returns snapshots and chat images for the chat session" do
      ...
      expect(body["chat_images"]).to contain_exactly(
        include(
          ...
          "file_path" => "/api/v1/app/chats/#{chat.id}/media/chat_images/#{doc.id}/file",
          "image_url" => "/api/v1/app/repository_documents/#{doc.id}/file"
        )
      )
    end

    it "serves chat image files through the chat-scoped media path" do
      sign_in_as(user)
      chat = ChatSession.create!(user: user, repository: repository, last_message_at: Time.current)
      ...
      get "/api/v1/app/chats/#{chat.id}/media/chat_images/#{doc.id}/file"
    ...
    context "with the syrus-rails plugin registered" do
      it "includes renderer_type for a submitted rails_schema_erd typed artifact" do
        registered_before = Syrus::PluginRegistry.registered_names.include?("syrus-rails")

        begin
          unless registered_before
          ...
          Syrus::PluginRegistry.reset! unless registered_before
        end
      end
    end
  end
  ...
  it "serves Markdown preview panel files as text even when Marcel reports octet-stream" do
    sign_in_as(user)
    chat = ChatSession.create!(user: user, last_message_at: Time.current)
    panel = PreviewPanel::Service.open!(chat_session: chat, title: "Layout mockup", files: { "notes.mkdn" => "# Notes" }, entry_file: "notes.mkdn")

    get "/api/v1/app/chats/#{chat.id}/preview_panels/#{panel.id}/files/notes.mkdn"

    expect(response).to have_http_status(:ok)
    expect(parse_body).to include("content" => "# Notes", "binary" => false)
  end
  ...
  it "confirms and rejects proposals through the app API" do
    ...
    expect(parse_body["pending_proposal_count"]).to eq(0)

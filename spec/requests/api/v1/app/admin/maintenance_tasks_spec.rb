...
RSpec.describe "API: /api/v1/app/admin/maintenance_tasks", type: :request do
  ...
  it "paginates task log events 100 at a time" do
    ...
    sign_in_as(admin)
    task = create_task_with_events(105)
  ...
  it "clamps task log pagination to the last available page" do
    admin = Factories.user(admin: true)
    sign_in_as(admin)
    task = create_task_with_events(105)

    get "/api/v1/app/admin/maintenance_tasks/#{task.id}", params: { events_page: 99 }
  ...
  def create_task_with_events(count)

...
RSpec.describe "API: /api/v1/app/repositories/:repository_id/skills", type: :request do
  ...
  describe "GET index" do
    ...
    it "lists the built-in skill when the repo has no .syrus/skills overrides" do
      ...
      expect(parse_body["skills"].map { |s| s["name"] }).to eq([ "add-ci-workflow", "changelog-generate", "coverage-gap-report", "dead-code-sweep", "debug", "dependency-audit", "explain-failing-ci", "init-docs", "investigate", "license-audit", "onboard-to-syrus", "security-review" ])

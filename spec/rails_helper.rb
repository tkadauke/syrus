...
require 'spec_helper'
# RSpec must never inherit the app server/worker environment. Syrus repeat
# gates invoke file-scoped specs from production workers, so defaulting with
# ||= would boot the application against the wrong database.
ENV['RAILS_ENV'] = 'test'

...
RSpec.describe Skills do
  ...
  describe ".all_for" do
    ...
    it "lists only built-in skills when the repo has no .syrus/skills directory" do
      ...
      expect(resolutions.map { |r| r.definition.name }).to eq([ "debug", "explain-failing-ci", "investigate", "onboard-to-syrus" ])
    ...
    it "includes a repo-local skill alongside built-ins, unshadowed" do
      ...
      expect(by_name.keys.sort).to eq([ "audit", "debug", "explain-failing-ci", "investigate", "onboard-to-syrus" ])

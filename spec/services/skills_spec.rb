...
RSpec.describe Skills do
  ...
  describe ".all_for" do
    ...
    it "lists only built-in skills when the repo has no .syrus/skills directory" do
      ...
      expect(resolutions.map { |r| r.definition.name }).to eq([ "add-ci-workflow", "changelog-generate", "coverage-gap-report", "dead-code-sweep", "debug", "dependency-audit", "explain-failing-ci", "init-docs", "investigate", "license-audit", "onboard-to-syrus", "security-review" ])
    ...
    it "includes a repo-local skill alongside built-ins, unshadowed" do
      ...
      expect(by_name.keys.sort).to eq([ "add-ci-workflow", "audit", "changelog-generate", "coverage-gap-report", "dead-code-sweep", "debug", "dependency-audit", "explain-failing-ci", "init-docs", "investigate", "license-audit", "onboard-to-syrus", "security-review" ])

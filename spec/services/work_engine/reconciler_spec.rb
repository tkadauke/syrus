...
RSpec.describe WorkEngine::Reconciler, :ci_only do
  ...
  it "declines to defer a landing Job through the executor precondition re-check when active landing work exists" do
    ...
    stale_plan = WorkEngine::RepairPlanner::Plan.new(
      ...
      preconditions: {},
      reason: "stale plan computed before the landing WorkUnit was visible"
    )

    execution = WorkEngine::RepairExecutor::Policies::DeferOrphanedLandingJob.new(plan: stale_plan, now: Time.current).execute
  ...
  it "wakes the landing dispatcher for requested landing-owned WorkIntents instead of launching them directly" do
    ...
    train = MergeTrain.create!(epic: epic, repository: job.repository, base_branch: "main")
    ...
    old_workflow = WorkUnits::Launcher.instantiate(kind: "merge_train", job: second, artifacts: { "merge_train_id" => train.id })
    old_intent = old_workflow.work_unit.work_intent
    old_unit = old_workflow.work_unit
  ...
  it "cancels requested WorkIntents left behind by superseded WorkUnits" do
    workflow = job.latest_workflow
    unit = workflow.work_unit
    intent = unit.work_intent
    unit.update_columns(
    ...
    expect(kind(result, :superseded_work_unit_uncancelled_intent)).to have_attributes(
      severity: "error",
      safe_to_auto_repair: true,
      recommended_repair_action: "cancel_work_intent_from_superseded_work_unit"
    )
    expect(plan(result, :cancel_work_intent_from_superseded_work_unit)).to have_attributes(
      auto_executable: true,
      target_type: "WorkIntent",
      target_id: intent.id
    )
  ...
  it "does not cancel a WorkIntent from a superseded WorkUnit while another sibling WorkUnit remains active" do
    workflow = job.latest_workflow
    unit = workflow.work_unit
    intent = unit.work_intent
    # update! (not update_columns) so active_dedup_key  clears
    ...
    expect(kind(result, :superseded_work_unit_uncancelled_intent)).to be_nil
    expect(intent.reload).to be_requested
  end

  it "fails requested WorkIntents left behind by failed WorkUnits" do
    workflow = job.latest_workflow
    unit = workflow.work_unit
    intent = unit.work_intent
    ...
    expect(kind(result, :failed_work_unit_unfailed_intent)).to have_attributes(
      severity: "error",
      safe_to_auto_repair: true,
      recommended_repair_action: "fail_work_intent_from_failed_work_unit"
    )
    expect(plan(result, :fail_work_intent_from_failed_work_unit)).to have_attributes(
      auto_executable: true,
      target_type: "WorkIntent",
      target_id: intent.id
    )
  ...
  it "does not fail a WorkIntent from a failed WorkUnit while another sibling WorkUnit remains active" do
    workflow = job.latest_workflow
    unit = workflow.work_unit
    intent = unit.work_intent
    # update! (not update_columns) so active_dedup_key  clears
    ...
    expect(kind(result, :failed_work_unit_unfailed_intent)).to be_nil
    expect(intent.reload).to be_requested
  end
  ...
  it "opens a repeated-failure circuit instead of rebuilding a merge train after identical diagnostics recur" do
    ...
    expect(AttentionItem.open_decisions.where(problem_code: "repeated_failure_circuit_open", job: job).count).to eq(1)
  end
  ...
  it "resets the repeated-failure circuit when the app revision changes" do
    ...
    expect(AttentionItem.where(problem_code: "repeated_failure_circuit_open", job: job)).to be_empty
  end

  it "schedules retryable failures with escalating backoff by attempt number" do
    now = Time.zone.parse("2026-08-01 10:00:00 UTC")

    AutoRetryAttempt::BACKOFFS.each_with_index do |backoff, prior_attempt_count|
      retry_job = Factories.job(agent_provider: "claude")
      retry_job.update!(job_provider_setting: "claude")
      retry_workflow = retry_job.latest_workflow
      retry_step = retry_workflow.first_step
      retry_run = retry_step.runs.first

      retry_step.update_columns(kind: "grader", state: "failed", finished_at: now)
      retry_workflow.update_columns(state: "failed", finished_at: now, cleaned_up_at: nil)
      retry_run.update_columns(state: "failed", finished_at: now)
      RunFailureClassification.create!(
        run: retry_run,
        classification: "timeout",
        retryable: true,
      ...
      prior_attempt_count.times do |index|
        ...
        AutoRetryAttempt.create!(
          ...
          performed_at: now - 30.minutes
        )
      end
      allow(File).to receive(:directory?).and_call_original
      allow(File).to receive(:directory?).with(WorkflowWorkspace.path_for(retry_workflow)).and_return(true)

      expect {
        reconcile_and_execute(run_id: retry_run.id, now: now)
      }.to change { AutoRetryAttempt.where(job: retry_job, skipped_reason: nil).count }.by(1)
        .and have_enqueued_job(AutoRetryJob)

      attempt = AutoRetryAttempt.where(job: retry_job).order(:id).last
      expect(attempt).to have_attributes(attempt_number: prior_attempt_count + 1)
      expect(attempt.scheduled_at.to_i).to eq((now + backoff).to_i)
    end
  end

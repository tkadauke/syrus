  ...
  describe ".instantiate(job:)" do
    ...
    it "inserts the adversarial review loop before the grade loop for Initial when enabled" do
      ...
      expect(wf.steps.pluck(:kind, :position, :iteration, :loop_id)).to eq([
        ...
        [ "format", 3, 1, grade_loop_id ],
        [ "generate", 4, 1, grade_loop_id ],
      ...
      expect(wf.steps.where.not(loop_id: nil).pluck(:kind)).to eq(%w[ adversarial_review format generate grader_fanout grader_collect ])
    end
    ...
    it "records the configured adversarial review round budget in the chain template" do
      ...
      expect(wf.chain_template.third).to eq(
        {
          "type" => "loop",
          "max_iterations" => 2,
          "steps" => %w[ implement adversarial_review ]
    ...
    it "omits prepare from Initial when the Job has opted out" do
      ...
      expect(wf.steps.pluck(:kind, :position)).to eq([
        ...
        [ "coverage_analyze", 5 ], [ "dependency_audit", 6 ], [ "summarize", 7 ], [ "test_plan", 8 ], [ "pr_open", 9 ]
      ])
      expect(wf.steps.where.not(loop_id: nil).pluck(:kind)).to eq(%w[ format generate grader_fanout grader_collect ])
    ...
    it "omits prepare from Retry when the Job has opted out" do
      ...
      expect(wf.steps.pluck(:kind)).to eq(%w[ implement format generate grader_fanout grader_collect coverage_analyze dependency_audit summarize test_plan pr_open ])
      expect(wf.steps.where.not(loop_id: nil).pluck(:kind)).to eq(%w[ format generate grader_fanout grader_collect ])
      expect(wf.trigger_kind).to eq("retry")
    end
    ...
    it "rejects nested Try declarations returned from steps_for" do
      workflow_class = Class.new(Workflows::Base) do
        def self.steps_for(_job)
          [
            Workflows::Loop.new(
              max_iterations: 3,
              steps: [ :implement, Workflows::Try.new(:push) ]
            )
          ]
        end
    ...
    it "rejects direct serialization of loop templates with a nested Try node" do
      loop = Workflows::Loop.new(
        max_iterations: 3,
        steps: [ :implement, Workflows::Try.new(:push) ]
      )
    ...
    it "rejects RetryUntil declarations with a nested Try node" do
      expect do
        Workflows::RetryUntil.new(
          repair: [ :implement ],
          check: [ Workflows::Try.new(:push) ]
        )
      end.to raise_error(ArgumentError, /nested workflow control nodes/)
    end

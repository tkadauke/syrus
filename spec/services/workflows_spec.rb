  ...
  describe ".instantiate(job:)" do
    it "creates the workflow + chain for Initial in transaction" do
      ...
      expect(wf.steps.pluck(:kind, :position)).to eq([
        [ "prepare", 0 ], [ "implement", 1 ], [ "autofix", 2 ], [ "grader_fanout", 3 ], [ "grader_collect", 4 ],
    ...
    it "inserts the adversarial review loop before the grade loop for Initial when enabled" do
      ...
      expect(wf.steps.pluck(:kind, :position, :iteration, :loop_id)).to eq([
        ...
        [ "autofix", 4, 1, grade_loop_id ],
        [ "grader_fanout", 5, 1, grade_loop_id ],
        [ "grader_collect", 6, 1, grade_loop_id ],
        [ "coverage_analyze", 7, 1, nil ],
        [ "dependency_audit", 8, 1, nil ],
        [ "summarize", 9, 1, nil ],
        [ "test_plan", 10, 1, nil ],
        [ "pr_open", 11, 1, nil ],
        [ "review_plan", 12, 1, nil ]
      ])
      ...
      expect(wf.steps.where.not(loop_id: nil).pluck(:kind)).to eq(%w[ implement adversarial_review implement autofix grader_fanout grader_collect ])
    end
    ...
    it "keeps the Initial chain unchanged when adversarial review rounds is zero" do
      ...
      expect(wf.steps.pluck(:kind, :position, :iteration)).to eq([
        ...
        [ "grader_fanout", 3, 1 ],
        [ "grader_collect", 4, 1 ],
        [ "coverage_analyze", 5, 1 ],
        [ "dependency_audit", 6, 1 ],
        [ "summarize", 7, 1 ],
        [ "test_plan", 8, 1 ],
        [ "pr_open", 9, 1 ],
        [ "review_plan", 10, 1 ]
    ...
    it "omits prepare from Initial when the Job has opted out" do
      ...
      expect(wf.steps.pluck(:kind, :position)).to eq([
        [ "implement", 0 ], [ "autofix", 1 ], [ "grader_fanout", 2 ], [ "grader_collect", 3 ],
        [ "coverage_analyze", 4 ], [ "dependency_audit", 5 ], [ "summarize", 6 ], [ "test_plan", 7 ], [ "pr_open", 8 ], [ "review_plan", 9 ]
      ])
      expect(wf.steps.where.not(loop_id: nil).pluck(:kind)).to eq(%w[ implement autofix grader_fanout grader_collect ])
    ...
    it "omits prepare from Retry when the Job has opted out" do
      ...
      expect(wf.steps.pluck(:kind)).to eq(%w[ implement autofix grader_fanout grader_collect coverage_analyze dependency_audit summarize test_plan pr_open review_plan ])
      expect(wf.steps.where.not(loop_id: nil).pluck(:kind)).to eq(%w[ implement autofix grader_fanout grader_collect ])
      expect(wf.trigger_kind).to eq("retry")
    end
    ...
    it "wires next_step_id top-down (linear chain)" do
      wf = Workflows::Initial.instantiate(job: job)
      a, b, c, d, e, f, g, h, i, j, k = wf.steps.order(:position)
    ...
    it "instantiates PrFeedback with respond → grader_fanout → grader_collect → summarize_amend → refresh_job_metadata → try(push)" do
      ...
      expect(wf.steps.pluck(:kind)).to eq(%w[ prepare respond autofix grader_fanout grader_collect coverage_analyze coverage_pr_comment dependency_audit dependency_audit_pr_comment summarize_amend refresh_job_metadata push ])
      expect(wf.steps.where.not(loop_id: nil).pluck(:kind)).to eq(%w[ respond autofix grader_fanout grader_collect ])
      expect(wf.chain_template).to include(
        {
          "type" => "retry_until",
          "max_iterations" => AppSetting.grade_max_iterations,
          "repair" => %w[ respond autofix ],
          "check" => %w[ grader_fanout grader_collect ],
          "repair_first" => true
        }
      )

...
RSpec.describe TouchedTestRepeatGate do
  ...
  describe "building the rerun command without BaseRevisionRetry's base_retry opt-in" do
    ...
    it "builds a single-file command for plugin-scoped RSpec graders and records the grader environment" do
      step = grader_step_with({
        "name" => "plugins-throughput-rspec",
        "command" => 'RAILS_ENV=${RAILS_ENV:-test} COVERAGE=false bundle exec rspec plugins/throughput/spec',
        "grader_framework" => "rspec"
      })
    ...
    it "runs focused provider commands with relevant inline environment from the owning grader" do
      stub_focused_command("test \"$RAILS_ENV:$COVERAGE:$BUNDLE_PATH\" = \"test:false:#{@dir}/vendor/bundle\"")
      step = grader_step_with({
        "name" => "plugins-throughput-rspec",
        "command" => 'RAILS_ENV=${RAILS_ENV:-test} COVERAGE=false BUNDLE_PATH=$PWD/vendor/bundle bundle exec rspec plugins/throughput/spec',
        "grader_framework" => "rspec"
      })

...
RSpec.describe RetryFailedStepEnqueuer do
  ...
  it "raises a clear retry error when the selected fanout cannot be reopened" do
    ...
    fanout.runs.create!(job: job, trigger_kind: "initial", state: "succeeded", finished_at: 2.minutes.ago)
    ...
    end.to raise_error(described_class::Error, "Step #{fanout.id} cannot be reopened from succeeded")
  end

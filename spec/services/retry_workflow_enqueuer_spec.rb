...
RSpec.describe RetryWorkflowEnqueuer do
  ...
  it "syncs skip-prepare from the source issue before instantiating the workflow" do
    ...
    expect(result.workflow.steps.order(:position).pluck(:kind)).to eq(%w[ implement autofix grader_fanout grader_collect coverage_analyze dependency_audit summarize test_plan pr_open review_plan ])
  end

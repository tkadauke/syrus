...
RSpec.describe Admin::McpToolCardCoverage, :reset_plugin_registry do
  ...
  let(:plugin_tool_set) do
    Class.new do
      ...
      def self.available_for?(_chat_session, tier:)
        %i[essential deferred].include?(tier)
      end
  ...
  it "uses the chat-session-aware plugin advertisement path for owner metadata" do
    ...
    session_dependent_tool_set = Class.new do
      ...
      def self.tool_definitions(tier:, chat_session: nil)
        return [] unless tier == :essential
        return [] unless chat_session&.user&.admin?

        [
          { name: "admin_session_tool", description: "Admin session tool", input_schema: {}, mutation: true }
        ]
      end
    end
    deferred_only_tool_set = Class.new do
      include Syrus::Plugin::ChatMcpToolSet
      ...
      def self.tool_definitions(tier:)
        return [] unless tier == :deferred

        [
          { name: "deferred_only_tool", description: "Deferred only tool", input_schema: {} }
        ]
      end
    end
    ...
    Syrus::PluginRegistry.register(
      ...
      version: "1.0.0",
      provides: { chat_mcp_tool_set: deferred_only_tool_set }
    )

    report = described_class.call(
      usages: McpToolUsage.none,
      advertised_tools: %w[admin_session_tool deferred_only_tool],
      chat_session: chat_session
    )

    expect(report.fetch(:classified_tools)).to include(
      include(
        tool_name: "admin_session_tool",
        owner_type: "plugin",
        owner_name: "session_plugin",
        recommendation_target: "plugin:session_plugin",
        tier: "essential",
        mutation: true
      ),
      include(
        tool_name: "deferred_only_tool",
        owner_type: "plugin",
        owner_name: "deferred_plugin",
        recommendation_target: "plugin:deferred_plugin",
        tier: "deferred",
        classification: "intentionally_obscure_deferred"
      )
    )
  end

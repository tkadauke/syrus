...
RSpec.describe WorkflowMcpTransportSelector do
  let(:host) { "127.0.0.1" }
  let(:port) { 48_105 }
  ...
  describe "#select" do
    ...
    context "with the feature enabled" do
      ...
      it "falls back to stdio with a daemon_incompatible reason when the daemon lacks workflow_tools capability" do

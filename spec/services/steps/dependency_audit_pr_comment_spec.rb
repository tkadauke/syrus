...
RSpec.describe Steps::DependencyAuditPrComment do
  let(:job) { Factories.job }
  let(:workflow) { job.workflows.last }
  let(:step) do
    Step.create!(workflow: workflow, kind: "dependency_audit_pr_comment", position: 99)
  end
  let(:run) do
  ...
  let(:pr_comment_body) do
    "#{DependencyAuditReport::PrCommentFormatter::MARKER}\n## Dependency Vulnerability Scan\n\nsomething flagged"
  end
  ...
  context "when the dependency_audit artifact has no pr_comment_body" do
    ...
    it "skips gracefully when no artifact was set at all (no lockfile changed)" do
      expect { handler.call }.not_to raise_error
    end

    it "skips gracefully when the scan was clean" do
      workflow.set_artifact!("dependency_audit", { "results" => [ { "ecosystem" => "Ruby", "clean" => true } ] })
      expect(fake_client).not_to receive(:add_issue_comment)
      handler.call
    end
  end
  ...
  context "when pr_comment_body is set but no PR exists" do
    before do
      workflow.set_artifact!("dependency_audit", { "pr_comment_body" => pr_comment_body })
    end
  ...
  context "when pr_comment_body is set and PR exists" do
    ...
    context "when no existing dependency audit comment is found" do
      before do
    ...
    context "when an existing dependency audit comment is found" do
      let(:existing_comment) do
        double("comment", id: 777,
               body: "#{DependencyAuditReport::PrCommentFormatter::MARKER}\nold content")
      end

...
RSpec.describe Mcp::Tools::RuntimeSnapshotTool do
  ...
  it "delegates to the provider's snapshot capability" do
    session
    body = payload(call(options: { target: "e1" }))

    expect(body).to eq(snapshot: true, options: { target: "e1" })
  end

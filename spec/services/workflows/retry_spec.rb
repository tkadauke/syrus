...
RSpec.describe Workflows::Retry do
  ...
  context "when visual review is enabled" do
    ...
    it "inserts an implement/visual_review loop before the grader retry chain" do
      ...
      expect(workflow.steps.order(:position).pluck(:kind)).to eq(
        %w[ prepare implement visual_review implement format generate grader_fanout grader_collect coverage_analyze dependency_audit summarize test_plan pr_open ]
      )
    end
  end
  ...
  context "when adversarial review is enabled" do
    ...
    it "inserts an implement/adversarial_review loop before the grader retry chain" do
      ...
      expect(workflow.steps.order(:position).pluck(:kind)).to eq(
        %w[ prepare implement adversarial_review implement format generate grader_fanout grader_collect coverage_analyze dependency_audit summarize test_plan pr_open ]
      )
    end
  ...
  context "when adversarial and visual review are both enabled" do
    ...
    it "places adversarial review before visual review and the grader retry chain" do
      ...
      expect(workflow.steps.order(:position).pluck(:kind)).to eq(
        %w[ prepare implement adversarial_review implement visual_review implement format generate grader_fanout grader_collect coverage_analyze dependency_audit summarize test_plan pr_open ]
      )
    end

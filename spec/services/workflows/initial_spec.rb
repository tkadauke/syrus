...
RSpec.describe Workflows::Initial do
  ...
  before do
    allow(RepoDefaultBranchSyrusYml).to receive(:for_job).and_return(
      RepoDefaultBranchSyrusYml::Result.new(config: nil, source: "none", note: "stubbed")
    )
  ...
  context "when only formatters is configured" do
    ...
    it "shows format but not generate, and still shows the grade loop's grader_fanout/grader_collect" do
      ...
      expect(workflow.steps.order(:position).pluck(:kind)).to eq(
        %w[ prepare implement format grader_fanout grader_collect coverage_analyze dependency_audit summarize test_plan pr_open ]
      )
    end
  end
  ...
  context "when only grade is configured" do
    ...
    it "shows grader_fanout/grader_collect without format or generate" do
      ...
      expect(workflow.steps.order(:position).pluck(:kind)).to eq(
        %w[ prepare implement grader_fanout grader_collect coverage_analyze dependency_audit summarize test_plan pr_open ]
      )
    end
  end
  ...
  context "when visual review is enabled" do
    ...
    it "inserts an implement/visual_review loop before the grader retry chain" do
      ...
      expect(workflow.steps.order(:position).pluck(:kind)).to eq(
        %w[ prepare implement implement visual_review format generate grader_fanout grader_collect coverage_analyze dependency_audit summarize test_plan pr_open ]
      )
    end
    ...
    context "and adversarial review is also enabled" do
      ...
      it "places the visual_review loop after the adversarial_review loop and before the grader retry chain" do
        ...
        expect(workflow.steps.order(:position).pluck(:kind)).to eq(
          %w[ prepare implement adversarial_review implement visual_review format generate grader_fanout grader_collect coverage_analyze dependency_audit summarize test_plan pr_open ]
        )
      end
    end
  end
  ...
  context "when adversarial review is enabled but no grade loop is configured" do
    ...
    it "does not add a second implement step for the unconfigured grade loop" do
      ...
      expect(workflow.steps.order(:position).pluck(:kind)).to eq(
        %w[ prepare implement adversarial_review coverage_analyze dependency_audit summarize test_plan pr_open ]
      )

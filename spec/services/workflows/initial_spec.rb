...
RSpec.describe Workflows::Initial do
  ...
  it "materializes the standard chain with coverage_analyze always present" do
    ...
    expect(workflow.steps.order(:position).pluck(:kind)).to eq(
      %w[ prepare implement autofix grader_fanout grader_collect coverage_analyze dependency_audit summarize test_plan pr_open review_plan ]
    )
  end
  ...
  context "when visual review is enabled" do
    ...
    it "inserts an implement/visual_review loop before the grader retry chain" do
      ...
      expect(workflow.steps.order(:position).pluck(:kind)).to eq(
        %w[ prepare implement visual_review implement autofix grader_fanout grader_collect coverage_analyze dependency_audit summarize test_plan pr_open review_plan ]
      )
    end
    ...
    context "and adversarial review is also enabled" do
      ...
      it "places the visual_review loop after the adversarial_review loop and before the grader retry chain" do
        ...
        expect(workflow.steps.order(:position).pluck(:kind)).to eq(
          %w[ prepare implement adversarial_review implement visual_review implement autofix grader_fanout grader_collect coverage_analyze dependency_audit summarize test_plan pr_open review_plan ]
        )
      end
    end
  end

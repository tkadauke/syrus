...
RSpec.describe ChatQueuedMessagePromoter do
  ...
  describe "#deliver_one_if_idle!" do
    ...
    it "batches ready goal continuations with adjacent system notices" do
      goal = ChatGoal.create!(
        chat_session: chat,
        user: user,
        prompt: "Keep going",
        mode_snapshot: { "mode" => "ask" }
      )
      ...
      expect(message.content["notices"]).to contain_exactly(
        a_hash_including(
          "queued_message_id" => continuation.id,
          "source" => "goal_continuation",
        ...
        a_hash_including(
          "queued_message_id" => notice.id,
          "source" => "proposal_notification",
          "content" => a_hash_including("source" => "proposal_notification")
        )
      )
      expect(continuation.reload.delivered_at).to be_present
      expect(notice.reload.delivered_at).to be_present
    end

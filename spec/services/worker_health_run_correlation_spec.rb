...
RSpec.describe WorkerHealthRunCorrelation do
  ...
  it "reuses Run process rows when bounding multiple command span windows" do
    wf = workflow
    step = step_for(wf)
    run = run_for(step)
    ...
    2.times do |index|
      process = SpawnedProcess.create!(
        ...
        workflow: wf,
        kind: "grader",
        command: "bin/rspec spec/#{index}_spec.rb",
        hostname: "worker-a",
        started_at: now - 9.minutes,
        finished_at: now - 3.minutes,
        outcome: "orphaned"
      ...
      CommandSpan.create!(
        ...
        sequence: index + 1,
        name: "rspec #{index}",
        command_excerpt: process.command,
        hostname: "worker-a",
        started_at: now - 8.minutes,
        finished_at: nil,
        outcome: "incomplete"
    ...
    ActiveSupport::Notifications.subscribed(callback, "sql.active_record") do
      described_class.for_run(run, now: now)
    end

    sql = queries.join("\n")
    expect(sql.scan(/FROM ["`]?spawned_processes["`]?/i).size).to eq(1)
    expect(sql).not_to match(/FROM ["`]?runs["`]?/i)
  end

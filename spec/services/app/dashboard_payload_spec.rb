...
RSpec.describe App::DashboardPayload, :ci_only do
  ...
  describe "dashboard parity columns" do
    ...
    it "serializes and sorts epic dependency and child progress metrics" do
      ...
      slow_epic = Factories.epic(user: user, repository: repo, title: "Slow road")
      fast_epic = Factories.epic(user: user, repository: repo, title: "Fast road")
      ...
      expect(slow_item).to include(child_progress_percent: 0, open_child_count: 2, blocked_child_count: 1, dependency_count: 1, has_epic_dependency: true)
    end
  ...
  describe "job bulk action eligibility" do
    ...
    it "uses the preloaded latest workflow for retry eligibility" do
      job = Factories.job_record(user: user, repository: repo, kind: "direct", state: "queued", issue_number: nil, owner_user: user)
      Workflow.create!(job: job, trigger_kind: "initial", state: "failed")

      allow_any_instance_of(Job).to receive(:latest_workflow).and_raise("latest workflow should be preloaded")

      result = call(subject: "job", section: "rows")
      item = result.fetch(:items).find { |row| row.fetch(:id) == job.id }

      expect(item.fetch(:bulk_actions)).to include(retry: true)
    end
  end
end

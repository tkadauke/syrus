  ...
  describe "#setup" do
    context "fresh workflow (no prior workspace)" do
      ...
      it "reclones an invalid workspace from a published run checkpoint after an agentic step succeeded", ci_only: false do
        ...
        Dir.mktmpdir("syrus-wfws-run-checkpoint") do |work|
          sh("git clone -q #{bare_remote_dir} #{work}")
          sh("git -C #{work} checkout -q -b syrus/issue-7-#{job.id}")
          File.write(File.join(work, "implementation.rb"), "durable implementation\n")
          sh("git -C #{work} add implementation.rb")
          sh("git -C #{work} -c user.email=t@e -c user.name=t commit -q -m 'implementation'")
        ...
        expect(ws.path.join("implementation.rb").read).to eq("durable implementation\n")
      ...
      it "prefers the latest published source snapshot when recovering an invalid checkout after graders passed", ci_only: false do
        ...
        Dir.mktmpdir("syrus-wfws-source-snapshot") do |work|
          ...
          File.write(File.join(work, "formatted.rb"), "post-grader source\n")
          sh("git -C #{work} add formatted.rb")
          sh("git -C #{work} -c user.email=t@e -c user.name=t commit -q -m 'source snapshot'")
        ...
        workflow.steps.create!(kind: "coverage_analyze", position: 3, state: "queued")
        ...
        expect(ws.path.join("agent-output.tmp")).not_to exist
        expect(ws.path.join("implementation.rb").read).to eq("durable implementation\n")
        expect(ws.path.join("formatted.rb").read).to eq("post-grader source\n")

...
RSpec.describe WorkflowWorkspace, :ci_only do
  ...
  describe "#setup" do
    context "fresh workflow (no prior workspace)" do
      ...
      it "reclones an existing partial workspace before any step has succeeded" do
        ws = described_class.new(workflow)
        FileUtils.mkdir_p(ws.path)
        File.write(ws.path.join("clone-output.tmp"), "partial clone")
      ...
      it "prefers a newer repair checkpoint over an older grader source snapshot" do
        source_ref = "refs/syrus/source-snapshots/runs/#{SecureRandom.hex(4)}"
        repair_checkpoint_ref = "refs/syrus/checkpoints/runs/#{SecureRandom.hex(4)}"
        source_sha = nil
        source_tree_sha = nil
        repair_sha = nil

        Dir.mktmpdir("syrus-wfws-repair-checkpoint") do |work|
          sh("git clone -q #{bare_remote_dir} #{work}")
          File.write(File.join(work, "feature.rb"), "needs repair\n")
          sh("git -C #{work} add feature.rb")
          sh("git -C #{work} commit -q -m 'implement failing feature'")
          source_sha = sh("git -C #{work} rev-parse HEAD").strip
          source_tree_sha = sh("git -C #{work} rev-parse HEAD^{tree}").strip
          sh("git -C #{work} push -q origin HEAD:#{source_ref}")

          File.write(File.join(work, "feature.rb"), "repaired\n")
          sh("git -C #{work} add feature.rb")
          sh("git -C #{work} commit -q -m 'repair failing feature'")
          repair_sha = sh("git -C #{work} rev-parse HEAD").strip
          sh("git -C #{work} push -q origin HEAD:#{repair_checkpoint_ref}")
        end
        ...
        workflow.steps.create!(kind: "grader_collect", position: 3, state: "failed")
        repair_step = workflow.steps.create!(kind: "implement", position: 4, state: "succeeded")
        repair_run = repair_step.runs.create!(
          job: job,
          user: user,
          trigger_kind: workflow.trigger_kind,
          agent_provider: workflow.agent_provider,
        ...
        RunCheckpoint.create!(
          ...
          remote_ref: repair_checkpoint_ref,
          status: "published",
        ...
        ws = described_class.new(workflow)
        FileUtils.mkdir_p(ws.path)
        File.write(ws.path.join("agent-output.tmp"), "corrupt workspace debris before next fanout")

        expect { ws.setup }.not_to raise_error
        expect(ws.path.join("feature.rb").read).to eq("repaired\n")
        expect(sh("git -C #{ws.path} rev-parse HEAD").strip).to eq(repair_sha)
        expect(sh("git -C #{ws.path} rev-parse --abbrev-ref HEAD").strip).to eq("syrus/issue-7-#{job.id}")
      end
  ...
  describe ".base_ref_for" do
    ...
    it "uses the open parent branch when RebaseTarget would stack a job with a target_branch override" do
      parent = Factories.job_record(repository: repository, state: "implemented", branch_name: "syrus/direct-parent", pr_number: 41)

...
RSpec.describe SyrusMcp::Sidecar do
  ...
  def server_for(run)
    MCP::Server.new(
      ...
      tools: [ SyrusMcp::ReadLiveStateTool, SyrusMcp::ReadMemoryTool, SyrusMcp::SubmitSummaryTool, SyrusMcp::SubmitTestPlanTool, SyrusMcp::SubmitAdversarialReviewTool, SyrusMcp::ReportMainConcernTool ],
  ...
  describe "MCP handshake" do
    ...
    it "advertises the run-scoped tools via `tools/list`" do
      ...
      expect(tool_names).to eq(%w[read_live_state read_memory submit_summary submit_test_plan submit_adversarial_review report_main_concern])
    end

...
RSpec.describe ChatContextCompactor do
  ...
  # This compactor's only consumer was the admin Supervisor chat, now removed.
  # Widening it to every ordinary chat would be a real behavior/cost change
  # for any operator who already has this flag on, so it stays permanently
  # off regardless of the feature flag until a future Job decides whether to
  # generalize or delete it.
  it "does nothing for an ordinary chat even when the feature flag is enabled" do
    enable_compaction!
    session = chat
    add_messages(session, 130)

    expect(described_class.enabled_for?(session)).to be(false)
    expect {
      described_class.maybe_compact!(session)
    }.not_to change { ChatContextCheckpoint.count }
    expect(described_class.context_messages_for(session).size).to eq(130)
  end

  # The compaction algorithm itself stays fully covered (in case a future Job
  # re-enables it for some chat) by forcing the otherwise-permanent gate open
  # for the scope of these examples.
  context "with the compaction gate forced open" do
    before do
      allow_any_instance_of(described_class).to receive(:enabled_for_chat?).and_return(true)
    end

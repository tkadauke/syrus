...
RSpec.describe PollRepositoryJob do
  ...
  describe "#perform with per-issue controls" do
    ...
    it "creates an initial workflow whose first step is implement when the skip-prepare label is present" do
      ...
      expect(workflow.steps.order(:position).pluck(:kind)).to eq(%w[ implement format generate grader_fanout grader_collect coverage_analyze dependency_audit summarize test_plan pr_open review_plan ])
    ...
    it "keeps the initial workflow starting with prepare when the skip-prepare label is absent" do
      ...
      job.advance_after_triage!
      workflow = job.workflows.first
      expect(workflow.steps.order(:position).pluck(:kind)).to eq(%w[ prepare implement format generate grader_fanout grader_collect coverage_analyze dependency_audit summarize test_plan pr_open review_plan ])
  ...
  describe "#perform", vcr: { cassette_name: "poll_repository_job/lists_issues" } do
    ...
    it "creates a Job whose workflow skips prepare when the issue has the prepare-skip label" do
      ...
      expect(workflow.steps.pluck(:kind)).to eq(%w[ implement format generate grader_fanout grader_collect coverage_analyze dependency_audit summarize test_plan pr_open review_plan ])

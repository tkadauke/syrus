...
RSpec.describe Workflow, :ci_only do
  ...
  describe "#cleanup_workspace!" do
    ...
    it "reports deferred cleanup when the workspace helper sees a late active descendant" do
      wf = described_class.create!(job: job, trigger_kind: "initial", state: "succeeded", started_at: 2.minutes.ago, finished_at: 1.minute.ago)
      allow(WorkflowWorkspace).to receive(:cleanup_for).with(wf).and_return(false)
  ...
  describe "Job state propagation (Phase 2)" do
    ...
    it "marks Job failed on workflow.fail! for local_mode_handoff grader failures" do
      job.update!(state: "running")

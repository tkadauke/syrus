...
RSpec.describe Step::Kind do
  ...
  it "gives newer non-agentic steps deliberate product copy" do
    ...
    expect(described_class.label_for("local_mode_handoff_fix")).to eq("Repair handoff")
    expect(described_class.style_for("local_mode_handoff_fix")).to eq("bg-blue-100 text-blue-700")
  ...
  describe "fail_policy" do
    ...
    it "returns :default for all other step kinds" do
      %w[prepare implement respond summarize pr_open push auto_merge landing_fix coding_handoff_fix local_mode_handoff_fix].each do |kind|

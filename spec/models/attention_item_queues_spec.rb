...
RSpec.describe "AttentionItem queues" do
  ...
  it "routes the two queues separately" do
    ...
    expect(AttentionItem.operator_queue).to contain_exactly(operator)
    expect(AttentionItem.triage_queue).to contain_exactly(triage)
  end
  ...
  it "orders a queue by urgency, then age" do
    ...
    expect(AttentionItem.operator_queue.in_attention_order.to_a).to eq([ urgent, old_normal, low ])
  end
  ...
  it "summarizes one queue without counting the other" do
    ...
    expect(AttentionItem.queue_summary("operator")).to eq(queue: "operator", open: 1, by_urgency: { "urgent" => 1 })
  end
  ...
  it "leaves decided and expired work out of a queue summary" do
    ...
    expect(AttentionItem.queue_summary("operator")[:open]).to eq(0)
  end
end

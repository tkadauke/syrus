...
RSpec.describe User do
  ...
  describe ".agent_providers" do
    it "returns provider keys from the plugin registry" do
      expect(User.agent_providers).to contain_exactly("agy", "claude", "codex")
    end
  ...
  describe "#agent_provider_failover_policy" do
    ...
    it "persists an explicitly empty cause list" do
      user = User.create!(
        attrs.merge(
          agent_provider_failover_policy: {
            enabled: true,
      ...
      expect(user.reload.agent_provider_failover_policy["causes"]).to eq([])
      expect(user.agent_provider_failover_candidates(cause: "usage_exhausted")).to be_empty
    end

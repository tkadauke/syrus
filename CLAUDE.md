...
- **Go CLI** lives under `cli/` and talks to the app-scoped JSON API
  ...
  behavior; `cli/.syrus.yml` owns the core CLI prepare/test target, while the
  root `.syrus.yml` keeps a Go-workspace backstop for plugin-owned CLI modules
  and release/desktop packaging paths that span projects. For local broad
  validation, run `go test $(go list -m -f '{{.Dir}}/...')` from the repo root,

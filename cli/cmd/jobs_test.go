...
func TestJobsCommandFiltersClosedJobsByRepo(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	...
	command.SetErr(&bytes.Buffer{})
	command.SetArgs([]string{"jobs", "--closed", "--repo", "tkadauke/myapp"})
...
func TestJobsCommandTruncatesLongTitlesForEightyColumns(t *testing.T) {
	output := &bytes.Buffer{}

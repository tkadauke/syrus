...
func initCheckoutFixture(t *testing.T, branchName string) string {
	...
	localOrigin := filepath.Join(repoDir, "demo", "syrus-preview")
	if err := os.MkdirAll(filepath.Dir(localOrigin), 0o755); err != nil {
		t.Fatal(err)
	}
	runGit(t, "", "clone", "--bare", remoteDir, localOrigin)
	runGit(t, repoDir, "remote", "set-url", "origin", e2eRepoSlug)
	return repoDir
}
